...
- **Portal retry guard (2026-05-08)** — the LAF Gmail monitor no longer retries pending portal downloads at every MAGI boot, preventing surprise NAS/portal batches after restart; set `MAGI_LAF_PORTAL_RETRY_ON_START=1` to enable. Background file-review email/portal checks are off unless `MAGI_ENABLE_BACKGROUND_FILE_REVIEW_CHECK=1`; `file_review_auto_worker` also skips its first cycle unless `MAGI_FILE_REVIEW_AUTO_RUN_ON_START=1`.
