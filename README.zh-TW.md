...
### 重型雲端兜底 — NVIDIA NIM（Plan A，2026-04-19）

當本機 oMLX 失敗或需要 SOTA 推理時，MAGI 可向 NVIDIA NIM 免費雲端推理兜底：
...
| **結案辦理** | 依規則草擬結案申請，附正確備註格式；支援 `引用OOO的會議`（繼承他案次數）與 `OOO就是結案文件`（依關鍵字指定任意文件為結案依據） |
...
| `MAGI_LAF_OCR_CONSENSUS_SHADOW` | `1` | LAFVision shadow（三模組保護守則，需觀察一週真實開辦通知書） |
| `MAGI_LAF_OCR_CONSENSUS_ENABLE` | `0` | LAFVision 完全切換（shadow 未審查前禁止啟用） |
...
| `NVIDIA_NIM_ENABLE` | `0` | 啟用 NVIDIA NIM 雲端重型兜底（Plan A） |
...
pytest tests/test_tools_api_async_jobs.py       # 異步任務佇列 API（18 個測試）
...
pytest tests/test_translator_post_edit.py       # APE 後編輯管線（22 個測試）
...
| **基礎設施** | 33 | 218 | 健康探針、會話/context 管理、音頻管線、文字處理、日誌、packaging、entrypoint、安全基線（CORS / 標頭 / Cookie） |
...
| **文件與 PDF** | 7 | 86 | MarkItDown adapter、PDF bridge（OCR + timeout 恢復）、pdf-namer（命名驗證、動態信心度）、pdf-bookmarker（OLA 自適應閾值、Vision fallback） |
...
| **工具 API** | 8 | 76 | 工具優先管線、異步任務佇列（202/poll 模式）、推理閘道路由、Shortcuts 端點 |
| **翻譯** | 5 | 65 | 三層法學術語庫（MOJ SQLite / JSON / prompt）、Apple Translation + APE 後編輯驗證器、管線韌性、統一 API |
...
| **資料與持久化** | 6 | 45 | 任務佇列（SQLite）、embedding 路由器、遷移框架、DB helper、向量管線 NLP |
