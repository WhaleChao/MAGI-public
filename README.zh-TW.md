...
- `dist/installers/MAGI-macOS-Installer.dmg`：開啟後會啟動 Terminal 安裝精靈，自動偵測 Apple Silicon，協助安裝 MariaDB/Tailscale、準備 oMLX/MLX、下載建議模型，然後執行 MAGI 客戶安裝精靈。
- `dist/installers/windows/dist/MAGI-Setup.exe`：由 Windows 工作流程或在 Windows 執行 `dist/installers/windows/build_windows_exe.ps1` 產生；會自動偵測 Windows 硬體，協助安裝 MariaDB/Tailscale、準備 Ollama、下載適合的模型，並執行同一套安裝精靈。

只想預演 runtime/model 選擇，不改動電腦：
...
此分支已在 2026-06-23 後整理為 public-safe 發行分支：包含可 self-host 的安裝程式/精靈、公開文件、release gate wiring、正式 `magi` CLI 與公開安全的測試定義；不得包含私有正式環境 runtime。`.runtime/`、`.claude/`、`.claire/`、`runtime/supplement_cache/`、私有部署手札、本機信箱/NAS 標記、憑證、DB dump、客戶/案件檔、portal 截圖與 runtime 報告都應維持私有且不得被 git 追蹤。
...
- [一般使用者圖文操作手冊 DOCX](docs/guides/MAGI_一般使用者圖文操作手冊_2026-05-19.docx)
- [一般使用者圖文操作手冊 PDF](docs/guides/MAGI_一般使用者圖文操作手冊_2026-05-19.pdf)
- [一般使用者完整操作手冊 DOCX](docs/guides/MAGI_一般使用者完整操作手冊_2026-05-18.docx)
