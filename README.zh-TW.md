...
`first_run_setup.py` 是第一次使用者的引導入口：可建立本機 `.env`、列出缺少的必要設定、保留下一步命令，且不會列印 token 或密碼。`public_release_audit.py` 會阻擋高可信度 secret 與被追蹤的私有路徑；公開推送前請加上 `--public-isolation`，一併阻擋私有實務見解來源整合與私人信箱/NAS 標記。正式發布與商用部署請使用 `--strict`；發布分支預期應通過 `0 errors / 0 warnings`。
