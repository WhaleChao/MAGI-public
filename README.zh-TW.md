...
- `dist/installers/MAGI-macOS-Installer.dmg`：開啟後會啟動 Terminal 安裝精靈，自動偵測 Apple Silicon、準備 oMLX/MLX、下載建議模型，然後執行 MAGI 客戶安裝精靈。
- `dist/installers/windows/dist/MAGI-Setup.exe`：由 Windows 工作流程或在 Windows 執行 `dist/installers/windows/build_windows_exe.ps1` 產生；會自動偵測 Windows 硬體、準備 Ollama、下載適合的模型，並執行同一套安裝精靈。
