...
def _load_dotenv_value(name: str) -> str:
    env_path = Path(os.environ.get("MAGI_ENV_FILE") or MAGI_ROOT / ".env").expanduser()
    try:
...
def main() -> int:
    ...
    agent_dir = Path(
        ...
        or os.environ.get("MAGI_DATA_DIR", "").strip()
        or MAGI_ROOT / ".agent"
    ).expanduser()
    log_dir = Path(
        os.environ.get("MAGI_LOG_DIR", "").strip()
        or os.environ.get("MAGI_V3_LOG_DIR", "").strip()
        or (runtime_dir / "logs" if os.environ.get("MAGI_RUNTIME_DIR", "").strip() else MAGI_ROOT / "logs")
    ).expanduser()
