...
def business_jobs(repo_root: Path = REPO_ROOT, python_path: Path | None = None) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "desc": "閱卷可下載狀態高密度探測（平日 08:00-20:59 每 15 分鐘；只查入口不下載）",
            "channel_id": None,
            "last_run": None,
            "last_run_minute": None,
            "enabled": True,
        ...
        },
        {
            ...
            "desc": "Obsidian 來源文件匯入（每日 02:30；高價值資料夾，原生 PDF/OCR 優先，產生 v2 摘要）",
            "channel_id": None,
            "last_run": None,
            "last_run_minute": None,
            "enabled": True,
        ...
        },
        {
            ...
            "desc": "Obsidian Wiki 合成（每日 04:30；overview/timeline/parties/issues/evidence/tasks/procedural_status）",
            "channel_id": None,
            "last_run": None,
            "last_run_minute": None,
            "enabled": True,
        ...
        },
        {
            ...
            "command": guarded_cron_command(
                ...
                token_refresh_cron_command(
                    ...
                    qcmd(
                        ...
                        "MAGI_DRIVE_SYNC_DRIVE_LIST_TIMEOUT_SEC=60",
                        "MAGI_DRIVE_SYNC_API_RETRIES=2",
                        "MAGI_DRIVE_SYNC_LOCAL_HASH_MAX_BYTES=5000000",
                        "MAGI_DRIVE_SYNC_MAX_SINGLE_DOWNLOAD_BYTES=5000000",
                        "MAGI_DRIVE_SYNC_MAX_SINGLE_UPLOAD_BYTES=5000000",
                        "MAGI_DRIVE_SYNC_DEFERRED_DOWNLOADS_ARE_OK=1",
                        "MAGI_DRIVE_SYNC_DEFERRED_UPLOADS_ARE_OK=1",
                        "MAGI_DRIVE_SYNC_UNVERIFIED_EXISTING_ARE_OK=1",
                        "--",
                        python_bin,
                        repo_root / "scripts" / "drive_case_sync_worker.py",
                        "--direct-all-cases",
            ...
            "desc": "Google Drive/NAS 全案件小批量輪巡雙向同步（每 6 小時 1 案、30 分鐘 watchdog；錯開近期待辦同步避免鎖定互相飢餓；依 DB folder_path 輪巡，不改兩邊命名規則；缺檔才補，不覆蓋、不刪除）",
            "channel_id": None,
            "last_run": None,
            "last_run_minute": None,
            "enabled": True,
        ...
        },
        {
            ...
            "command": token_refresh_cron_command(
                ...
                qcmd(
                    ...
                    "OSC_EVENTS_REFRESH_CALENDAR_LIMIT=120",
                    "OSC_EVENTS_REFRESH_GCAL_PUSH_LIMIT=60",
                    "OSC_PDF_CALENDAR_FULL_FILENAME_SWEEP=1",
                    "OSC_PDF_CALENDAR_FILENAME_SWEEP_LIMIT=8000",
                    "OSC_PDF_CALENDAR_FULL_TEXT_SCAN=0",
                    "OSC_EVENTS_REFRESH_PDF_LIMIT=120",
                    "OSC_EVENTS_REFRESH_SCAN_BUDGET_SEC=480",
                    "OSC_EVENTS_REFRESH_TRANSCRIPT_LIMIT=40",
                    "OSC_TRANSCRIPT_TODO_TIMEOUT_SEC=60",
                    python_bin,
                    repo_root / "scripts" / "ops" / "osc_events_refresh.py",
                    "--skip-drive-sync",
                    "--skip-transcript-todos",
                    "--skip-calendar-audit",
                    "--skip-calendar-source-audit",
                    "--skip-share-repair",
                ),
            ),
            "desc": "OSC/PDF 待辦與行事曆快刷（每 6 小時；只做 bounded PDF 檔名掃描與 GCal import/push；Drive/NAS 補檔、筆錄與深掃留給每日治理）",
            "channel_id": None,
            "last_run": None,
            "last_run_minute": None,
            "enabled": True,
        ...
        },
        {
            ...
            "command": token_refresh_cron_command(
                ...
                qcmd(
                    ...
                    "OSC_PDF_CALENDAR_FULL_TEXT_ALL_CASES=0",
                    "OSC_PDF_CALENDAR_FULL_TEXT_SCAN_LIMIT=40",
                    "OSC_PDF_CALENDAR_FILE_TIMEOUT_SEC=8",
                    "OSC_EVENTS_REFRESH_PDF_LIMIT=40",
                    "OSC_EVENTS_REFRESH_PDF_CASE_BATCH=20",
                    "OSC_EVENTS_REFRESH_SCAN_BUDGET_SEC=300",
                    "OSC_EVENTS_REFRESH_SOURCE_AUDIT_DRIVE_REMEDIATE_ENABLE=0",
                    python_bin,
                    repo_root / "scripts" / "ops" / "osc_events_refresh.py",
                    "--force-rebuild",
                    "--skip-drive-sync",
            ...
            "desc": "OSC 待辦治理巡檢（每日 03:35；輪巡掃描法院 PDF 檔名與正文、補漏、重複/殘影日曆清理；Drive/NAS 補檔交由獨立同步排程負責）",
            "channel_id": None,
            "last_run": None,
            "last_run_minute": None,
            "enabled": True,
...
def operational_jobs(repo_root: Path = REPO_ROOT, python_path: Path | None = None) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "desc": "功能健康索引（每日 06:10；彙整凌晨資料任務結果，唯讀）",
            "channel_id": None,
            "last_run": None,
            "last_run_minute": None,
            "enabled": True,
        ...
        },
        {
            ...
            "command": guarded_cron_command(
                ...
                qcmd(
                    python_bin,
                    repo_root / "scripts" / "ops" / "resummary_legacy_judgments_quality.py",
                    "--scope",
                    "court",
                    "--limit",
                    "12",
            ...
            "desc": "判決摘要品質持續清理（每日小批次；來源支持後才寫回）",
            "channel_id": None,
            "last_run": None,
            "last_run_minute": None,
            "enabled": True,
