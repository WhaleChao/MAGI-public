...
def business_jobs(repo_root: Path = REPO_ROOT, python_path: Path | None = None) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "cron": "20 9 * * 1,5",
            "command": f"{python_bin} {run_with_env} -- {python_bin} {repo_root / 'scripts' / 'import_accounting_sheet.py'} --commit --include-previous",
        ...
        },
        {
            ...
            "cron": "0 12 * * *",
            "command": f"{python_bin} {run_with_env} -- {python_bin} {repo_root / 'scripts' / 'accounting_monthly_bonus.py'} --commit --refresh-import --catch-up --export-xlsx",
        ...
        },
        {
            ...
            "desc": "OSC/PDF/筆錄待辦與行事曆事件更新（每 6 小時；先補 Drive/NAS 缺檔，再檔名全量巡檢 + bounded OCR/text 補漏 + GCal 去重/修復；缺來源補到檔時當輪二次掃描與推送）",
            "channel_id": None,
            "last_run": None,
            "last_run_minute": None,
            "enabled": True,
    ...
    return [
        ...
        },
        {
            ...
            "command": guarded_cron_command(
                ...
                f"{python_bin} {run_with_env} -- {python_bin} {repo_root / 'scripts' / 'ops' / 'heavy_translation_quality_live.py'} --json-out {repo_root / '.runtime' / 'heavy_translation_quality_latest.json'}",
                block_at="core_only",
                timeout_sec=420,
            ),
