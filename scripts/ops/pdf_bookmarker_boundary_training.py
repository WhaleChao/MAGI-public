...
def main() -> int:
    agent_dir = Path(os.environ.get("MAGI_AGENT_DIR") or str(ROOT / ".agent"))
    runtime_dir = Path(os.environ.get("MAGI_RUNTIME_DIR") or str(ROOT / ".runtime"))
    parser = argparse.ArgumentParser(description="Export pdf-bookmarker no-boundary training data.")
    parser.add_argument("--state", default=str(agent_dir / "bookmark_batch_state.json"))
    parser.add_argument("--out", default=str(runtime_dir / "pdf_bookmarker_boundary_training_latest.jsonl"))
    parser.add_argument("--summary", default=str(runtime_dir / "pdf_bookmarker_boundary_training_summary.json"))
    ...
    if not state_path.is_absolute():
        state_path = (agent_dir if os.environ.get("MAGI_AGENT_DIR") else ROOT) / state_path
    if not out_path.is_absolute():
        out_path = (runtime_dir if os.environ.get("MAGI_RUNTIME_DIR") else ROOT) / out_path
    if not summary_path.is_absolute():
        summary_path = (runtime_dir if os.environ.get("MAGI_RUNTIME_DIR") else ROOT) / summary_path
