...
from api.osc.calendar_sources import calendar_todo_source_sql, osc_todo_source_sql

RUNTIME_DIR = Path(os.environ.get("MAGI_RUNTIME_DIR", "").strip() or ROOT / ".runtime").expanduser()
LATEST_PATH = RUNTIME_DIR / "osc_events_refresh_latest.json"
INVOCATION_PATH = RUNTIME_DIR / "osc_events_refresh_invocation_latest.json"
PDF_SCAN_CACHE_PATH = RUNTIME_DIR / "pdf_calendar_scan_cache.json"
PDF_SCAN_CURSOR_PATH = RUNTIME_DIR / "pdf_calendar_scan_cursor.json"
...
def _pdf_scan_worker_timeout_sec(args: argparse.Namespace) -> int:
    ...
    if configured_budget and refresh_budget:
        ...
        effective_budget = (
            ...
            if full_corpus
            else min(configured_budget, refresh_budget)
        )
    else:
...
def _run_pdf_calendar_scan(args: argparse.Namespace) -> dict[str, Any]:
    """Run the read-only corpus scan without starving behind Drive writes.

    The case-file lock is an exclusive *mutation* lock.  Holding it for a
    potentially hour-long read-only PDF crawl caused every calendar refresh to
    lose whenever the twenty-minute Drive sync happened to be active.  The
    scanner already treats a disappearing/unreadable file as an item-level
    retry, so the safe unattended default is a non-exclusive coordination
    receipt.  Deployments may opt back into the old exclusive policy while
    diagnosing a storage appliance with ``OSC_PDF_CALENDAR_EXCLUSIVE_LOCK=1``.
    """
    try:
        ...
        return {
            "ok": False,
            "skipped": True,
            "reason": "case_file_operation_lock_unavailable",
            "error": f"{type(exc).__name__}: {str(exc)[:160]}",
        }

    exclusive = str(
        os.environ.get("OSC_PDF_CALENDAR_EXCLUSIVE_LOCK", "0") or "0"
    ).strip().lower() in {"1", "true", "yes", "on"}
    if not exclusive:
        lock = acquire_case_file_operation_lock(
            owner="osc_events_refresh:pdf_calendar_scan",
            exclusive=False,
        )
        if not lock.get("acquired"):
            return {
                "ok": False,
                "skipped": True,
                "reason": "case_file_read_coordination_unavailable",
                "active_owner": lock.get("active_owner") or {},
            }
        result = _run_pdf_calendar_scan_isolated(args)
        result["case_file_operation_lock"] = {
            "acquired": True,
            "exclusive": False,
            "read_only": True,
            "owner": "osc_events_refresh:pdf_calendar_scan",
    ...
    wait_sec = max(0, min(300, int(os.environ.get("OSC_PDF_CALENDAR_LOCK_WAIT_SEC", "15") or "15")))
    deadline = time.monotonic() + wait_sec
    lock: dict[str, Any] = {}
    while True:
        lock = acquire_case_file_operation_lock(owner="osc_events_refresh:pdf_calendar_scan")
        ...
        if time.monotonic() >= deadline:
            return {
                "ok": False,
                "skipped": True,
                "reason": "case_file_operation_lock_busy",
                "wait_sec": wait_sec,
                "active_owner": lock.get("active_owner") or {},
            }
        time.sleep(1.0)
...
def _run_pdf_calendar_scan_isolated(args: argparse.Namespace) -> dict[str, Any]:
    """Run native PDF parsing behind a process timeout and stderr boundary.

    PyMuPDF may be inside a SWIG/C++ diagnostic callback when a POSIX alarm
    fires.  Raising from that callback produces a traceback on stderr even
    though the refresh catches the timeout and returns success.  The cron
    policy correctly treats such tracebacks as hard failures.  Isolating the
    complete PDF scan keeps native diagnostics out of the scheduler process and
    lets the parent enforce a bounded timeout without interrupting C++ code.
    """
    worker_timeout = _pdf_scan_worker_timeout_sec(args)
    worker_env = os.environ.copy()
    worker_env["OSC_EVENTS_REFRESH_PDF_SCAN_WORKER"] = "1"
    # The parent process is the timeout boundary.  Never inject SIGALRM into
    # PyMuPDF/RapidOCR native callbacks in the worker.
...
def _run_pdf_calendar_scan_in_process(args: argparse.Namespace) -> dict[str, Any]:
    ...
    manual_confirmation_inserted = 0
    manual_confirmation_skipped = 0
    quarantined_todos: list[dict[str, Any]] = []
    timeout_count = 0
    error_count = 0
    cache_skipped = 0
            ...
            if PDF_SCAN_CACHE_PATH.exists():
                max_cache_bytes = max(
                    ...
                    min(
                        ...
                        int(
                            os.environ.get(
                                "OSC_PDF_CALENDAR_CACHE_MAX_BYTES", str(32 * 1024 * 1024)
                            )
                            or str(32 * 1024 * 1024)
                        ),
                    ),
                )
                if PDF_SCAN_CACHE_PATH.stat().st_size > max_cache_bytes:
                ...
                if isinstance(data, dict):
                    ...
                    if isinstance(files, dict) and len(files) > 20000:
                        ordered = sorted(
                            files.items(),
                            key=lambda item: str((item[1] or {}).get("scanned_at") or ""),
                        )
    ...
    case_batch = max(configured_case_batch, total_case_rows or configured_case_batch) if full_text_scan and full_text_all_cases else configured_case_batch
    ...
    elif bool(getattr(args, "force_rebuild", False)) and not full_text_scan:
        case_offset = 0
    else:
    ...
    configured_target_timeout = int(
        os.environ.get("OSC_PDF_CALENDAR_TARGET_TIMEOUT_SEC", "45") or "45"
    )
    if full_text_scan and full_text_all_cases:
        # A complete 130+ case inventory can legitimately exceed the legacy
        # 180-second directory walk, especially over SMB.  Keep it bounded but
        # give enumeration up to fifteen minutes within the explicit corpus
        # budget so the cursor can actually advance.
        configured_target_timeout = max(
            configured_target_timeout,
            min(max(10, budget_sec or 10), 900),
        )
    target_timeout_sec = max(
        5,
        min(max(10, budget_sec or 10), configured_target_timeout),
    )
    ...
    for path, case_number, client_name, use_text, scan_mode in target_specs:
        try:
            ...
            identity_confirmation = item.get("identity_confirmation") or {}
            if identity_confirmation:
                identity_conflict_count += 1
                if not dry_run and item.get("case_number"):
                    confirmation_result = osc_pdf._insert_document_identity_confirmation(
                        path=Path(item.get("path") or path),
                        case_number=str(item.get("case_number") or ""),
                        client_name=str(item.get("client_name") or ""),
                        identity_confirmation=dict(identity_confirmation),
                    )
                    manual_confirmation_inserted += int(confirmation_result.get("inserted") or 0)
                    manual_confirmation_skipped += int(confirmation_result.get("skipped") or 0)
            ...
            elif raw_todos and dry_run and len(sample_items) < 12:
                sample_items.append(
                    {
                        "case_number": item.get("case_number") or case_number,
                        "client_name": item.get("client_name") or client_name,
                        "file_name": path.name,
                        "scan_mode": scan_mode,
                        "todo_count": len(raw_todos),
                        "event_count": len(raw_todos),
                        "write_result": write_result,
                        "todos": raw_todos[:3],
                        "diagnostic": "filtered_after_scan",
                    }
                )
        except _PdfScanTimeout as exc:
            timeout_count += 1
    ...
    next_case_offset = 0 if full_text_scan and full_text_all_cases else ((case_offset + case_batch) if total_case_rows else case_offset)
    if total_case_rows and next_case_offset >= total_case_rows:
        next_case_offset = 0
    if not dry_run and not (full_text_scan and full_text_all_cases):
        try:
            _save_cursor(
                {
                    "version": 1,
    ...
    return {
        ...
        "manual_confirmation_inserted": manual_confirmation_inserted,
        "manual_confirmation_skipped": manual_confirmation_skipped,
...
def _write_already_running_result(data: dict[str, Any], explicit_out: str = "") -> None:
    """Audit a skipped invocation without replacing the last full refresh."""
    if explicit_out:
        _write_latest(data, Path(explicit_out))
        return
    audit = dict(data)
    audit["canonical_latest_preserved"] = LATEST_PATH.exists()
    _write_latest(audit, INVOCATION_PATH)


def _drive_presync_bounded_partial_is_success(returncode: int, parsed: dict[str, Any]) -> bool:
    """Treat a fully successful bounded slice as resumable, not failed.

    The Drive worker intentionally exits 75 when a download/byte ceiling is
    reached so its own rotating cursor retries the same slice.  For the
    calendar pre-sync this is a successful bounded contribution when every
    attempted file completed and there are no hard failures or unverified
    files; the remaining files are handled by the next scheduled slice.
    """
    if int(returncode) != 75:
        return False
    execution = parsed.get("execution_summary") or {}
    if not isinstance(execution, dict):
        return False
    bounded = bool(execution.get("stopped_by_limit") or execution.get("stopped_by_bytes"))
    hard_keys = (
        "failed",
        "download_failed",
        "upload_failed",
    ...
    try:
        hard_failures = sum(int(execution.get(key) or 0) for key in hard_keys)
        attempted = int(execution.get("attempted") or 0)
        completed = (
            int(execution.get("downloaded") or 0)
            + int(execution.get("uploaded") or 0)
            + int(execution.get("skipped_existing") or 0)
        )
    except (TypeError, ValueError):
        return False
    return bool(bounded and hard_failures == 0 and attempted > 0 and completed >= attempted)
...
def _run_drive_case_sync_before_pdf(args: argparse.Namespace) -> dict[str, Any]:
    ...
    all_cases = bool(getattr(args, "drive_sync_all_cases", False))
    download_limit = max(0, int(getattr(args, "drive_sync_download_limit", 16)))
    ...
    cmd = [
        ...
        "--max-case-items",
        str(max(1, int(getattr(args, "drive_sync_max_case_items", 160)))),
        ...
        "--priority-case-limit",
        str(max(1, min(int(getattr(args, "drive_sync_priority_case_limit", 24)), 48))),
        "--inventory-timeout-sec",
        str(max(30, int(getattr(args, "drive_sync_timeout_sec", 420)))),
    ]
    if download_limit <= 0:
        cmd.append("--no-downloads")
    else:
        cmd.extend(["--execute-downloads", "--download-limit", str(download_limit)])
    if upload_limit <= 0:
        cmd.append("--no-uploads")
    else:
        cmd.extend(["--execute-uploads", "--upload-limit", str(upload_limit)])
    # This hook is a file remediation step, never a Drive case-provisioning
    # path.  Missing Drive case folders are reported and skipped.
    cmd.append("--no-create-drive-folders")
    if all_cases:
        cmd.extend([
            "--direct-all-cases",
            "--direct-all-case-limit",
            str(max(1, min(int(getattr(args, "drive_sync_all_case_limit", 12)), 12))),
        ])
    else:
        cmd.extend([
            "--direct-priority-case-limit",
            str(max(1, min(int(getattr(args, "drive_sync_priority_direct_limit", 8)), 12))),
        ])
    ...
    timeout_sec = max(60, int(getattr(args, "drive_sync_timeout_sec", 420)) + 30)
    worker_dir = RUNTIME_DIR / "drive_presync_workers"
    worker_dir.mkdir(parents=True, exist_ok=True)
    worker_id = f"{os.getpid()}-{time.time_ns()}"
    stdout_path = worker_dir / f"{worker_id}.stdout.log"
    stderr_path = worker_dir / f"{worker_id}.stderr.log"
    ...
    try:
        with stdout_path.open("wb") as stdout_handle, stderr_path.open("wb") as stderr_handle:
            ...
            if proc.poll() is None:
                ...
                return {
                    ...
                    "error": f"drive_case_sync_timeout:{timeout_sec}s",
                    "timeout_isolated": True,
                    "process_group_terminated": True,
                    "worker_pid": proc.pid,
                    "worker_stdout_path": str(stdout_path),
        ...
        return {
            ...
            "status": "failed",
            "mode": "direct_all_cases" if all_cases else "priority_cases",
            "error": f"drive_case_sync_worker_failed:{type(exc).__name__}: {str(exc)[:180]}",
            "timeout_isolated": True,
            "worker_stdout_path": str(stdout_path),
            "worker_stderr_path": str(stderr_path),
        }
    ...
    returncode = int(proc.returncode if proc and proc.returncode is not None else -1)
    bounded_partial = _drive_presync_bounded_partial_is_success(returncode, parsed)
    if returncode != 0 and not bounded_partial:
        return {
            "ok": False,
            "status": "failed",
            "mode": "direct_all_cases" if all_cases else "priority_cases",
    ...
    try:
        ...
        pass
    return {
        "ok": True if bounded_partial else bool(parsed.get("ok", True)),
        "status": "partial_complete" if bounded_partial else "ok",
        "resumable": bounded_partial,
        "timeout_isolated": True,
...
def _run_calendar_gap_drive_remediation(
    ...
    *,
    ...
    # A disconnected NAS/Drive mount can leave Python in an uninterruptible
    # filesystem call.  SIGALRM cannot safely unwind that state and used to
    # leave the whole calendar governance job stuck indefinitely.  Put the
    # remediation in its own process group and exchange the result through
    # files (not pipes), so the parent can return at the declared deadline even
    # when the child is waiting inside the kernel.
    timeout_sec = max(5, int(os.environ.get("OSC_EVENTS_REFRESH_SOURCE_AUDIT_DRIVE_TIMEOUT_SEC", "180") or "180"))
    worker_dir = RUNTIME_DIR / _DRIVE_REMEDIATION_WORKER_DIRNAME
    worker_dir.mkdir(parents=True, exist_ok=True)
    worker_id = f"{os.getpid()}-{time.time_ns()}"
    input_path = worker_dir / f"{worker_id}.input.json"
    result_path = worker_dir / f"{worker_id}.result.json"
    ...
    tmp_input_path = input_path.with_suffix(".input.json.tmp")
    tmp_input_path.write_text(json.dumps(payload, ensure_ascii=False, sort_keys=True), encoding="utf-8")
    tmp_input_path.replace(input_path)
    cmd = [
        sys.executable,
        str(Path(__file__).resolve()),
        "--drive-remediation-worker",
        "--worker-input",
        str(input_path),
        "--worker-output",
        str(result_path),
    ...
    try:
        ...
        if not result_path.exists():
            return {
                "ok": False,
                "reason": "drive_remediation_result_missing",
                "case_numbers": case_numbers,
                "timeout_sec": timeout_sec,
                "timeout_isolated": True,
                "worker_returncode": int(proc.returncode if proc and proc.returncode is not None else -1),
        ...
        result = json.loads(result_path.read_text(encoding="utf-8"))
        if not isinstance(result, dict):
            raise TypeError("drive remediation result must be an object")
        result["timeout_isolated"] = True
        result["worker_returncode"] = int(proc.returncode if proc and proc.returncode is not None else -1)
        if result.get("ok") and result["worker_returncode"] != 0:
            result["ok"] = False
            result["error"] = f"drive_remediation_worker_failed:returncode={result['worker_returncode']}"
        return result
    except Exception as exc:
        if proc is not None and proc.poll() is None:
            try:
        ...
        return {
            ...
            "timeout_sec": timeout_sec,
            "timeout_isolated": True,
            "error": f"drive_remediation_worker_failed:{type(exc).__name__}: {str(exc)[:180]}",
            "worker_stderr_path": str(stderr_path),
        }
    finally:
        try:
...
def _run_calendar_gap_drive_remediation_in_process(
    case_numbers: list[str],
    *,
    args: argparse.Namespace,
) -> dict[str, Any]:
    """Run Drive remediation inside the disposable worker process."""
...
def _run_calendar_source_audit(args: argparse.Namespace) -> dict[str, Any]:
        ...
        try:
            cur.execute(
                ...
                WHERE COALESCE(g.case_number, '') != ''
                  ...
                  AND {calendar_todo_source_sql('g.source_file', 'g.todo_type')}
    ...
    # Source gaps are evidence deficits, not authorization to mutate imported
    # Google events.  In dry-run mode even Drive remediation is suppressed so
    # the receipt is provably zero-mutation across Calendar, DB and storage.
    if bool(getattr(args, "dry_run", False)):
        drive_remediation = {
            "ok": True,
            "dry_run": True,
            "skipped": True,
            "reason": "source_gap_retained_pending_evidence_zero_mutation",
        }
    else:
    ...
    return {
        ...
        "mutation_policy": "retain_pending_evidence_never_create_or_delete_google_event",
        "message": "這些是人工/Google 匯入且看起來應由 PDF 支撐的行程；缺來源時一律保留待補證，絕不因此刪除或新建 Google 行程。",
    }
...
def _calendar_source_audit_downloaded_count(audit: dict[str, Any]) -> int:
    ...
    return max(
        ...
        _safe_int(summary.get("skipped_existing")),
        _safe_int(summary.get("download_skipped_existing")),
    )
...
def _apply_outcome_semantics(result: dict[str, Any]) -> dict[str, Any]:
    """Separate a successful partial refresh from an actual execution failure.

    Warnings represent quality follow-up that the UI should keep visible as an
    amber partial state.  They must not trigger the cron runner's terminal
    failure/retry contract when every required component completed safely.
    """
    ...
    result["success"] = result.get("ok") is True
    return result
...
def _run_refresh_locked(args: argparse.Namespace) -> dict[str, Any]:
    ...
    scan_only_all_mutating_steps_skipped = (
        bool(getattr(args, "scan_only", False))
        and bool(getattr(args, "skip_drive_sync", False))
        and bool(getattr(args, "skip_pdf_todos", False))
        and bool(getattr(args, "skip_transcript_todos", False))
    )
    if scan_only_all_mutating_steps_skipped:
        result["historical_todo_completion"] = {
            "ok": True,
            "skipped": True,
            "reason": "scan_only_all_mutating_steps_skipped",
            "dry_run": bool(getattr(args, "dry_run", False)),
        }
    else:
    ...
    def _run_transcript_todo_step() -> dict[str, Any]:
        try:
            ...
            transcript_timeout = env_timeout if env_timeout > 0 else 600
    ...
    if not args.scan_only:
        if bool(getattr(args, "dry_run", False)):
            ...
            if getattr(args, "skip_share_repair", False):
                result["pdf_todo_share_repair"] = {"ok": True, "skipped": True, "reason": "skip_share_repair"}
            else:
                try:
                    ...
                    elif int(result["pdf_todo_share_repair"].get("unresolved") or 0) > 0:
                        result["warnings"].append("pdf_todo_share_repair_pending")
                except _PdfScanTimeout as exc:
            ...
            if getattr(args, "skip_calendar_source_audit", False):
                result["calendar_source_audit"] = {"ok": True, "skipped": True, "reason": "skip_calendar_source_audit"}
            else:
    ...
    if not args.calendar_only and not getattr(args, "skip_transcript_todos", False):
        ...
        if result["transcript_todos"].get("deferred"):
            result["warnings"].append("transcript_todo_source_retry_pending")
    ...
    _apply_outcome_semantics(result)
...
def run_refresh(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not refresh_lock.acquired:
        ...
        _write_already_running_result(result, str(getattr(args, "json_out", "") or ""))
        return result
    try:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--drive-sync-all-case-limit", type=int, default=int(os.environ.get("OSC_EVENTS_REFRESH_DRIVE_SYNC_ALL_CASE_LIMIT", "12")))
    parser.add_argument("--drive-sync-priority-direct-limit", type=int, default=int(os.environ.get("OSC_EVENTS_REFRESH_DRIVE_SYNC_PRIORITY_DIRECT_LIMIT", "8")))
    parser.add_argument("--drive-sync-priority-case-limit", type=int, default=int(os.environ.get("OSC_EVENTS_REFRESH_DRIVE_SYNC_PRIORITY_CASE_LIMIT", "24")))
    ...
    parser.add_argument("--drive-sync-download-limit", type=int, default=int(os.environ.get("OSC_EVENTS_REFRESH_DRIVE_SYNC_DOWNLOAD_LIMIT", "16")))
    ...
    parser.add_argument("--drive-sync-max-case-items", type=int, default=int(os.environ.get("OSC_EVENTS_REFRESH_DRIVE_SYNC_MAX_CASE_ITEMS", "160")))
    ...
    parser.add_argument("--drive-sync-timeout-sec", type=int, default=int(os.environ.get("OSC_EVENTS_REFRESH_DRIVE_SYNC_TIMEOUT_SEC", "420")))
    ...
    parser.add_argument("--skip-calendar-source-audit", action="store_true")
    parser.add_argument("--skip-share-repair", action="store_true")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.pdf_scan_worker:
        try:
            payload = json.loads(sys.stdin.read() or "{}")
            if not isinstance(payload, dict):
                raise TypeError("worker payload must be an object")
            worker_args = argparse.Namespace(**payload)
            os.environ["OSC_PDF_CALENDAR_FILE_TIMEOUT_SEC"] = "0"
            result = _run_pdf_calendar_scan_in_process(worker_args)
        except Exception as exc:
            result = {
                "ok": False,
                "error": f"pdf_scan_worker_exception:{type(exc).__name__}: {str(exc)[:180]}",
            }
        print(
            _PDF_SCAN_WORKER_RESULT_MARKER
            + json.dumps(_json_safe(result), ensure_ascii=False, sort_keys=True)
        )
        return 0 if result.get("ok") else 1
    if args.drive_remediation_worker:
        output_path = Path(str(args.worker_output or "")).expanduser()
        try:
            input_path = Path(str(args.worker_input or "")).expanduser()
            payload = json.loads(input_path.read_text(encoding="utf-8"))
            case_numbers = [str(value).strip() for value in (payload.get("case_numbers") or []) if str(value).strip()]
            worker_args = argparse.Namespace(**(payload.get("args") or {}))
            result = _run_calendar_gap_drive_remediation_in_process(case_numbers, args=worker_args)
        except Exception as exc:
            result = {
                "ok": False,
...
def _hard_exit_after_flush(
    ...
    *,
    ...
    """Exit after durable output without running unstable native destructors.

    PDF/OCR libraries occasionally segfault during interpreter teardown after the
    complete, valid refresh report has already been atomically persisted.  The
    refresh is already a scheduler child process, so flushing its two streams and
    using ``os._exit`` is the narrow process-isolation boundary: locks and reports
    have been finalized by ``main`` while native module destructors are skipped.
    """

    stdout = sys.stdout if stdout is None else stdout
    stderr = sys.stderr if stderr is None else stderr
    exit_fn = os._exit if exit_fn is None else exit_fn
...
if __name__ == "__main__":
    exit_code = main()
    if str(os.environ.get("OSC_EVENTS_REFRESH_HARD_EXIT_AFTER_FLUSH", "1")).strip().lower() not in {
        "0",
        "false",
        "no",
        "off",
    }:
        _hard_exit_after_flush(exit_code)
