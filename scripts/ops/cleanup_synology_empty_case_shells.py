...
def _scan_local_synology_roots() -> bool:
    """Allow a full File Provider scan only through an explicit safe override.

    The legacy INCLUDE_LOCAL switch still permits removal of exact sibling
    shell paths derived from a verified NAS shell.  It must not make the
    scheduler enumerate an entire CloudStorage tree, because macOS can block
    indefinitely while materialising a dataless folder.
    """

    return os.environ.get(
        "MAGI_CLEAN_EMPTY_CASE_SHELL_SCAN_LOCAL",
        "0",
...
def _active_roots() -> list[str]:
    if os.environ.get("MAGI_V3_SCHEDULE_ADAPTER", "").strip() == "real_entrypoint_fixture_v1":
        fixture_raw = os.environ.get("MAGI_V3_SCHEDULE_FIXTURE_ROOT", "").strip()
        roots_raw = os.environ.get("MAGI_CLEAN_EMPTY_CASE_SHELL_ROOTS", "").strip()
        if not fixture_raw or not roots_raw:
            raise RuntimeError("schedule adapter active case roots are not bound to a fixture")
        fixture_root = Path(fixture_raw).expanduser().resolve()
        ...
        for raw in roots_raw.split(os.pathsep):
            ...
            try:
                ...
                raise RuntimeError("schedule adapter active case root escaped its fixture") from exc
            fixture_roots.append(str(candidate))
        if not fixture_roots:
            raise RuntimeError("schedule adapter has no active case root")
        return fixture_roots
    roots: list[str] = []
    include_local = _scan_local_synology_roots()
...
def _real_file_count(folder: str, *, timeout: float = 10.0) -> tuple[int, int]:
    """Return whether a shell contains a real file without hanging the worker.

    Synology File Provider and unhealthy SMB paths can block ``os.walk`` in
    the scheduler process indefinitely.  ``find`` runs in a killable child and
    stops at the first non-placeholder file.  A timeout/error is treated as
    unsafe to delete by the caller.
    """

    command = ["/usr/bin/find", folder, "-type", "f"]
    for name in sorted(IGNORED_FILENAMES):
    ...
    if result.returncode != 0:
        raise OSError(
            (result.stderr or result.stdout or f"find exited {result.returncode}").strip()
        )
    return (1 if (result.stdout or "").strip() else 0), 0
...
def run(*, apply: bool, limit: int, max_seconds: float = 0.0) -> dict:
    ...
    try:
        ...
        for case_number, folder in _iter_active_case_shells():
            ...
            checked += 1
            remaining = (
                max(0.5, max_seconds - (time.monotonic() - started))
                if max_seconds > 0
                else 10.0
            )
            try:
                real_files, dirs = _real_file_count(
                    folder,
                ...
                errors.append(
                    {
                        "folder": folder,
                        "reason": "folder_probe_unavailable",
            ...
            if real_files == 0:
                ...
                for peer in peer_paths:
                    ...
                    try:
                        peer_real_files, peer_dirs = _real_file_count(
                            peer,
                            timeout=min(10.0, remaining),
                        )
                    except (OSError, subprocess.TimeoutExpired) as exc:
