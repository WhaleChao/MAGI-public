...
_SELF_CRON_DESC_TO_ID = {
    "MAGI 自我修復守護": "job_magi_self_repair_guardian",
    "功能健康索引": "job_function_health_index",
}


def _failed_cron_job_ids(detail: str) -> set[str]:
    """Extract only known cron identifiers from Doctor's compact detail.

    Doctor may use the schedule's localized description instead of its ID.
    Match those descriptions only at an entry boundary, so unrelated prose can
    never suppress a real fault.
    """
    found: set[str] = set()
    for raw_entry in re.split(r"[;\n]+", str(detail or "")):
        ...
        if not entry:
            continue
        explicit_ids = set(re.findall(r"\bjob_[A-Za-z0-9_]+", entry))
        if explicit_ids:
            found.update(explicit_ids)
            continue
        matched_self = False
        for description, job_id in _SELF_CRON_DESC_TO_ID.items():
            if entry.startswith(description) and re.search(r"[:：]", entry):
                found.add(job_id)
                matched_self = True
                break
        if not matched_self:
            found.add("__unclassified_cron_failure__")
    return found
...
def _collect_doctor_issues(report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for check in report.get("checks") or []:
        ...
        if name == "cron_state_failures":
            ...
            failed_job_ids = _failed_cron_job_ids(detail)
...
def _collect_health_issues(report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for key, severity, label in specs:
        for item in runtime_health.get(key) or []:
            ...
            display_name = str(item.get("name") or path)
            ...
            issues.append(
                _issue(
                    ...
                    severity=severity,
                    summary=f"{display_name}：{item.get('reason') or label}",
                    evidence=item,
                    recommendation=(
                        "MAGI 會先按該作業的重試規則重跑，成功後重建功能健康索引；"
                        "不會藉由刪除健康證據清除紅燈。"
                    ),
                )
            )
...
def _persist_evolution_proposals(
    *, root: Path, runtime_dir: Path, issues: list[dict[str, Any]]
) -> dict[str, Any]:
    """Persist de-identified source-evolution proposals, never source edits.

    Known low-risk ``auto_repair`` actions stay with the guardian.  Every other
    warning/error becomes a candidate-only proposal whose lifecycle is owned
    by :mod:`magi_v3.controlled_evolution`.  That controller intentionally has
    no deploy operation.
    """
    _ensure_import_root(root)
    ...
    release_id = os.environ.get("MAGI_RELEASE_ID", "").strip() or root.name
    proposals = ingest_signals(issues, root=root, release_id=release_id, store=store)
    return {
        "ok": True,
        ...
        "auto_deploy": False,
        "next_action": (
            "generate_and_verify_isolated_candidate" if proposals else "no_open_evolution_signal"
        ),
    }
...
def build_report(
    *,
    ...
    if mode in {"repair-safe", "repair-propose"}:
        try:
            evolution = _persist_evolution_proposals(
                root=root, runtime_dir=runtime_dir, issues=issues
            )
            for proposal_id in evolution.get("proposal_ids") or []:
                actions.append(
                    {
                        "id": f"controlled_evolution:{proposal_id}",
                        "kind": "isolated_source_candidate",
                        "status": "planned",
                        "auto_deploy": False,
                    }
                )
        except Exception as exc:
            # Proposal persistence must fail closed without hiding the owning
            # health issue or turning it into a LIVE source mutation.
            evolution = {
                "ok": False,
                "proposal_count": 0,
                "auto_deploy": False,
                "next_action": "inspect_controlled_evolution_store",
    ...
    verifications: list[dict[str, Any]] = []
    if mode in {"repair-safe", "repair-propose"} and not evolution.get("ok"):
        verifications.append(
            {
                "id": "controlled_evolution_proposal_persistence",
                "ok": False,
                "error_type": evolution.get("error_type"),
            }
        )
    ...
    return {
        ...
        "controlled_evolution": evolution,
        "issues": issues,
        "actions": actions,
        ...
        "safety": {
            ...
            "source_evolution_policy": (
                "Open source-quality issues may create de-identified isolated proposals; "
                "candidates can never deploy themselves."
            ),
