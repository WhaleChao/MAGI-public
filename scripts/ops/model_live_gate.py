...
_SCHEDULE_ADAPTER_MODE = "real_entrypoint_fixture_v1"
_SCHEDULE_ENDPOINTS_ENV = "MAGI_MODEL_GATE_SCHEDULE_ENDPOINTS"
_SCHEDULE_PROFILE_PATH_ENV = "MAGI_MODEL_GATE_SCHEDULE_PROFILE_PATH"


def _schedule_adapter_enabled() -> bool:
    return os.environ.get("MAGI_V3_SCHEDULE_ADAPTER", "").strip() == _SCHEDULE_ADAPTER_MODE


def _schedule_endpoint_urls() -> dict[int, str]:
    """Return fixture endpoints only for the reviewed V3 schedule adapter.

    Production topology remains hard-bound to ports 8080--8083.  The adapter
    override is deliberately unavailable outside the Seatbelt certification
    mode and accepts only explicit 127.0.0.1 HTTP URLs.
    """

    raw = os.environ.get(_SCHEDULE_ENDPOINTS_ENV, "").strip()
    if not _schedule_adapter_enabled() or not raw:
        return {}
    try:
        payload = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ValueError("schedule model endpoints must be valid JSON") from exc
    if not isinstance(payload, dict) or set(payload) != {"8080", "8081", "8082", "8083"}:
        raise ValueError("schedule model endpoints must map exactly ports 8080-8083")
    endpoints: dict[int, str] = {}
    for raw_port, raw_url in payload.items():
        ...
        if (
            ...
            raise ValueError("schedule model endpoint must be a plain 127.0.0.1 HTTP URL")
        endpoints[int(raw_port)] = url
    return endpoints
...
def active_profile() -> str:
    profile_path = Path.home() / ".omlx" / "active_profile"
    if _schedule_adapter_enabled():
        raw = os.environ.get(_SCHEDULE_PROFILE_PATH_ENV, "").strip()
        if raw:
...
def probe_port(port: int, timeout: float = 3.0) -> EndpointProbe:
    try:
        url = _schedule_endpoint_urls().get(port) or f"http://127.0.0.1:{port}/v1/models"
...
def build_report(expect: str = "auto", *, require_aux: bool = True) -> ModelGateReport:
    ...
    if expected == "day":
        ...
        if _has_keyword(by_port[8080], NIGHT_MODEL_KEYWORD):
            ...
            active == "night-e4b-degraded"
            and _has_keyword(by_port[8080], DAY_FALLBACK_MODEL_KEYWORD)
        ):
            warnings.append(
                f"8080 is night last-resort {DAY_FALLBACK_MODEL_KEYWORD.upper()}, "
                f"expected {NIGHT_MODEL_KEYWORD.upper()}"
            )
        else:
            failures.append(
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--json-out", default=str(RUNTIME_DIR / "model_live_gate_latest.json"))
