...
DEFAULT_REPORT_PATH = RUNTIME_DIR / "token_health" / "token_health_latest.json"
...
def _discover_google_tokens() -> list[GoogleTokenSpec]:
    ...
    specs.append(google_calendar_token_spec())
...
def google_calendar_token_path() -> Path:
    ...
    if configured_calendar_raw:
        return Path(configured_calendar_raw).expanduser()
    if paperclip_calendar.exists():
        return paperclip_calendar
    return MAGI_ROOT / "json" / "google_calendar_token.json"


def google_calendar_credentials_path() -> Path:
    return Path(
        os.environ.get("MAGI_GOOGLE_CREDENTIALS_PATH", "") or MAGI_ROOT / "json" / "credentials.json"
    ).expanduser()


def google_calendar_token_spec(
    *,
    token_path: Path | None = None,
    credentials_path: Path | None = None,
    required: bool | None = None,
) -> GoogleTokenSpec:
    resolved_token = (token_path or google_calendar_token_path()).expanduser()
    configured_calendar_raw = (os.environ.get("MAGI_GOOGLE_CALENDAR_TOKEN_PATH") or "").strip()
    if required is None:
        required = resolved_token.exists() or bool(configured_calendar_raw)
    return GoogleTokenSpec(
        name="google_calendar",
        token_path=resolved_token,
        credentials_path=(credentials_path or google_calendar_credentials_path()).expanduser(),
...
def google_token_connected(check: dict[str, Any]) -> bool:
    return str(check.get("status") or "") in {"ok", "refreshed", "refreshable_soon"}
...
def _discover_api_keys() -> list[ApiKeySpec]:
    return [
        ApiKeySpec(
            ...
            # LINE is an optional channel. Treat it as required only after
