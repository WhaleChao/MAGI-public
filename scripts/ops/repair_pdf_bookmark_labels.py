...
def _resolve_runtime_dir(magi_root: Path = MAGI_ROOT) -> Path:
    """Resolve mutable state without ever defaulting into a sealed release."""
    explicit = os.environ.get("MAGI_RUNTIME_DIR", "").strip()
    if explicit:
        return Path(explicit).expanduser()
    shared = (
        os.environ.get("MAGI_V3_SHARED_STATE_DIR", "").strip()
        or os.environ.get("MAGI_SHARED_STATE_DIR", "").strip()
    )
    if shared:
        return Path(shared).expanduser() / "runtime"
    root = Path(magi_root).expanduser()
    if root.parent.name == "releases":
        return root.parent.parent / "runtime" / "MAGI_v3" / "shared" / "runtime"
    return root / ".runtime"


RUNTIME_DIR = _resolve_runtime_dir()
DEFAULT_REPORT = RUNTIME_DIR / "pdf_bookmark_label_repair_latest.json"
HISTORY_PATH = RUNTIME_DIR / "pdf_bookmark_label_repair_history.jsonl"
DEFAULT_RETRY_STATE = RUNTIME_DIR / "pdf_bookmark_label_repair_retry_state.json"
DEFAULT_PROMOTED_REPAIR_TIMEOUT_SEC = 900
...
def _retry_identity(path: Path) -> dict[str, int]:
    try:
        meta = path.stat()
    except OSError:
        return {}
    return {"size": int(meta.st_size), "mtime_ns": int(meta.st_mtime_ns)}


def _retry_key(path: Path) -> str:
...
def _save_retry_state(path: Path, state: dict[str, Any]) -> None:
...
def _active_retry_entry(
    state: dict[str, Any], path: Path, *, now: float
) -> dict[str, Any] | None:
    ...
    if not isinstance(entry, dict):
        return None
    identity = _retry_identity(path)
    if not identity or entry.get("identity") != identity:
    ...
    try:
        ...
        next_retry_at = 0.0
    return entry if next_retry_at > now else None


def _is_promoted_repair_retry(entry: dict[str, Any] | None, *, attempts: int) -> bool:
    if not isinstance(entry, dict) or int(entry.get("attempts") or 0) < max(1, attempts):
        return False
    reason = str(entry.get("reason") or "").lower()
    return "repair " in reason and " exceeded " in reason


def _repair_timeout_for(
    state: dict[str, Any],
    path: Path,
    *,
    base_timeout_sec: int,
    promote_after_attempts: int,
    promoted_timeout_sec: int,
) -> int:
    entry = state.setdefault("entries", {}).get(_retry_key(path))
    if _is_promoted_repair_retry(entry, attempts=promote_after_attempts):
        return max(int(base_timeout_sec or 0), int(promoted_timeout_sec or 0))
    return int(base_timeout_sec or 0)


def _record_retry_backoff(
    state: dict[str, Any],
    path: Path,
    *,
    reason: str,
    ...
    delay_hours = min(168.0, max(1.0, float(base_hours)) * (2 ** min(attempts - 1, 4)))
    entry = {
        "path_sha256": key,
        "identity": identity,
        "attempts": attempts,
        "reason": str(reason or "repair_retry")[:500],
        "last_attempt_at": now,
        "next_retry_at": now + delay_hours * 3600.0,
    }
    entries[key] = entry
    return entry


def _clear_retry_backoff(state: dict[str, Any], path: Path) -> None:
    state.setdefault("entries", {}).pop(_retry_key(path), None)
...
def scan_pdf_candidates_in_subprocess(
    roots: list[str],
    *,
    case_number: str = "",
    target_hints: tuple[str, ...] = TARGET_DIR_HINTS,
    ...
    """Isolate SMB traversal so a stuck mount cannot outlive the cron budget."""
    cmd = [
        sys.executable,
    ...
    if case_number:
        cmd.extend(["--case-number", case_number])
    for root in roots:
        cmd.extend(["--root", root])
    for hint in target_hints:
        cmd.extend(["--target-hint", hint])
    try:
        proc = subprocess.run(
            cmd,
            capture_output=True,
            text=True,
            timeout=max(5, int(max_seconds or 600)),
            check=False,
        )
    except subprocess.TimeoutExpired as exc:
        raise PerFileTimeout(f"NAS scan exceeded {max_seconds}s") from exc
    try:
        payload = json.loads(proc.stdout or "{}")
    except json.JSONDecodeError as exc:
        raise RuntimeError((proc.stderr or "candidate scan returned invalid JSON").strip()) from exc
    if proc.returncode == 75 or payload.get("status") == "deferred":
        return [], {
            "visited_dirs": int(payload.get("visited_dirs") or 0),
            "stopped_reason": str(payload.get("reason") or "storage_device_temporarily_unavailable"),
            "elapsed_sec": float(payload.get("elapsed_sec") or 0),
            "deferred": True,
        }
    if proc.returncode != 0:
        raise RuntimeError((proc.stderr or payload.get("error") or "candidate scan failed").strip())
    return [Path(value) for value in payload.get("pdfs") or []], dict(payload.get("scan") or {})
...
def audit_pdf(pdf_path: Path, bookmarker, *, verify_rebuild: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        for page in pages:
            ...
            # Existing bookmarks in court bundles frequently point to
            # image-only pages.  Post-write verification must inspect the same
            # OCR evidence as the generator; embedded-text-only verification
            # otherwise reports a false mismatch after a correct rebuild.
            if len(_compact(text)) < 30:
                try:
                    text = getattr(bookmarker, "_ocr_page")(doc[page - 1]) or text
                except Exception:
                    pass
...
def repair_pdf(pdf_path: Path, bookmarker) -> dict[str, Any]:
    ...
    if success:
        verification = audit_pdf(pdf_path, bookmarker, verify_rebuild=False)
        success = not bool(verification.get("needs_repair"))
    message = str(result.get("message", ""))
    if verification and verification.get("needs_repair"):
        message = (
            f"{message}；重建後仍有 "
            f"{len(verification.get('issues') or [])} 個不一致，未列為成功"
        )
    return {
        "pdf": str(pdf_path),
        "success": success,
        ...
        "message": message,
        "verified_clean": success if verification is not None else False,
        "remaining_issue_count": (
            len(verification.get("issues") or [])
...
def _last_json_object(text: str) -> dict[str, Any] | None:
    """Return the final complete JSON object from mixed child stdout.

    The bookmark engine may emit informational log lines before its
    machine-readable result.  Treating the complete stdout stream as one JSON
    document turns a successful repair into ``JSONDecodeError: Extra data``.
    """
    if not text:
        return None
    stripped = text.strip()
    ...
    for start in reversed([idx for idx, char in enumerate(stripped) if char == "{"]):
        ...
        if isinstance(payload, dict) and trailing in {"", "```"}:
            return payload
    return None


def _parse_child_result(proc: subprocess.CompletedProcess[str], *, operation: str) -> dict[str, Any]:
    payload = _last_json_object(proc.stdout or "")
    if payload is not None:
        return payload
    stderr_tail = (proc.stderr or "").strip()[-300:]
    detail = f": {stderr_tail}" if stderr_tail else ""
    raise RuntimeError(f"{operation} child returned no machine-readable JSON{detail}")


def _child_environment() -> dict[str, str]:
    """Bind every isolated child to the parent's writable shared runtime."""

    environment = os.environ.copy()
    environment["MAGI_RUNTIME_DIR"] = str(RUNTIME_DIR)
    environment.setdefault("MAGI_BACKGROUND_LOCK_DIR", str(RUNTIME_DIR / "locks"))
    return environment
...
def audit_pdf_in_subprocess(pdf_path: Path, *, timeout_sec: int, verify_rebuild: bool = False) -> dict[str, Any]:
    ...
    if proc.returncode == 75:
        return _parse_child_result(proc, operation=f"audit {pdf_path.name}")
    if proc.returncode != 0:
...
def repair_pdf_in_subprocess(pdf_path: Path, *, timeout_sec: int) -> dict[str, Any]:
    ...
    if proc.returncode == 75:
        return _parse_child_result(proc, operation=f"repair {pdf_path.name}")
    if proc.returncode != 0:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--promote-retry-attempts",
        type=int,
        default=2,
        help="Promote repeated repair timeouts to the long repair lane after this many attempts.",
    )
    parser.add_argument(
        "--promoted-repair-timeout",
        type=int,
        default=DEFAULT_PROMOTED_REPAIR_TIMEOUT_SEC,
        help="Per-file timeout used by the long repair lane.",
    )
    parser.add_argument(
        "--repair-reserve-seconds",
        type=int,
        default=300,
        help="When --apply is active, preserve this part of the overall budget for actual repairs.",
    )
    ...
    parser.add_argument("--retry-state", default=str(DEFAULT_RETRY_STATE))
    parser.add_argument(
        "--retry-backoff-hours",
        type=float,
        default=24.0,
        help="Back off repeatedly timing-out/unrepairable PDFs so later files can progress.",
    )
    ...
    parser.add_argument("--scan-one-json", action="store_true", help=argparse.SUPPRESS)
                ...
                json.dumps(
                    {
                        ...
                        "elapsed_sec": round(time.time() - scan_started, 3),
                    },
                    ensure_ascii=False,
                )
            )
            return 75
        print(json.dumps({"pdfs": [str(path) for path in pdfs], "scan": scan_meta}, ensure_ascii=False))
        return 0
    ...
    started = time.time()
    retry_state_path = Path(args.retry_state).expanduser()
    retry_state = _load_retry_state(retry_state_path)

    try:
        pdfs, scan_meta = scan_pdf_candidates_in_subprocess(
            roots,
            case_number=args.case_number,
            target_hints=target_hints,
            max_files=args.max_files,
        ...
        scan_meta = {
            "visited_dirs": 0,
            "stopped_reason": "storage_scan_time_budget_exhausted",
            "elapsed_sec": round(time.time() - started, 3),
            "deferred": True,
        }

    scanned_pdf_count = len(pdfs)
    retry_backoff_items: list[dict[str, Any]] = []
    eligible_pdfs: list[Path] = []
    for pdf in pdfs:
        entry = _active_retry_entry(retry_state, pdf, now=started)
        if args.apply and _is_promoted_repair_retry(
            entry, attempts=args.promote_retry_attempts
        ):
            eligible_pdfs.append(pdf)
            continue
        if entry is None:
            eligible_pdfs.append(pdf)
            continue
        retry_backoff_items.append(
            {
                "pdf_sha256": _retry_key(pdf),
                "attempts": int(entry.get("attempts") or 0),
                "reason": str(entry.get("reason") or "repair_retry")[:300],
                "next_retry_at": float(entry.get("next_retry_at") or 0.0),
            }
        )
    pdfs = eligible_pdfs
    ...
    for pdf in pdfs:
        remaining_seconds = (
            max(0, int(args.max_seconds - (time.time() - started)))
            if args.max_seconds
            else args.per_file_timeout
        )
        if args.max_seconds and remaining_seconds <= 0:
            audit_stopped_reason = "max_seconds"
            break
        repair_reserve_seconds = (
            # A configured reserve must never consume the whole run.  Short
            # manual/LIVE runs still need enough time to discover at least
            # one candidate before the repair lane can do useful work.
            min(
                max(0, int(args.repair_reserve_seconds)),
                max(
                    0,
                    int(args.max_seconds)
                    if args.max_seconds <= 30
                    else int(args.max_seconds * 0.7),
                ),
            )
            if args.apply and args.max_seconds
            else 0
        )
        if repair_reserve_seconds and remaining_seconds <= repair_reserve_seconds:
            audit_stopped_reason = "repair_budget_reserved"
            break
        try:
            if args.per_file_timeout > 0:
                item = audit_pdf_in_subprocess(
                    pdf,
                    timeout_sec=min(args.per_file_timeout, max(1, remaining_seconds)),
            ...
            if item.get("deferred"):
                ...
                continue
            audited.append(item)
            if item.get("needs_repair") and item.get("repairable"):
                repair_candidate_count += 1
                # Applying repairs is the purpose of this run. Once the
                # bounded repair batch is full, stop re-auditing unrelated
                # PDFs and spend the remaining budget on rebuilding and
                # verifying those candidates.
                if args.apply and args.limit > 0 and repair_candidate_count >= args.limit:
                    audit_stopped_reason = "repair_limit_reached"
                    break
            if not item.get("needs_repair"):
                _clear_retry_backoff(retry_state, pdf)
        except PerFileTimeout as exc:
            entry = _record_retry_backoff(
                retry_state,
                pdf,
                reason=str(exc),
                base_hours=args.retry_backoff_hours,
                now=time.time(),
            )
            retry_backoff_items.append(
                {
    ...
    if args.apply:
        for item in repairable[: max(0, args.limit)]:
            ...
            try:
                if args.per_file_timeout > 0:
                    repair_timeout = _repair_timeout_for(
                        retry_state,
                        Path(item["pdf"]),
                        base_timeout_sec=args.per_file_timeout,
                        promote_after_attempts=args.promote_retry_attempts,
                        promoted_timeout_sec=args.promoted_repair_timeout,
                    )
                    repaired_item = repair_pdf_in_subprocess(
                        Path(item["pdf"]),
                        timeout_sec=min(repair_timeout, max(1, remaining_seconds)),
                    )
                else:
                    repaired_item = repair_pdf(Path(item["pdf"]), bookmarker)
                if repaired_item.get("deferred"):
                    deferred_items.append(repaired_item)
                else:
                    repaired.append(repaired_item)
                    if repaired_item.get("success"):
                        _clear_retry_backoff(retry_state, Path(item["pdf"]))
                    else:
                        entry = _record_retry_backoff(
                            retry_state,
                            Path(item["pdf"]),
                            reason=str(repaired_item.get("message") or "repair_not_verified"),
                            base_hours=args.retry_backoff_hours,
                            now=time.time(),
                        )
                        retry_backoff_items.append(
                            {
                                "pdf_sha256": _retry_key(Path(item["pdf"])),
                                "reason": entry["reason"],
                                "kind": "repair_not_verified",
                                "attempts": entry["attempts"],
                                "next_retry_at": entry["next_retry_at"],
                            }
                        )
    ...
    repair_failure_count = sum(
        ...
        if not item.get("success") and not item.get("deferred")
    )
    hard_failure_count = len(errors) + repair_failure_count
    time_budget_deferred = (
        bool(scan_meta.get("deferred"))
        or scan_meta.get("stopped_reason") == "max_seconds"
        or audit_stopped_reason in {"max_seconds", "repair_budget_reserved"}
    )
    is_deferred = not hard_failure_count and (time_budget_deferred or bool(deferred_items))
    if bool(scan_meta.get("deferred")) or scan_meta.get("stopped_reason") == "max_seconds":
        deferred_reason = str(scan_meta.get("stopped_reason") or "storage_scan_time_budget_exhausted")
    elif audit_stopped_reason in {"max_seconds", "repair_budget_reserved"}:
        deferred_reason = (
            "repair_budget_reserved"
            if audit_stopped_reason == "repair_budget_reserved"
            else "overall_time_budget_exhausted"
        )
    elif deferred_items:
        deferred_reason = "per_file_time_budget_exhausted"
    else:
        deferred_reason = ""
    report = {
        "ok": not hard_failure_count and not is_deferred,
        "status": "deferred" if is_deferred else ("completed" if not hard_failure_count else "failed"),
        ...
        "errors_count": hard_failure_count,
        "deferred_count": len(deferred_items),
        "retry_backoff_count": len(retry_backoff_items),
        "retry_state_file": str(retry_state_path),
        "elapsed_sec": round(time.time() - started, 3),
        "scan": scan_meta,
        ...
        "retry_backoff_items": retry_backoff_items[:20],
        "errors": errors[:20],
    }
    _save_retry_state(retry_state_path, retry_state)
                ...
                json.dumps(
                    {
                        ...
                        "audited_pdf_count": report["audited_pdf_count"],
