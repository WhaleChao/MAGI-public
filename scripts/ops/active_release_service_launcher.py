...
The launcher itself is installed in MAGI's stable host-owned ``bin`` directory.
It never embeds an immutable release path.  Before starting a child it verifies
the active marker, release manifest, and selected script hash.  While the child
is running it watches the active marker and performs a bounded restart when a
new valid release becomes active, allowing old releases to be archived safely.
"""
