...
The command is deliberately non-mutating: it reads existing plists and writes
reviewed replacements to a caller-owned empty directory. Installation remains
a separate, rollback-capable maintenance transaction.
"""
...
def render_migrated_plist(
    ...
    *,
    ...
    if not release.name.startswith("v3-"):
    ...
    if _contains_versioned_release(rendered):
        raise HostSingletonMigrationError(
            f"{label} still contains an immutable release reference"
        )
    return rendered
