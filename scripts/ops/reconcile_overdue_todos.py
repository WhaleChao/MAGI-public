...
_ACTIONABLE_OBLIGATION_MARKERS = (
    "補正",
    "提出",
    "繳費",
    "繳納",
...
_ORIGINAL_TYPE_RE = re.compile(r"原類型[：:]\s*([^\s／/｜|\n]+)")
_PURE_OCCURRENCE_REASONS = {
    "past_calendar_occurrence",
    "past_occurrence",
    "overdue_confirmation_for_past_calendar_occurrence",
    "overdue_confirmation_for_past_occurrence",
}
...
def _original_todo_type(row: dict) -> str:
    match = _ORIGINAL_TYPE_RE.search(str(row.get("description") or ""))
    return match.group(1).strip() if match else ""


def _is_overdue_escalation(row: dict) -> bool:
    return "【MAGI逾期治理：原待辦#" in str(row.get("description") or "")


def _is_past_pure_occurrence(row: dict) -> bool:
    """Whether a past row records an event, rather than an unfinished duty.

    Calendar imports do not always preserve a specific ``todo_type``; some
    hearings arrive as a generic confirmation.  An event marker alone is not
    enough when the same row also requires a filing or payment, so explicit
    actionable language always wins and remains subject to overdue review.
    """

    todo_type = str(row.get("todo_type") or "").strip()
    if todo_type == "逾期確認":
        todo_type = _original_todo_type(row) or todo_type
    text = f"{todo_type} {row.get('description') or ''}"
    if any(marker in text for marker in _ACTIONABLE_OBLIGATION_MARKERS):
        return False
    if todo_type in OCCURRENCE_TYPES:
        return True
    return _source_kind(row.get("source_file")) == "calendar" and any(
        marker in text for marker in _OCCURRENCE_MARKERS
    )


def _terminal_status(row: dict, *, action: str, reason: str) -> str:
    """Close an internal todo without erasing a real calendar occurrence.

    Only the synthetic overdue-confirmation copy is handed to the normal
    calendar refresh as ``deleted``.  The original hearing/mediation/meeting
    remains visible as calendar history after its date has passed.
    """

    is_synthetic_overdue = (
        _is_overdue_escalation(row)
        or str(row.get("todo_type") or "").strip() == "逾期確認"
    )
    if (
        is_synthetic_overdue
        and reason in _PURE_OCCURRENCE_REASONS
        and str(row.get("google_calendar_id") or "").strip()
    ):
        return "deleted"
    if action == "archive":
        return "calendar_deduped"
    return "completed"


def _overdue_action_label(row: dict) -> str:
    """Return the real work label; never expose the vague legacy label."""

    original = _original_todo_type(row)
    current = str(row.get("todo_type") or "").strip()
    label = original or current
    if label and label != "逾期確認":
        return label
    text = str(row.get("description") or "")
    for marker in _ACTIONABLE_OBLIGATION_MARKERS:
        if marker in text:
            return marker
    return "待辦"


def _actionable_overdue_description(row: dict) -> str:
    text = str(row.get("description") or "").strip()
    legacy = "尚無可驗證的完成證據，請確認是否已辦理；確認後即可結束本待辦。"
    replacement = "已超過原期限，尚無可驗證的完成證據；請立即處理，完成後將本待辦標示為完成。"
    if legacy in text:
...
def classify_todo(row: dict, *, verified_ids: set[int] | None = None) -> tuple[str, str]:
    ...
    todo_type = str(row.get("todo_type") or "").strip()
    if todo_type == "逾期確認" or _is_overdue_escalation(row):
        # Earlier versions could escalate a calendar hearing before its
        # original type was recognized.  The escalation description retains
        # that original context, so reconcile it now only when it is still a
        # pure occurrence.  Do not auto-close an escalation mentioning an
        # actionable duty such as filing or payment.
        if _is_past_pure_occurrence(row):
            return (
                ("archive", "overdue_confirmation_for_past_calendar_occurrence")
                if _source_kind(row.get("source_file")) == "calendar"
                else ("complete", "overdue_confirmation_for_past_occurrence")
            )
        if todo_type == "逾期確認":
            return "normalize", "legacy_overdue_confirmation_label"
        return "skip", "already_escalated_actionable"
...
def reconcile(*, apply: bool, cutoff: date, verified_ids: set[int]) -> dict:
    ...
    try:
        ...
        backup_path = RUNTIME_DIR / f"overdue_todo_reconcile_backup_{stamp}.json"
        if apply:
                ...
                for item in decisions:
                    ...
                    if action == "normalize":
                        cur.execute(
                            """
                            UPDATE case_todos
                            SET todo_type=%s, todo_date=%s, description=%s
                            WHERE id=%s AND status='pending'
                            """,
                            (
                                _overdue_action_label(row),
                                review_date,
                                _actionable_overdue_description(row),
                                todo_id,
                            ),
                        )
                        continue
                    if action in {"complete", "archive"}:
                        status = _terminal_status(row, action=action, reason=reason)
                        cur.execute(
                    ...
                    cur.execute(
                        ...
                        WHERE status='pending' AND description LIKE %s
                        LIMIT 1
                        """,
                        (f"%{marker}%",),
                    )
                    ...
                    if not existing:
                        ...
                        cur.execute(
                            ...
                            VALUES (%s,%s,%s,%s,NULL,%s,'pending',NULL,%s,NULL,%s)
                            """,
                            (
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--json-out", default=str(RUNTIME_DIR / "overdue_todo_reconcile_latest.json"))
