...
FILE_REVIEW_PORTAL_LOCK_PATH_ENV = "MAGI_FILE_REVIEW_PORTAL_LOCK_PATH"
...
FILE_REVIEW_PORTAL_LOCK_NAME = "file_review_portal"
...
def lock_dir() -> Path:
    ...
    if override:
        ...
        base = Path(runtime_override).expanduser() / "locks"
    else:
        # Preserve the V2 layout when no runtime environment is configured.
        base = ROOT / ".runtime" / "locks"
...
def file_review_portal_lock_path() -> Path:
    """Return one user-level path shared by source and deployed runtimes."""
    override = os.environ.get(FILE_REVIEW_PORTAL_LOCK_PATH_ENV, "").strip()
    if override:
        return Path(override).expanduser()
    if os.environ.get(LOCK_DIR_ENV, "").strip():
        return lock_path(FILE_REVIEW_PORTAL_LOCK_NAME)
    if sys.platform == "darwin":
        base = Path.home() / "Library" / "Application Support" / "MAGI" / "runtime" / "locks"
    else:
        state_home = os.environ.get("XDG_STATE_HOME", "").strip()
        base = Path(state_home).expanduser() / "MAGI" / "locks" if state_home else Path.home() / ".local" / "state" / "MAGI" / "locks"
    return base / f"{FILE_REVIEW_PORTAL_LOCK_NAME}.lock"
