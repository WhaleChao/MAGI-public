...
def _verify(
    ...
    *,
    ...
    metadata_checks = {
        "python_executable": python_bin.is_file() and os.access(python_bin, os.X_OK),
        "omlx_source": (src / "omlx" / "omlx" / "model_discovery.py").is_file(),
        "mlx_lm_gemma4": (src / "mlx-lm" / "mlx_lm" / "models" / "gemma4.py").is_file(),
        "mlx_vlm_gemma4_unified": (
            src / "mlx-vlm" / "mlx_vlm" / "models" / "gemma4_unified.py"
        ).is_file(),
        "model_path_exists": bool(model_dir and model_dir.is_dir()),
        "metal_probe": "not_run_requires_explicit_live_context",
    }
    required = (
        metadata_checks["python_executable"]
        and metadata_checks["omlx_source"]
        and metadata_checks["mlx_lm_gemma4"]
        and metadata_checks["mlx_vlm_gemma4_unified"]
        and (model_dir is None or metadata_checks["model_path_exists"])
    )
    if not required:
        raise RuntimeError(json.dumps(metadata_checks, ensure_ascii=False))
    if not live_metal_probe:
        return json.dumps(metadata_checks, ensure_ascii=False, indent=2)

    # Importing mlx.core initializes Metal and can abort the interpreter when
    # invoked from a headless/sandboxed diagnostic process.  It is therefore
    # opt-in and must only run in the same unlocked launchd context as oMLX.
    code = r"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--live-metal-probe",
        action="store_true",
        help="initialize MLX/Metal; only use from an unlocked launchd service context",
    )
    ...
    print(
        _verify(
            ...
            python_bin,
            model_dir,
            live_metal_probe=args.live_metal_probe,
        )
    )
    return 0
