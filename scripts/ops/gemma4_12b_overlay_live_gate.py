...
def run_gate(server_url: str, model: str, *, stress_count: int = 6, verify_overlay: bool = True) -> GateReport:
    ...
    def deterministic_transcript_extractor() -> tuple[bool, str, str, float]:
        ...
        items = mod.extract_candidates_from_pages(
            [(1, "法官諭知：本案候核辦，請律師確認是否補提資料。")],
            pdf_path=Path("/tmp/20260601 訊問筆錄.pdf"),
            transcript_date="2026-06-01",
        ...
        ok = (
            len(items) == 1
            and items[0].type == "追蹤"
            and items[0].date == "2026-06-08"
            and items[0].rule == "candidate_review_7_days"
            and "無下次庭期" in items[0].description
        )
        return ok, actual, actual, time.time() - started

    _run_case(report, "transcript_extractor_candidate_review", deterministic_transcript_extractor, "正式抽取器：候核辦 = 無下次庭期，7日後追蹤")

    def transcript_todo_reasoning() -> tuple[bool, str, str, float]:
        prompt = (
            "你是 OSC 筆錄待辦建立器。請只輸出 JSON。規則：『候核辦』代表沒有下次庭期，應在筆錄日7日後建立追蹤提醒；"
            "若是候核辦，嚴禁寫成『庭期前』或『下次庭期前』。"
            "開庭時有待辦但沒指定時間，應在下次庭前7個工作日設提醒。"
            "筆錄日期：2026-06-01。筆錄：法官諭知本案候核辦，請律師確認是否補提資料。請判斷待辦。"
        )
        obj, elapsed = _chat(server_url, model, [{"role": "user", "content": prompt}], max_tokens=220)
        text = _content(obj)
        ok = "候核辦" in text and "2026-06-08" in text and "確認是否補提資料" in text and "庭期前" not in text
        return ok, text, text, elapsed

    _run_case(report, "transcript_todo_structured_rule", transcript_todo_reasoning, "候核辦 structured rule understood")
