...
def main() -> int:
    ...
    query = os.environ.get("MAGI_TWLEGALRAG_LIVE_QUERY", "通譯 最高法院")

    health = tlr_health()
    search = search_practical_judgments_via_tlr(query, limit=2, fulltext_limit=1)
    ...
    report = {
        "ok": bool(health.get("ok") and search.get("success") and augmented.get("success") and raziel_preview.get("ok")),
