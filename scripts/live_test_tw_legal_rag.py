...
from api.legal_research_quality import (  # noqa: E402
    EXTERNAL_CANDIDATE,
...
def main() -> int:
    ...
    search_items = search.get("items") if isinstance(search.get("items"), list) else []
    ranked = enrich_and_rank_items(query, search_items)
    external_not_draft_eligible = bool(ranked) and all(
        item.get("verification_state") == EXTERNAL_CANDIDATE
        and item.get("draft_eligible") is False
        for item in ranked
    )
    citation_lock = citation_lock_for_items(ranked)
    synthetic_mobile = "09" + "12345678"
    privacy_probe = prepare_external_legal_query(
        f"實務見解 當事人王小明 A123456789 {synthetic_mobile} 2026-0008 民法第184條因果關係"
    )
    privacy_ok = bool(
        privacy_probe.external_allowed
        and "王小明" not in privacy_probe.safe_query
        and "A123456789" not in privacy_probe.safe_query
        and synthetic_mobile not in privacy_probe.safe_query
        and "2026-0008" not in privacy_probe.safe_query
        and "民法第184條" in privacy_probe.safe_query
    )
    ...
    report = {
        "ok": bool(
            ...
            and external_not_draft_eligible
            and not citation_lock.get("allowed")
            and privacy_ok
        ),
        ...
        "privacy_gate": {
            "ok": privacy_ok,
            "external_allowed": privacy_probe.external_allowed,
            "redaction_labels": list(privacy_probe.redactions),
        },
        "ranked_count": len(ranked),
        "rank_axes_present": all(
            "authority_score" in item and "similarity_score" in item
            for item in ranked
        ),
        "external_candidates_not_draft_eligible": external_not_draft_eligible,
        "citation_lock_allowed_count": len(citation_lock.get("allowed") or []),
        "human_approval_required": bool(citation_lock.get("human_approval_required")),
    }
    out_dir = RUNTIME_DIR / "live_checks"
