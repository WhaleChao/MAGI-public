...
_magi_root() {
    ...
    echo "${MAGI_ROOT_DIR:-$HOME/Desktop/MAGI_v3}"
}
...
cmd_status() {
    ...
    echo "NAS Mounts:"
    # 優先讀 env；沒有再從 .env 撈
    local magi_root_guess="${MAGI_ROOT:-${MAGI_ROOT_DIR:-$HOME/Desktop/MAGI_v3}}"
    ...
    echo ""
    echo "Vector DB:"
    local magi_root faiss_dir faiss_meta faiss_vectors
    magi_root="${MAGI_ROOT_DIR:-$HOME/Desktop/MAGI_v3}"
    if [ -n "${FAISS_INDEX_DIR:-}" ]; then
        faiss_dir="$FAISS_INDEX_DIR"
    elif [ -n "${MAGI_SHARED_STATE_DIR:-}" ]; then
        faiss_dir="$MAGI_SHARED_STATE_DIR/memory/index_cache"
    else
        faiss_dir="$magi_root/skills/memory/index_cache"
    fi
