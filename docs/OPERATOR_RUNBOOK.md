...
版本：2026-07-02 operator edition
適用對象：MAGI 發布、部署、維運、交接人員
canonical CLI：`magi`

本 runbook 是 2026-07-02 後的操作員發布手冊。它區分三種環境：public install、private production、developer checkout。正式操作以 `magi` 為 canonical CLI；腳本路徑只在安裝、測試、發版與故障排除時使用。
...
- 本機全量 pytest 與 production handoff 不接受 residual skipped 或 warnings；若外部環境不足，應補 deterministic fallback、fixture 或獨立 live 記錄，不用 skip 取代驗證。
...
- pytest summary 應為 0 skipped / 0 warnings；任何外部 live 限制需另列為風險或暫緩範圍。
