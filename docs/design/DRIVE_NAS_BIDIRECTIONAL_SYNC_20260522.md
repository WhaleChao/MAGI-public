...
- `2025-0034 中央選舉委員會／海上賽鴿案` 已依使用者確認改回進行中，DB 路徑為 `Z:\lumi63181107\01_案件\一般案件\行政\2025-0034-中央選舉委員會-一審-公民投票法`。
...
1. **OSC DB 優先**
   ...
   - DB 內保存 Windows 路徑：進行中用 `Z:\lumi63181107\...`，結案用 `Y:\lumi\...`；Mac 本機路徑只在讀寫時轉換。
...
3. **保留兩邊原本資料夾邏輯**
   - NAS/OSC 仍使用正式案件資料夾規則，例如 `10_判決書`、`09_法院通知或程序裁定`、`08_筆錄`。
...
- 一般檔案直接下載。
- 下載前先把 Drive 相對路徑轉成 NAS/OSC 相對路徑，例如 `法院判決/a.pdf` 會落在 `10_判決書/a.pdf`。
- 下載到暫存檔，完成後再改名，避免半檔。
- 不覆蓋既有檔案。
...
- 上傳前先把 NAS/OSC 相對路徑轉成 Drive 相對路徑，例如 `10_判決書/a.pdf` 會上傳到 `法院判決/a.pdf`。
- 不刪除 Drive 既有檔案。
...
- 法扶消債案件因已位於 `01.消債` bucket，Drive 顯示名稱只保留 `當事人-法扶案號`，例如 `金李連芯-1150519-E-014`，不再附加「更生」「清算」等程序詞。
