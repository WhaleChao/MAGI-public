...
def test_cleanup_stale_lock_metadata_requires_free_flock(tmp_path, monkeypatch):
    monkeypatch.setenv("MAGI_BACKGROUND_LOCK_DIR", str(tmp_path))
    from scripts.ops import background_task_locks as locks

    lock_path = tmp_path / "unit-stale.lock"
    meta_path = tmp_path / "unit-stale.lock.json"
    payload = {"domain": "unit-stale", "owner": "dead", "pid": 99999999}
    lock_path.write_text(json.dumps(payload), encoding="utf-8")
    meta_path.write_text(json.dumps(payload), encoding="utf-8")

    dry = locks.cleanup_stale_lock_metadata([tmp_path], apply=False)

    assert dry["stale_count"] == 1
    assert dry["cleaned_count"] == 0
    assert meta_path.exists()

    applied = locks.cleanup_stale_lock_metadata([tmp_path], apply=True)

    assert applied["stale_count"] == 1
    assert applied["cleaned_count"] == 1
    assert lock_path.read_text(encoding="utf-8") == ""
    assert not meta_path.exists()
