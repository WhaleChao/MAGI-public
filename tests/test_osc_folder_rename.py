...
def test_case_root_folder_rename_updates_case_path_and_references(tmp_path, monkeypatch):
    ...
    expected_db_path = osc_utils._osc_norm_path(str(new_folder))
...
def test_case_folder_reconcile_atomically_renames_existing_local_folder(tmp_path, monkeypatch):
    from api.blueprints import osc_cases as mod
    ...
    monkeypatch.setattr(mod, "_get_translate_local_path_to_canonical", lambda: (lambda p: str(p)))
    monkeypatch.setattr(mod, "_osc_find_sibling_case_folder", lambda *_args, **_kwargs: "")
    monkeypatch.setattr(mod, "acquire_case_file_operation_lock", lambda **_kwargs: {"acquired": True})
    monkeypatch.setattr(mod, "release_case_file_operation_lock", lambda: None)
    ...
    assert resolved["source"] == "filesystem_atomic_rename"
    assert resolved["pending_repair"] is False
    assert resolved["suggested_local_folder"] == str(new_folder)
    assert not old_folder.exists()
    assert (new_folder / "note.txt").read_text(encoding="utf-8") == "keep"
    assert any("UPDATE cases SET folder_path=%s" in sql for sql, _params, _fetch in calls)


def test_case_folder_reconcile_rolls_back_rename_when_case_row_update_fails(tmp_path, monkeypatch):
    from api.blueprints import osc_cases as mod

    old_folder = tmp_path / "2026-0080-權利回復基金會-一審"
    old_folder.mkdir()
    (old_folder / "keep.txt").write_text("keep", encoding="utf-8")

    def fake_exec(sql, params=(), fetch="none"):
        if "UPDATE cases SET folder_path=%s" in sql:
            raise RuntimeError("db unavailable")
        return {"rowcount": 0}, None

    row = {
        "id": "case-80",
        "case_number": "2026-0080",
        "client_name": "權利回復基金會",
        "case_category": "一般案件",
        "case_type": "民事",
        "case_stage": "一審",
    ...
    expected = tmp_path / "2026-0080-權利回復基金會-一審-權利回復條例"
    assert resolved["updated"] is False
    assert resolved["source"] == "rename_rolled_back"
    assert resolved["pending_repair"] is True
    assert resolved["folder_path"] == mod._osc_norm_path(str(old_folder))
    assert "db_update_failed" in resolved["rename_error"]
    assert old_folder.is_dir()
    assert not expected.exists()
    assert (old_folder / "keep.txt").read_text(encoding="utf-8") == "keep"
