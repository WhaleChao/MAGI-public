...
def test_drive_imported_alias_detection_and_mapping():
    ...
    assert is_noncanonical_drive_folder("和解筆錄") is True
    assert is_noncanonical_drive_folder("準備程序筆錄") is True
    ...
    assert mapped_file_relative_path("和解筆錄", "b.pdf") == "08_筆錄/b.pdf"
    assert mapped_file_relative_path("準備程序筆錄", "b.pdf") == "08_筆錄/b.pdf"
...
def test_repair_case_folder_moves_transcript_alias_folder(tmp_path: Path):
    case = tmp_path / "2025-0008-林黃阿姐-一審-損害賠償"
    (case / "08_筆錄").mkdir(parents=True)
    src = case / "和解筆錄" / "和解筆錄.pdf"
    src.parent.mkdir(parents=True)
    src.write_bytes(b"settlement transcript")

    report = repair_case_folder(case, apply=True)

    dst = case / "08_筆錄" / "和解筆錄.pdf"
    assert report["errors"] == []
    assert dst.read_bytes() == b"settlement transcript"
    assert not src.exists()
    assert "和解筆錄" in report["alias_folders"]


def test_repair_case_folder_ignores_gitkeep_placeholders(tmp_path: Path):
    case = tmp_path / "2025-0009-測試-一審-損害賠償"
    src = case / "10_判決書或終局裁定及處分" / ".gitkeep"
    dst = case / "09_法院通知或程序裁定" / ".gitkeep"
    src.parent.mkdir(parents=True)
    dst.parent.mkdir(parents=True)
...
def test_repair_case_folder_renumbers_category_specific_folders(tmp_path: Path):
    case = tmp_path / "無償案件" / "刑事" / "2025-0088-余秋菊-二審-毒品危害防制條例"
    wrong_opening = case / "02_開辦資料" / "委任狀.pdf"
    wrong_pleading = case / "04_我方歷次書狀" / "聲請狀.pdf"
    duplicate_src = case / "12_信件往返" / "mail.pdf"
    duplicate_dst = case / "11_信件往返" / "mail.pdf"
    wrong_opening.parent.mkdir(parents=True)
    wrong_pleading.parent.mkdir(parents=True)
    duplicate_src.parent.mkdir(parents=True)
    duplicate_dst.parent.mkdir(parents=True)
    wrong_opening.write_bytes(b"opening")
    wrong_pleading.write_bytes(b"pleading")
    duplicate_src.write_bytes(b"same-mail")
    duplicate_dst.write_bytes(b"same-mail")

    report = repair_case_folder(case, apply=True, delete_duplicate=True)

    assert report["errors"] == []
    assert (case / "01_無償委任資料" / "委任狀.pdf").read_bytes() == b"opening"
    assert (case / "02_我方歷次書狀" / "聲請狀.pdf").read_bytes() == b"pleading"
    assert duplicate_dst.read_bytes() == b"same-mail"
    assert not duplicate_src.exists()
    assert not wrong_opening.exists()
    assert not wrong_pleading.exists()
    assert len(report["canonical_folder_moves"]) == 2
    assert len(report["duplicates"]) == 1


def test_repair_case_folder_can_skip_category_specific_renumbering(tmp_path: Path):
    case = tmp_path / "無償案件" / "刑事" / "2025-0088-余秋菊-二審-毒品危害防制條例"
    wrong_pleading = case / "04_我方歷次書狀" / "聲請狀.pdf"
    wrong_pleading.parent.mkdir(parents=True)
    wrong_pleading.write_bytes(b"pleading")

    report = repair_case_folder(case, apply=True, renumber_folders=False)

    assert report["errors"] == []
    assert report["canonical_folder_moves"] == []
    assert wrong_pleading.read_bytes() == b"pleading"
    assert not (case / "02_我方歷次書狀" / "聲請狀.pdf").exists()


def test_repair_case_folder_can_skip_hashing_existing_targets(tmp_path: Path):
    case = tmp_path / "2025-0002-游秀鈴-一審-傷害致死"
    src = case / "訊問筆錄" / "t.pdf"
    dst = case / "08_筆錄" / "t.pdf"
    src.parent.mkdir(parents=True)
    ...
    report = repair_case_folder(case, apply=False, hash_existing=False)
...
def test_repair_general_case_shell_keeps_general_pleading_number(tmp_path: Path):
    case = tmp_path / "一般案件" / "民事" / "2025-0028-鑫源企業社-一審-給付工程款"
    (case / "02_我方歷次書狀").mkdir(parents=True)
    src = case / "鑫源企業社-給付工程款" / "準備一狀.pdf"
    src.parent.mkdir(parents=True)
    src.write_bytes(b"pleading")

    report = repair_case_folder(case, apply=True)

    assert report["errors"] == []
    assert (case / "02_我方歷次書狀" / "準備一狀.pdf").read_bytes() == b"pleading"
    assert not (case / "04_我方歷次書狀").exists()
    assert not src.exists()
