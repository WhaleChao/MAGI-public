...
def test_worker_clears_only_its_own_launcher_pid(tmp_path):
    from scripts.ops import slow_archive_closed_cases as mod

    pid_path = tmp_path / "worker.pid"
    pid_path.write_text(str(mod.os.getpid()), encoding="utf-8")
    assert mod._clear_launcher_pid(pid_path) is True
    assert not pid_path.exists()

    pid_path.write_text(str(mod.os.getpid() + 1000), encoding="utf-8")
    assert mod._clear_launcher_pid(pid_path) is False
