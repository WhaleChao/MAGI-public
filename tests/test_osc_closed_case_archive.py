...
def test_closed_case_prefers_complete_categorized_archive_over_flat_user_mount_ghost(tmp_path, monkeypatch):
    from api.blueprints import osc_cases as mod

    active_root = tmp_path / "01_案件"
    user_archive = tmp_path / ".magi_mounts" / "10_結案"
    real_archive = tmp_path / "Volumes" / "10_結案"
    ghost = user_archive / "2026-0034-陳文明-一審-洗錢防制法"
    complete = real_archive / "法扶案件" / "刑事" / ghost.name
    ghost.mkdir(parents=True)
    (ghost / "01_法扶資料").mkdir()
    complete.mkdir(parents=True)
    (complete / "10_判決書或終局裁定及處分").mkdir()
    updates = []
    ...
    monkeypatch.setattr(
        ...
        lambda include_closed=False: (
            [str(active_root), str(real_archive), str(user_archive)]
            if include_closed
            else [str(active_root)]
        ),
    )
    monkeypatch.setattr(mod, "_osc_archive_local_base", lambda: (str(real_archive), str(real_archive)))
    monkeypatch.setattr(mod, "_get_translate_local_path_to_canonical", lambda: (lambda p: str(p)))
    monkeypatch.setattr(mod, "_osc_resolve_existing_local_path", lambda *_args, **_kwargs: str(ghost))
    ...
    result = mod._osc_effective_case_folder_for_row(
        {
            ...
            "status": "已結案",
            "legal_aid_status": "已結案",
            "folder_path": str(ghost),
        },
        update_db=True,
    )

    assert result["source"] == "closed_archive"
    assert result["local_folder"] == str(complete)
    assert result["folder_path"].replace("\\", "/") == str(complete)
    assert result["legacy_path_upgraded"] is True
    assert updates and updates[0][1][0].replace("\\", "/") == str(complete)


def test_archive_local_base_prefers_real_smb_over_user_mount(tmp_path, monkeypatch):
    from api.blueprints import osc_cases as mod

    real = tmp_path / "Volumes" / "lumi" / "03_工作資料" / "10_結案"
    ghost = tmp_path / ".magi_mounts" / "lumi" / "03_工作資料" / "10_結案"
    real.mkdir(parents=True)
    ghost.mkdir(parents=True)
    real_text = "/Volumes/lumi/lumi/03_工作資料/10_結案"
    ghost_text = "/Users/ai/.magi_mounts/lumi/lumi/03_工作資料/10_結案"

    monkeypatch.setattr(mod, "_osc_get_closed_archive_base", lambda: "Y:/lumi/03_工作資料/10_結案")
    monkeypatch.setattr(mod, "_osc_local_path_candidates", lambda _path: [ghost_text, real_text])
    monkeypatch.setattr(mod.os.path, "exists", lambda value: value in {ghost_text, real_text})

    _canonical, local = mod._osc_archive_local_base()

    assert local == real_text
