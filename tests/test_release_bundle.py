...
def test_build_release_bundle_excludes_sensitive_and_runtime_files(tmp_path):
    ...
    (source / "api" / "blob.zip").write_bytes(b"zip")
    (source / "scripts").mkdir()
    (source / "scripts" / "helper.py").write_text("print('helper')\n", encoding="utf-8")
    (source / "scripts" / "private.dylib").write_bytes(b"mach-o")
    (source / "scripts" / "tool.exe").write_bytes(b"exe")
    (source / "bin" / "__init__.py").write_text("", encoding="utf-8")
    (source / "bin" / "_runtime.py").write_text("ROOT = 'test'\n", encoding="utf-8")
    (source / "bin" / "cli.py").write_text("def main(): return 0\n", encoding="utf-8")
    (source / "bin" / "check.py").write_text("def main(): return 0\n", encoding="utf-8")
    (source / "bin" / "start.py").write_text("def main(): return 0\n", encoding="utf-8")
    (source / "bin" / "release_bundle.py").write_text("def main(): return 0\n", encoding="utf-8")
    (source / "bin" / "private-tool").write_text("#!/bin/bash\n", encoding="utf-8")
    (source / "bin" / "libprivate.dylib").write_bytes(b"mach-o")
    (source / "bin" / "llama.zip").write_bytes(b"zip")
    ...
    assert not (bundle / "api" / "blob.zip").exists()
    assert (bundle / "scripts" / "helper.py").exists()
    assert not (bundle / "scripts" / "private.dylib").exists()
    assert not (bundle / "scripts" / "tool.exe").exists()
    assert (bundle / "bin" / "cli.py").exists()
    assert (bundle / "bin" / "check.py").exists()
    assert (bundle / "bin" / "start.py").exists()
    assert not (bundle / "bin" / "private-tool").exists()
    assert not (bundle / "bin" / "libprivate.dylib").exists()
    assert not (bundle / "bin" / "llama.zip").exists()
