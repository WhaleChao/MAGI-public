...
def test_build_google_calendar_service_uses_authorized_http_timeout(monkeypatch, tmp_path):
    ...
    class FakeFlow:
        ...
        def from_client_secrets_file(*_args, **_kwargs):
            raise AssertionError("interactive oauth should not run")
    ...
    google_pkg = fake_pkg("google")
    oauth2_pkg = fake_pkg("google.oauth2")
    credentials_mod = types.ModuleType("google.oauth2.credentials")
    credentials_mod.Credentials = FakeCredentials
    auth_pkg = fake_pkg("google.auth")
    transport_pkg = fake_pkg("google.auth.transport")
    requests_mod = types.ModuleType("google.auth.transport.requests")
    requests_mod.Request = FakeRequest
    oauthlib_pkg = fake_pkg("google_auth_oauthlib")
    flow_mod = types.ModuleType("google_auth_oauthlib.flow")
    flow_mod.InstalledAppFlow = FakeFlow
    api_pkg = fake_pkg("googleapiclient")
    ...
    for name, module in {
        ...
        "google_auth_httplib2": auth_http_mod,
        "httplib2": httplib2_mod,
    }.items():
        monkeypatch.setitem(sys.modules, name, module)

    monkeypatch.setenv("MAGI_GCAL_HTTP_TIMEOUT_SEC", "17")
    out = mod._build_google_calendar_service("", str(token_path), interactive=False)

    assert out == {"ok": True, "service": {"service": True}}
...
def test_gcal_sync_repair_query_includes_calendar_deduped_rows(monkeypatch):
    ...
    assert "{osc_todo_source_sql" not in sql
    assert "COALESCE(ct.source_file, '') NOT LIKE 'gcal_import%%'" in sql
    assert out.get("ok") is True
