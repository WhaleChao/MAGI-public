...
def test_doc_kind_supports_pleading_filter():
    from api.osc.judicial import _osc_doc_kind_label, _osc_doc_kind_match

    blob = "02_我方歷次書狀 20260114_民事聲請調查證據狀(鑫源企業社)清稿.docx"

    assert _osc_doc_kind_match("pleading", blob) is True
    assert _osc_doc_kind_label(blob) == "書狀"
...
```"""
    ...
    assert "113年度訴字第100號　股別：義股" in table_text
    assert paragraphs[1].text.startswith("一、")
    assert paragraphs[1].paragraph_format.first_line_indent is not None
...
def test_export_html_strips_markdown_and_applies_body_classes():
    ...
    assert "class='meta-table'" in html
    assert "class='meta-label'>案號" in html
    assert "<td>113年度訴字第1號</td>" in html
