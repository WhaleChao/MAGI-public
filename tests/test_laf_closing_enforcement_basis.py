...
def test_consumer_debt_nonexemption_ruling_outcome_is_closing_basis(tmp_path):
    case_dir = tmp_path / "2025-0084-王台銘-消費者債務清理-清算"
    judgment_dir = case_dir / "10_判決書或終局裁定及處分"
    judgment_dir.mkdir(parents=True)
    terminal = judgment_dir / "20260703 臺北地方法院115年度消債職聲免字第26號民事裁定（王台銘；主文：債務人王台銘不免責）.pdf"
...
def test_consumer_debt_terminal_ruling_in_notice_folder_stays_out_of_closing_basis(tmp_path):
    ...
    assert str(terminal) not in docs["closing_basis_files"]
    assert str(terminal) in docs["misfiled_closing_basis_files"]
    assert not scanner._is_auto_closing_basis_candidate(
        str(terminal),
        case_reason="消費者債務清理",
        folder_path=str(case_dir),
...
def test_consumer_debt_rejected_rehabilitation_ruling_in_judgment_folder_counts_as_closing_basis(tmp_path):
    case_dir = tmp_path / "2025-0068-劉亞箖-消費者債務清理-更生"
    judgment_dir = case_dir / "10_判決書或終局裁定及處分"
    judgment_dir.mkdir(parents=True)
    terminal = judgment_dir / "20260617 宜蘭地方法院114年度消債更字第83號民事裁定（劉亞箖；主文：更生之聲請駁回、聲請程序費用由聲請人負擔）.pdf"
    terminal.write_bytes(b"%PDF-1.4\n")
...
def test_consumer_debt_nonterminal_ruling_in_notice_folder_stays_out_of_closing_basis(tmp_path):
    ...
    assert str(intermediate) not in docs["closing_basis_files"]
    assert str(intermediate) not in docs["misfiled_closing_basis_files"]
