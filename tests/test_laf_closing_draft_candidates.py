...
def test_auto_closing_accepts_nonexemption_ruling_outcome(tmp_path):
    ...
    assert len(candidates) == 1
    assert candidates[0]["laf_case_number"] == "1130919-T-057"
    assert candidates[0]["closing_basis_files"] == [str(terminal)]


def test_auto_closing_skips_terminal_consumer_debt_ruling_from_notice_folder(tmp_path):
...
def test_auto_closing_accepts_terminal_consumer_debt_ruling_from_judgment_folder(tmp_path):
    case_dir = tmp_path / "2025-0068-劉亞箖-消費者債務清理-更生"
    judgment_dir = case_dir / "10_判決書或終局裁定及處分"
    judgment_dir.mkdir(parents=True)
