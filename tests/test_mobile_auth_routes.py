...
def test_mobile_home_is_chat_first_and_accessible(monkeypatch):
    from pathlib import Path

    body = (Path(__file__).resolve().parents[1] / "templates" / "mobile_home.html").read_text(encoding="utf-8")
    assert 'id="mobileChatForm"' in body
    assert 'id="mobileChatInput"' in body
    assert 'id="mobileIntentStatus"' in body
    assert 'href="/osc"' in body
    assert "需要確認的動作仍會先詢問" in body


def test_mobile_chat_refreshes_csrf_without_logging_out_on_forbidden():
    from pathlib import Path

    root = Path(__file__).resolve().parents[1]
    script = (root / "static" / "mobile" / "mobile.js").read_text(encoding="utf-8")
    template = (root / "templates" / "mobile_home.html").read_text(encoding="utf-8")
    service_worker = (root / "static" / "mobile" / "sw.js").read_text(encoding="utf-8")

    assert 'readCookie("X-CSRF-Token")' in script
    assert 'headers["X-CSRF-Token"] = token' in script
    assert 'data.code === "csrf_validation_failed" && allowCsrfRetry' in script
    assert 'return sendChatRequest(outbound, false)' in script
    assert 'response.status === 401 || response.status === 403' not in script
    assert 'response.status === 401 || data.error === "auth_required"' in script
    assert "20260815-mobile-ia-v1" in template
    assert "magi-mobile-v20260815-ia-v1" in service_worker


def test_mobile_chat_error_ui_does_not_render_server_error_or_trace_identifier():
    from pathlib import Path

    script = (Path(__file__).resolve().parents[1] / "static" / "mobile" / "mobile.js").read_text(encoding="utf-8")
    assert "function friendlyFailureMessage(error)" in script
    assert 'appendMessage("assistant", friendlyFailureMessage(error));' in script
    assert 'appendMessage("assistant", "目前無法完成這則訊息，原文已保留在輸入框。", String(error.message || error));' not in script
    assert ': (data.error ||' not in script


def test_mobile_connection_badge_is_intrinsically_sized_and_adaptive():
    from pathlib import Path

    root = Path(__file__).resolve().parents[1]
    css = (root / "static" / "mobile" / "mobile.css").read_text(encoding="utf-8")
    body = (root / "templates" / "mobile_home.html").read_text(encoding="utf-8")

    assert '.live-state > [aria-hidden="true"]' in css
    assert '.live-state.is-online > [aria-hidden="true"]' in css
    assert ".live-state-label" in css
    assert "background: transparent !important" in css
    assert "min-width: 0; flex: 1 1 auto" in css
    assert "@media (max-width: 340px)" in css
    assert 'class="live-state-label"' in body
