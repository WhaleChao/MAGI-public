...
def test_google_calendar_token_spec_and_connected_contract(tmp_path, monkeypatch):
    token = tmp_path / "calendar-token.json"
    _write_token(
        token,
        expiry=datetime.now(timezone.utc) + timedelta(days=30),
        scopes=["https://www.googleapis.com/auth/calendar"],
    )
    monkeypatch.setenv("MAGI_GOOGLE_CALENDAR_TOKEN_PATH", str(token))

    spec = thc.google_calendar_token_spec()
    result = thc.check_google_token(spec)

    assert spec.name == "google_calendar"
    assert spec.token_path == token
    assert result["status"] == "ok"
    assert thc.google_token_connected(result) is True
