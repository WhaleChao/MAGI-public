...
def test_omlx_profile_policy_boundaries_are_single_source_of_truth():
    ...
    assert expected_profile_for_minutes(6 * 60 + 35) == ("day", "12b")
    assert expected_profile_for_minutes(21 * 60 + 49) == ("day", "12b")
    assert expected_profile_for_minutes(21 * 60 + 50) == ("night", "26b")
    assert expected_profile_now(datetime(2026, 5, 27, 6, 35)) == ("day", "12b")
...
def test_day_gate_fails_when_question_asks_for_day_but_26b_is_live(monkeypatch):
    ...
    assert any("8080 expected 12B" in item for item in report.failures)


def test_day_gate_marks_e4b_as_degraded_fallback(monkeypatch):
    probes = {
    ...
    assert report.degraded_reason == "day_fell_back_to_e4b"
