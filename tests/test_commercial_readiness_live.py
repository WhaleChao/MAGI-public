...
def test_public_cleanroom_snapshot_uses_current_worktree(monkeypatch, tmp_path):
    monkeypatch.delenv("MAGI_PUBLIC_SOURCE_ROOT_DIR", raising=False)
    monkeypatch.delenv("MAGI_SOURCE_ROOT_DIR", raising=False)
    repo = tmp_path / "repo"
    repo.mkdir()
...
def test_formal_saas_readiness_warns_when_mode_not_enabled(monkeypatch, tmp_path):
    monkeypatch.setattr(gate, "MAGI_ROOT", tmp_path)
    ...
    result = gate.check_formal_saas_readiness("python3")

    assert result.ok is True
    assert result.status == "warn"
    assert "formal SaaS mode not enabled" in result.detail
