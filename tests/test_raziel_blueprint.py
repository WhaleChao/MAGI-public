...
def test_raziel_is_reachable_from_osc_shell():
    root = Path(__file__).resolve().parents[1]
    ...
    assert 'data-tab="raziel"' in osc_html
    assert 'partials/osc/raziel.html' in osc_html
    assert "tabs/raziel.js" in osc_html
    assert "initRazielControls" in events_js
