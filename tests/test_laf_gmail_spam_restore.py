...
def test_laf_gmail_list_retries_bounded_transient_timeout(monkeypatch):
    from skills.legal.laf import LAFGmailMonitor
    import skills.legal.laf as laf_module
    ...
    class FlakyMessages(_FakeMessages):
        ...
        def list(self, **kwargs):
            ...
            self.attempts += 1
            if self.attempts < 3:
                return _FakeRequest(exc=TimeoutError("temporary timeout"))
            return _FakeRequest({"messages": [{"id": "MSG-OK"}]})

    messages = FlakyMessages()
    service = _FakeService({})
    service.messages_api = messages
    monitor = LAFGmailMonitor("credentials.json", "token.pickle")
    monitor.service = service
    monkeypatch.setenv("MAGI_LAF_GMAIL_LIST_ATTEMPTS", "3")
    monkeypatch.setattr(laf_module.time, "sleep", lambda _seconds: None)

    result = monitor._gmail_list_messages("subject:法扶", max_results=10)
...
def test_laf_gmail_monitor_can_explicitly_hold_json_only_marker(tmp_path, monkeypatch):
    from skills.legal.laf import LAFGmailMonitor

    monkeypatch.setenv("MAGI_LAF_GMAIL_RECOVER_JSON_ONLY", "0")
    logs = []
...
def test_laf_gmail_monitor_recovers_json_processed_when_db_missing_by_default(tmp_path):
    from skills.legal.laf import LAFGmailMonitor
