...
def test_laf_poa_pdf_creates_fillable_word_companion(tmp_path):
    ...
    assert "王惠薰" not in xml, "exact PDF layout 不應把同一份 PDF 已有/可有的資料再疊印一次"
    assert "70年1月1日" not in xml
    assert "A123456789" not in xml
    assert "花蓮市測試路1號" not in xml
    assert "1150529-E-005" not in xml
    assert "更生" not in xml
    assert result["template_values"]["LAWYER_NAME"] not in xml
    assert result["template_values"]["LAW_FIRM_ADDRESS_LINE"] not in xml
    if result["template_values"].get("LAW_FIRM_PHONE"):
        assert result["template_values"]["LAW_FIRM_PHONE"] not in xml
    assert "臺灣花蓮地方法院" not in xml
    assert "消債更" not in xml
    assert "標楷體" in xml
...
def test_laf_poa_existing_static_companion_is_rebuilt_for_exact_layout(tmp_path):
    ...
    assert "王惠薰" not in xml


def test_laf_poa_existing_exact_companion_is_rebuilt_for_native_word_when_exact_disabled(tmp_path, monkeypatch):
    import os
    ...
    _make_pdf(pdf)
    docx_path = laf_poa_docx_path(pdf)
    _fake_docx(docx_path, {"old": '<wp:anchor behindDoc="1"/>magi_laf_poa_overlay_0 王惠薰'})
    future = pdf.stat().st_mtime + 60
    os.utime(docx_path, (future, future))
    ...
    assert result["ok"] is True
    assert result["status"] == "created"
    assert "stale_exact_docx_rebuilt" in result["warnings"]
    assert laf_poa_template_docx_path(pdf).exists()
    assert "magi_laf_poa_overlay_" not in xml
...
def test_criminal_poa_pdf_creates_usable_content_controls_without_template(tmp_path):
    ...
    assert "喬政翔律師" not in xml
    assert "過失傷害" not in xml
...
def test_laf_poa_uses_sibling_packet_pdfs_for_contact_and_court(tmp_path, monkeypatch):
    import api.laf_poa_docx as poa
    ...
    monkeypatch.setattr(
        ...
        "_read_pdf_text",
        lambda path: (
            "手機\n0912345678\n通訊地址\n[971]花蓮縣新城鄉嘉里村嘉里三街113巷2弄38號\n住所地址"
            if "法律扶助申請書" in Path(path).name
            else "4.本案管轄法院(或機關)：檢察署,臺灣台東地方檢察署 案號：不清楚股別："
            if "案件概述單" in Path(path).name
            else ""
        ),
    )
    case_dir = tmp_path / "法扶案件" / "刑事" / "2026-0074-王曄芯-偵查-詐欺等" / "01_法扶資料"
    case_dir.mkdir(parents=True)
    pdf = case_dir / "委任狀_1150706-J-003_1150707.pdf"
    _make_pdf(
        pdf,
        "財團法人法律扶助基金會專用委任狀\n"
        "本會申請編號：1150706-J-003\n"
        "姓名\n王曄芯\n"
        "出生年月日\n78年3月28日\n"
        "U222355308\n"
        "喬政翔律師\n"
        "本事件經本會 台東分會 審核准予扶助，請逕致電分會(089-361363)。",
    )
    _fake_pdf(case_dir / "法律扶助申請書_1150706-J-003_1150707.pdf")
    _fake_pdf(case_dir / "案件概述單_1150706-J-003_1150707.pdf")
    ...
    result = ensure_laf_poa_docx_companion(
        ...
        case_metadata={
            ...
            "case_type": "刑事",
            "stage": "偵查",
            "law_firm_address_line": "970花蓮縣花蓮市明禮路18之6號1樓",
            "law_firm_phone": "03-835-7186",
        },
    )
    xml = _docx_xml(laf_poa_docx_path(pdf))
    ...
    assert result["ok"] is True
    assert result["filled_fields"]["client_address_phone"] == "971花蓮縣新城鄉嘉里村嘉里三街113巷2弄38號\n手機：0912345678"
    assert result["filled_fields"]["court_name"] == "臺灣台東地方檢察署"
    assert "<wp:anchor" in xml and 'behindDoc="1"' in xml
    assert "magi_laf_poa_overlay_" in xml
    assert not laf_poa_template_docx_path(pdf).exists()
    assert "971花蓮縣新城鄉嘉里村嘉里三街113巷2弄38號" not in xml
    assert "手機：0912345678" not in xml
    assert "臺灣台東地方檢察署" not in xml
    assert "請填法院案號" not in xml
    assert "臺灣台東地方檢察署　　　　　　　法　院" not in xml
    assert "magi_laf_poa.case_type_criminal" in xml
    assert "magi_laf_poa.stage_investigation" in xml
    assert "magi_laf_poa.role_defender" in xml
