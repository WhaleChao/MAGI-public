...
try:
    ...
    _env_file = os.environ.get("MAGI_ENV_FILE", "").strip()
    load_dotenv(_env_file or os.path.join(os.path.dirname(os.path.dirname(__file__)), ".env"))
except Exception:
...
_CLOUDFLARED_LOG_PATH = os.path.abspath(
    os.path.expanduser(os.environ.get("MAGI_CLOUDFLARED_LOG_PATH", "").strip())
    or os.path.join(_MAGI_ROOT, "logs", "cloudflared.log")
)
...
async def _line_self_heal_funnel() -> str:
    ...
    if code != 0:
        ...
        os.makedirs(os.path.dirname(_CLOUDFLARED_LOG_PATH), exist_ok=True)
        await _run_shell(
            "/opt/homebrew/bin/cloudflared tunnel --url http://127.0.0.1:18790 --no-autoupdate "
            f"2>{shlex.quote(_CLOUDFLARED_LOG_PATH)} &",
            timeout_sec=5,
        )
        await asyncio.sleep(10)  # Wait for tunnel to establish

    # Extract tunnel URL from cloudflared log
    log_path = _CLOUDFLARED_LOG_PATH
    cf_url = ""
    try:
        code2, out2, _ = await _run_shell(
            f"grep -o 'https://[a-z0-9-]*\\.trycloudflare\\.com' {shlex.quote(log_path)} 2>/dev/null | head -1",
            timeout_sec=5,
        )
    ...
    async with semaphore:
            ...
            *,
            ...
            try:
                ...
                logging.getLogger(__name__).warning(
                    "cron completion marker failed for %s", job_id, exc_info=True
                )
        ...
        if scheduler is not None:
            try:
                ...
                logging.getLogger(__name__).warning(
                    "cron job %s blocked before execution: "
            ...
            if not start_confirmed:
                ...
                logger.error(
                    "cron job %s blocked before execution: %s "
        ...
        if command.startswith("@MAGI"):
            try:
                ...
                if clean_cmd.lower() in {
                    ...
                    # Scheduled diagnostics are read-only and deterministic.
                    # Never route them into the code auto-fix/internalisation
                    # flow: that mutates an immutable V3 release and can hold a
                    # scheduler worker until the watchdog kills it.
                    def _run_bounded_diagnostic() -> str:
                        import subprocess as _subprocess

                        output = pathlib.Path(
                            os.environ.get("MAGI_RUNTIME_DIR", "").strip()
                            or pathlib.Path(_MAGI_ROOT) / ".runtime"
                        ) / "system_diagnostic_latest.json"
                        proc = _subprocess.run(
                            [
                                sys.executable,
                                str(pathlib.Path(_MAGI_ROOT) / "scripts" / "ops" / "system_diagnostic_report.py"),
                                "--json-out",
                                str(output),
                            ],
                            cwd=_MAGI_ROOT,
                ...
                if not classification.success:
                    logger.warning(
                        "⚠️ @MAGI cron job %s produced no successful action: status=%s error=%s",
                        job_id,
        ...
        try:
            ...
            if classification.status == "deferred":
                logger.info(
                    "⏸️ Shell job %s deferred without completing work: %s",
                    job_id,
                    classification.error,
                )
            elif not classification.success:
                ...
                if should_log_cron_issue(_sr.returncode, _stdout_text, _stderr_text):
                    ...
                    logger.info(
                        "ℹ️ Shell job %s returned %d or failed its result contract; issue suppressed by policy but result remains failed",
                        job_id,
                        _sr.returncode,
                    )
            else:
            ...
            _record_result(
                ...
                timed_out=bool(_sr.timed_out),
                error=(
                    f"deferred:{classification.error}"
                    if classification.status == "deferred"
