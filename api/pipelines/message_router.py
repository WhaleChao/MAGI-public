...
from api.runtime_paths import get_mutable_static_dir, get_runtime_dir
try:
...
def magi_capability_overview() -> str:
    return (
        ...
        "• **受控自主 Agent**：可用自然語言交代目標；查詢與分析會自行選擇工具，"
        "寫入、上傳、送出或刪除會建立可追蹤計畫並取得一次性核准。\n"
        "  可輸入「列出自主計畫」、「自主計畫狀態 <編號>」或「取消自主計畫 <編號>」。\n"
        "• **一般對話**：也可以聊天、討論想法、解釋概念；涉及時效性資料時會使用查詢工具。\n\n"
...
_GIBBERISH_LOG_PATH = _Path(os.environ.get(
    "MAGI_GIBBERISH_LOG",
    str(get_mutable_static_dir() / "gibberish_samples.jsonl"),
))
...
def quick_fixed_reply(orch, message: str, role: str = "user") -> Optional[str]:
    ...
    if t in _AUTOCHECK_EXACT:
        ...
        try:
            ...
            out_path = get_runtime_dir() / "autocheck_latest.json"
...
def try_conversational_intent(orch, message: str, msg_lower: str, user_id, role: str, platform: str):
    ...
    is_direct_sentencing_request = bool(re.search(
        r"(?:法官量刑(?:與判決)?趨勢|量刑(?:與判決)?趨勢|判決趨勢|裁判趨勢)",
        low_compact,
        re.IGNORECASE,
    ))

    if not (is_question or has_conv_signal or is_soft or is_direct_sentencing_request):
        return None
    ...
    patterns = [
        ...
        (r"(?:查判決|找判決|判決搜尋|搜尋判決|收集判決|判決搜集|"
         ...
         r"法院判決|查法規|查法條|法規查詢|法條查詢|查釋字|釋字|憲判|實務見解|法律見解|法院見解|"
         r"(?:法律上|依民法|依刑法|侵權行為|違約|損害賠償|不當得利|舉證責任|消滅時效|請求權時效|構成要件|法律效果|管轄法院).{0,36}"
         r"(?:如何|怎麼|為何|是否|可否|哪些|要件|責任|時效|效力|認定|成立|分配)|court\s*judgment)", "judgment_search",
         "✅ **我可以幫您查判決！**\n\n"
         "• 直接輸入：`查判決 傷害`\n"
        ...
        (r"(?:法官量刑(?:與判決)?趨勢|量刑(?:與判決)?趨勢|判決趨勢|趨勢分析|案由分析|判決分析|案由統計|"
         r"判決統計|見解趨勢|裁判趨勢)", "judgment_trend",
         "✅ **我可以查法官量刑與判決趨勢！**\n\n"
         "• 特定案由：`判決趨勢 詐欺`\n"
         "• 法院、法官與期間：`查臺灣花蓮地方法院王小明法官詐欺案件，民國112年至115年的量刑趨勢`\n"
         "結果與網頁版共用同一套 MCP、官方全文、附表與定執行刑品質閘門。", True),
