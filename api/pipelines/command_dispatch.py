...
"📝 **文件產生**\n"
"━━━━━━━━━━━━━━━━━━━━\n"
"• `委任狀` / `幫我做委任狀` — 民事／刑事／行政委任狀\n"
"• `契約書` / `幫我做委任契約書` — 委任契約書\n"
"• `收據` / `幫我開收據` — 律師費收據\n"
"• `存證信函` / `幫我寫存證信函` — 存證信函 PDF\n"
"• `審閱契約 [上傳契約]` — 合約風險分析 🔄\n"
...
"• `法扶回報指令` — 顯示法扶回報完整說明\n"
"  以下均可用自然語言觸發：\n"
"• `幫我做[姓名]開辦回報` — 開辦草稿（存檔不送出）\n"
"• `幫[姓名]做疑義回報 原因...` — 疑義回報\n"
"• `[姓名]訴訟中費用支付回報 說明...` — 費用回報\n"
"• `[姓名]二階段回報` — 二階段回報\n"
"• `[姓名]結案回報 原因...` — 結案回報\n"
"• `[姓名]受扶助人撤回回報 原因...` — 撤回回報\n"
"  送出確認：系統會給確認碼，回覆 `正確送出 <確認碼>` 才送出\n"
"• `法扶監控` — 法扶案件狀態總覽\n"
"• `自動報結掃描` / `二階段批次` — 批次報結作業\n"
"  手動更新狀態：\n"
...
"• `判決趨勢 [案由]` — 判決趨勢分析\n"
"• `法規搜尋 [查詢]` — 查詢法規\n"
"• `加班費 [條件]` — 勞基法試算（加班費/特休/資遣費）\n"
"• `庭期` / `最近有什麼庭` — 開庭排程與提醒\n"
"• `準備 [案號] 的開庭資料` — 庭前準備\n"
"• `[姓名]繳了` / `[案號]交了` — 標記庭期任務完成\n"
"• `案件時程總覽` — 全部案件排程\n"
"• `司法工具` — 規費/折舊/刑度試算 🔄\n"
"\n"
"━━━━━━━━━━━━━━━━━━━━\n"
...
"🗂️ **案件 & 帳務管理**\n"
"━━━━━━━━━━━━━━━━━━━━\n"
"• `建案 <案號> <當事人> <類型> <案由>` — 新增案件\n"
"• `案件清單` / `查案件 <關鍵字>` — 查詢案件\n"
"• `<當事人> 改為已結案` — 更新案件狀態\n"
"• `業務概況` — 案件狀態統計\n"
"• `新增當事人 <姓名> <電話> <地址>` — 新增當事人\n"
"• `查當事人 <姓名>` / `<姓名>的資料` — 查詢當事人\n"
"• `記收入 <金額> <類別> <案件>` — 記錄收入\n"
"• `記支出 <金額> <類別> <案件>` — 記錄支出\n"
"• `本月帳務` — 本月收支統計\n"
...
"🤖 **AI 自主工具 (對話時自動呼叫)**\n"
"━━━━━━━━━━━━━━━━━━━━\n"
"MAGI 的 ReAct 引擎在回答時會視情況自主使用以下 **12 個唯讀/分析型工具**（不會寫入系統、不會操作 portal）：\n"
"• `judicial-web-search` — 司法院判決搜尋\n"
"• `statutes-vdb` — 法規條文檢索\n"
"• `labor-law-calculator` — 勞基法試算（資遣費/加班費/特休）\n"
"• `contract-review` — 合約風險分析\n"
"• `judgment-collector` — 依案由彙整判決摘要\n"
"• `worldmonitor-intel` — 全球/法律新聞\n"
"• `pdf-namer` / `pdf-bookmarker` — PDF 檔名提案 / 頁籤生成（已取代 deprecated 的 pdf-annotator）\n"
"• `translator` — 翻譯（法律長文自動 APE 潤飾）\n"
"• `market-briefing` — 股市晨報 / 追蹤清單\n"
"• `trial-prep` — 開庭準備摘要\n"
"• `osc-orchestrator` — 案件/當事人/帳務查詢\n"
"🚫 **安全紅線**：LAF / 閱卷 / 筆錄 / 記憶寫入 / portal 送出 **不在白名單**，必須透過對應專屬指令觸發\n"
"\n"
"━━━━━━━━━━━━━━━━━━━━\n"
"🧬 **技能進化 & 系統 (管理員)**\n"
"━━━━━━━━━━━━━━━━━━━━\n"
...
"• `幫我做[姓名]開辦回報` — 開辦草稿\n"
"• `幫[姓名]做疑義回報 原因...` — 疑義回報\n"
"• `[姓名]結案回報` / `[姓名]二階段回報` — 結案/二階段\n"
"  送出確認：系統給確認碼，回覆 `正確送出 <碼>` 送出\n"
"• `法扶監控` — 法扶案件狀態總覽\n"
"• `[姓名] 已開辦` / `[姓名] 已報結` — 手動更新狀態\n"
"\n"
"━━━━━━━━━━━━━━━━━━━━\n"
"📂 **閱卷 & 筆錄**\n"
"━━━━━━━━━━━━━━━━━━━━\n"
...
"• `建案 <案號> <當事人>` / `案件清單` / `查案件 <關鍵字>` — 案件管理\n"
"• `記收入` / `記支出` / `本月帳務` — 帳務管理\n"
"• `單檔命名` / `批次命名` — PDF 自動命名（pdf-namer；頁籤生成由 pdf-bookmarker 取代舊 pdf-annotator）\n"
"• 直接對話 — 一般問答\n"
"\n"
"━━━━━━━━━━━━━━━━━━━━\n"
"🤖 **AI 自主工具 (對話時自動呼叫)**\n"
"━━━━━━━━━━━━━━━━━━━━\n"
"MAGI 回答問題時會視需要自動使用以下 12 個唯讀/分析工具：\n"
"• 判決搜尋 / 法規檢索 / 判決收集 / 合約審閱\n"
"• 勞基法試算 / 開庭準備 / 翻譯 / 股市晨報\n"
"• PDF 檔名提案 + 頁籤生成 / 案件當事人帳務查詢 / 全球新聞\n"
"🚫 **安全紅線**：法扶 / 閱卷 / 筆錄 / 記憶寫入 / portal 送出 **不在此列**，必須用對應專屬指令\n"
"\n"
"⚡ = 即時回覆　🔄 = 語意路由（用自然語言觸發）\n"
"💡 直接打指令即可，專屬頻道也支援自然語言\n"
"🔒 系統管理、技能進化等需管理員權限"
