...
def _default_shared_cache_root() -> Path:
    ...
    if shared:
        return Path(shared).expanduser() / "cache" / "judgment_collector"
    return (
        Path.home()
        / "Library"
        / "Application Support"
...
DEFAULT_JUDGMENT_CACHE_ROOT = _default_shared_cache_root()
DEFAULT_JUDGMENT_CACHE_FALLBACK = DEFAULT_JUDGMENT_CACHE_ROOT.parent / "judgment_collector_local"
...
def _ordinary_pytest_mode() -> bool:
    return _truthy("MAGI_TEST_MODE") and not _truthy("MAGI_ENABLE_LIVE_TESTS")
...
def preferred_nas_judgment_cache_root(
    *,
    ...
    if _ordinary_pytest_mode() and not str(os.environ.get(NAS_FALLBACK_ENV) or "").strip():
        return None
