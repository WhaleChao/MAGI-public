    ...
    *,
    ...
    if heavy_opt_in:
        return ModelRouteDecision(
            ...
            reason="@heavy explicitly requested; NVIDIA NIM is required and failures close without local fallback",
            task_type=task,
    ...
    # Admission is safe by default: request threads only describe the queued
    # local deep work.  The separately-owned worker may switch topology after
    # its resource and transaction evidence gates pass.
    if quality_needed and _env_bool("MAGI_LOCAL_DEEP_ADMISSION", True):
        try:
            from api.deep_task_control import assess_deep_admission
            deep = assess_deep_admission(task_type=task, prompt=prompt)
        except Exception:
            deep = None
        if deep and deep.required and not _active_has(active, "26b"):
            return ModelRouteDecision(
                selected_model=stable_model,
                tier="stable_local",
                reason=f"{deep.reason}; enqueue controlled local deep worker (no request-thread switch)",
                task_type=task,
                provider="omlx",
                active_models=active,
                preferred_model=TEXT_HEAVY_MODEL,
                resource_level=rv.level,
                safe_context_tokens=(stable_spec.safe_context_tokens if stable_spec else 8192),
                should_queue=True,
                local_deep_queue=True,
            )
