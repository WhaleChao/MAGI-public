...
_TOOL_ACTION_VERB_RE = re.compile(
    r"(?:查一下|查詢|幫我查|幫忙查|幫.{0,6}查|看一下|看看|告訴我|請.{0,6}查|麻煩.{0,6}查|lookup|search|get)",
    re.IGNORECASE,
)
...
_AGENTIC_VERB_RE = re.compile(
    ...
    r"說明|解釋|評估|判斷|推論|規劃|建議|彙整|比對"
    r")",
    re.IGNORECASE,
)
_AGENTIC_DOMAIN_RE = re.compile(
    r"(?:"
    r"案件|當事人|案號|法院|判決|裁判|實務見解|法規|法條|法律|民法|刑法|"
    r"行程|日曆|開庭|調解|檔案|文件|資料|卷證|pdf|word|docx|"
