...
_FAILURE_PREFIX_RE = re.compile(
    r"^\s*(?:\[[^\]\r\n]{1,24}\]\s*)?"
    r"(?:❌|錯誤[:：]|失敗[:：]|處理失敗|執行失敗|"
    r"系統暫時忙碌|無法(?:完成|處理|執行|連線))",
    re.IGNORECASE,
)
_WAIT_RE = re.compile(r"(?:等待確認|請回覆[「\"]?確認|確認是否)")
_INTERNAL_STATUS_PLATFORMS = frozenset({"smoke", "test", "healthcheck", "health_check"})


def should_publish_public_agent_status(platform: str) -> bool:
    """Keep automated probes from replacing the operator's current activity."""
    return str(platform or "").strip().lower() not in _INTERNAL_STATUS_PLATFORMS
...
def observe_finish(message: str, result: Any, *, failed: bool = False) -> dict[str, Any]:
    ...
    error = _result_failed(result, result_text=result_text, failed=failed)
    snapshot = {
...
def _result_failed(result: Any, *, result_text: str, failed: bool) -> bool:
    """Recognise terminal failures without treating explanatory prose as one.

    A successful legal answer often contains words such as ``無法`` or ``逾時``
    while explaining a rule.  Those words are not execution evidence.  Prefer
    explicit structured outcomes and only accept a failure phrase when it is a
    user-facing failure prefix.
    """
    if failed:
        return True
    if isinstance(result, dict):
        ...
        if isinstance(verification, dict):
            if str(verification.get("status") or "").strip().lower() == "failed":
                return True
        elif str(verification or "").strip().lower() == "failed":
            return True
    return bool(_FAILURE_PREFIX_RE.search(str(result_text or "")))
