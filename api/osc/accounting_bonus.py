...
def export_monthly_bonus_xlsx(result: dict[str, Any], output_path: str | Path | None = None) -> str:
    ...
    if output_path is None:
        # V3 binds mutable exports through MAGI_EXPORTS_DIR.  Keep the legacy
        # singular name for compatibility, but never fall back to the sealed
        # release's static/exports directory when the V3 binding is present.
        out_dir = Path(
            os.environ.get("MAGI_EXPORT_DIR")
            or os.environ.get("MAGI_EXPORTS_DIR")
            or Path.cwd() / "static" / "exports"
        )
