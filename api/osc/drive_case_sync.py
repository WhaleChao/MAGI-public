...
DEFAULT_LOCAL_HASH_TIMEOUT_SEC = 20
...
def _db_row_to_local_case(row: dict[str, Any], *, for_write: bool = False) -> CaseFolder | None:
...
def db_local_cases_for_numbers(case_numbers: Iterable[str], *, for_write: bool = False) -> tuple[list[CaseFolder], list[dict[str, Any]]]:
    ...
    for case_number in [str(x or "").strip() for x in case_numbers if str(x or "").strip()]:
        ...
        if not row:
            ...
            continue
        case = _db_row_to_local_case(row, for_write=for_write)
        if not case:
            skipped.append({
                "case_number": case_number,
    ...
    *,
    ...
    for (semantic_first, size), same_size_entries in size_buckets.items():
        ...
        for entry in same_size_entries:
            try:
                digest = local_file_md5(entry.path)
            except Exception:
        ...
        for duplicate in sorted((e for e in entries if e is not canonical), key=lambda e: e.relative_path):
            ...
            if execute:
                try:
                    ...
                    if normalize_text(local_file_md5(str(source_path))) != digest:
                        raise DriveCaseSyncError(f"duplicate_hash_changed:{source_path}")
                    if normalize_text(local_file_md5(str(canonical_path))) != digest:
...
def _find_same_content_local_entry(drive_entry: FileEntry, local_entries: Iterable[FileEntry]) -> tuple[FileEntry | None, str, str]:
    ...
    for local_entry in local_entries:
        ...
        try:
            ...
            if pending[0] is None:
                pending = (local_entry, key, f"local_hash_failed:{type(exc).__name__}")
            continue
...
def upload_local_file_to_drive(
    ...
    *,
    ...
    if existing:
        ...
        return {
            "status": "pending_existing_unverified" if existing_size == local_size else "pending_existing_conflict",
            "drive_id": str(existing.get("id") or ""),
            "web_url": str(existing.get("webViewLink") or ""),
            "bytes": 0,
            "created_folders": created_folders,
...
def local_file_md5(path: str, *, chunk_size: int = 1024 * 1024, max_bytes: int | None = None) -> str:
    ...
    timeout = float(os.environ.get("MAGI_DRIVE_SYNC_LOCAL_HASH_TIMEOUT_SEC") or DEFAULT_LOCAL_HASH_TIMEOUT_SEC)
    can_alarm = (
        timeout > 0
        and hasattr(signal, "SIGALRM")
        and threading.current_thread() is threading.main_thread()
    ...
    for item in ordered_items:
        ...
        for key, drive_entry in sorted(drive_files.items()):
            ...
            if not local_entry:
                ...
                if not local_entry:
                    duplicate_entry, duplicate_key, duplicate_verification = _find_same_content_local_entry(drive_entry, local_entries)
...
def _cleanup_stale_drive_sync_tmp_files(directory: Path, *, now: float | None = None) -> dict[str, Any]:
...
def _download_drive_entry(service: Any, entry: FileEntry, target_path: Path) -> dict[str, Any]:
    ...
    _cleanup_stale_drive_sync_tmp_files(target_path.parent)
    request = None
    export = GOOGLE_EXPORT_MIME_MAP.get(entry.mime_type)
    if export:
...
def run_priority_case_sync(
    *,
    ...
    local_cases, skipped_db_cases = db_local_cases_for_numbers(clean_numbers, for_write=bool(execute_downloads))
