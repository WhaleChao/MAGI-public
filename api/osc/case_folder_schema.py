...
def is_judgment_folder_segment(segment: str, *, prefixes: tuple[int, ...] = JUDGMENT_FOLDER_REPAIR_PREFIXES) -> bool:
    ...
    if clean in {JUDGMENT_FOLDER_LABEL, LEGACY_JUDGMENT_FOLDER_LABEL, JUDGMENT_DRIVE_LABEL}:
        return True
    if any(text in judgment_folder_aliases(prefix, include_plain=False) for prefix in prefixes):
        return True
    # Imported and historical case trees use descriptive folder names instead
    # of the current numeric schema.  Recognise their meaning, while keeping
    # notices/procedural rulings in the separate notice scanner.
    compact = re.sub(r"[\s_、，,()（）.\-]+", "", clean)
    if any(token in compact for token in _NONFINAL_COURT_FOLDER_TOKENS):
        return False
    return any(token in compact for token in _FINAL_DOCUMENT_FOLDER_TOKENS)
