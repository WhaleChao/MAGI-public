...
from api.legal_research_quality import EXTERNAL_CANDIDATE, prepare_external_legal_query
...
class TLRJudgment:
    ...
    case_history: list[Any] = field(default_factory=list)
...
def sanitize_tlr_query(query: str, *, max_chars: int = 160) -> str:
    """Return a privacy-approved abstract query, or an empty fail-closed value."""
    decision = prepare_external_legal_query(query, max_chars=max_chars)
    return decision.safe_query if decision.external_allowed else ""
...
class TLRClient:
    ...
    def fetch_fulltext(self, judgment: TLRJudgment) -> TLRJudgment:
        ...
        judgment.fulltext = str(
            data.get("fulltext_excerpt")
        ...
        history = data.get("case_history") or []
        judgment.case_history = history if isinstance(history, list) else [history]
        return judgment
...
def _judgment_to_item(judgment: TLRJudgment, citation_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "case_history": judgment.case_history,
    }
...
def search_practical_judgments_via_tlr(
    ...
    *,
    ...
    safe_query = privacy.safe_query if privacy.external_allowed else ""
    if not safe_query:
        return {
            "success": False,
            "ok": False,
            "error": "privacy_gate_blocked_external_query",
            "source": SOURCE_NAME,
