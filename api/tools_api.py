...
from api.runtime_paths import get_agent_dir, get_metrics_dir, get_orch_dir
...
try:
    ...
    _load_dotenv(os.environ.get("MAGI_ENV_FILE", "").strip() or None)
except Exception:
...
EXTERNAL_CHAT_METRICS_PATH = os.environ.get("MAGI_EXTERNAL_CHAT_METRICS_PATH", "").strip()
if not EXTERNAL_CHAT_METRICS_PATH:
    metrics_root = os.environ.get("MAGI_METRICS_DIR", "").strip()
    EXTERNAL_CHAT_METRICS_PATH = os.path.join(
        metrics_root or os.path.join(_MAGI_ROOT, "static"),
        ...
        for root in skill_roots():
            ...
            for entry in os.scandir(root):
                if not entry.is_dir(follow_symlinks=False):
                    continue
                name = entry.name
                if name in seen or not is_public_skill_dir_name(name):
                    continue
                seen.add(name)
                try:
                    action_path = runtime_skill_dir(name) / "action.py"
                except ValueError:
                    continue
...
def _resolve_tools_events_path() -> str:
    """Keep mutable tool telemetry outside an immutable V3 release tree."""
    return str(get_agent_dir() / "tools_runtime_events.jsonl")


_TOOLS_EVENTS_PATH = _resolve_tools_events_path()
...
def _resolve_skill_action_path(skill_name: str) -> str:
    ...
    for candidate in candidates:
        ...
        seen.add(candidate)
        try:
            action_path = runtime_skill_dir(candidate) / "action.py"
        except ValueError:
            continue
        if action_path.is_file() and not action_path.is_symlink():
...
def _check_tool_access(
    ...
    *,
    command_subject: str = "",
    path_subject: str = "",
    require_path_subject: bool = False,
):
    ...
    if require_path_subject and not path_subject:
        return False, PermissionDecision(
            allowed=False,
            reason="skill_action_path_unresolved",
            mode=PermissionMode.PERMISSIVE,
            subject_kind="path",
            subject="",
            matched_rule="tools_api.require_skill_action_path",
...
_TOOLS_HEALTH_CACHE = {"ts": 0.0, "payload": None}


def _tools_health_snapshot(*, fresh: bool = False) -> dict:
    now = time.time()
    cached = _TOOLS_HEALTH_CACHE.get("payload")
    if isinstance(cached, dict) and not fresh and now - float(_TOOLS_HEALTH_CACHE.get("ts") or 0.0) < 10:
        payload = dict(cached)
        payload["cached"] = True
        return payload

    executor_ok = not bool(getattr(_INFERENCE_EXECUTOR, "_shutdown", False))
    checks = {
        "executor": {"ok": executor_ok},
        "capacity": {
            "ok": _TOOL_MAX_INFLIGHT > 0 and _INFER_MAX_INFLIGHT > 0,
            "tool_inflight": int(_TOOL_INFLIGHT_COUNT[0]),
            "tool_limit": int(_TOOL_MAX_INFLIGHT),
            "inference_inflight": int(_INFER_INFLIGHT_COUNT[0]),
            "inference_limit": int(_INFER_MAX_INFLIGHT),
        },
    }

    model_required = str(os.environ.get("MAGI_TOOLS_HEALTH_PROBE_MODEL", "1") or "1").strip().lower() in {
        "1", "true", "yes", "on",
    }
    if model_required:
        model_base = str(
        ...
        try:
            ...
            timeout = min(5.0, max(0.25, float(os.environ.get("MAGI_TOOLS_HEALTH_TIMEOUT_SEC", "1.5") or "1.5")))
            response = _get_session().get(model_url, timeout=timeout)
            status_code = int(getattr(response, "status_code", 0) or 0)
            data = response.json() if status_code == 200 else {}
            models = data.get("data") if isinstance(data, dict) and isinstance(data.get("data"), list) else []
            checks["model"] = {
                "ok": status_code == 200 and bool(models),
            ...
            checks["model"] = {
                "ok": False,
                "required": True,
                "error": f"{type(exc).__name__}: {str(exc)[:96]}",
            }
    else:
        checks["model"] = {"ok": None, "required": False, "status": "deferred"}

    required_checks = [item for item in checks.values() if item.get("required", True)]
    ok = all(item.get("ok") is True for item in required_checks)
    payload = {
        "ok": ok,
        "status": "ready" if ok else "not_ready",
        "service": "MAGI Tools API",
        "probe": "readiness",
        "checks": checks,
        "timestamp": now,
        "cached": False,
    }
    _TOOLS_HEALTH_CACHE["ts"] = now
    _TOOLS_HEALTH_CACHE["payload"] = dict(payload)
    return payload
...
def health():
    payload = _tools_health_snapshot(
...
def external_osc_health():
    ...
    key_present = bool(_resolve_external_api_key())
    snapshot = _tools_health_snapshot()
...
def sages_status():
    ...
    return jsonify({
        "casper": {
            "online": bool(casper_health.get("ok")),
            "status": casper_health.get("status"),
...
import threading
from api.startup import _startup_feature_enabled

if not _STARTUP_HOOKS_DISABLED and _startup_feature_enabled("MAGI_TOOLS_API_WARMUP_ON_START"):
    threading.Thread(target=_warmup_background, daemon=True).start()

if __name__ == '__main__':
    _tools_host = os.environ.get("MAGI_TOOLS_HOST", "127.0.0.1").strip() or "127.0.0.1"
    _tools_port = int(os.environ.get("MAGI_TOOLS_PORT", "5003") or "5003")
    logging.getLogger("tools_api").info("MAGI Tools API starting on http://%s:%s", _tools_host, _tools_port)
    from api.wsgi_server import serve

    serve(app, host=_tools_host, port=_tools_port, service_name="tools-api")
