...
from api.osc.utils import (
    ...
    _osc_isdir_quick, _osc_isfile_quick, _osc_stat_quick,
    _osc_stage_bulkhead,
...
from api.osc.judicial import (
    ...
    _osc_court_summary_displayable,
)
...
from api.osc.calendar_sources import (
    ...
    is_google_calendar_import,
    osc_todo_source_sql,
    todo_source_api_fields,
)
...
from magi_v3.case_lifecycle import (
    CaseLifecyclePhase,
...
def _osc_audit_file_event(action: str, path: str, *, status: str = "ok", metadata: dict | None = None) -> None:
    try:
        ref = file_ref(path)
        payload = {"file": ref}
        if metadata:
...
def _osc_sql_string_list(values: tuple[str, ...]) -> str:
    return ", ".join("'" + str(value).replace("'", "''").lower() + "'" for value in values)


def _osc_todo_open_status_sql(column: str = "status") -> str:
    done_values = _osc_sql_string_list(_osc_todo_done_statuses())
    return f"({column} IS NULL OR {column}='' OR LOWER({column}) NOT IN ({done_values}))"
...
def _osc_todo_api_row(row: dict | None) -> dict | None:
    if not isinstance(row, dict):
        return row
    out = dict(row)
    out["is_done"] = _osc_is_todo_done_status(out.get("status") or "")
    out.update(todo_source_api_fields(out))
    return out


def _osc_legal_aid_case_sql() -> str:
    return """
    (
...
def _laf_build_activity_stats(
    ...
    for t in todos or []:
        source = "Google Calendar" if is_google_calendar_import(t) else "待辦"
...
def _osc_is_real_nas_case_root(path: str) -> bool:
    norm = str(path or "").replace("\\", "/").rstrip("/")
    return (norm.startswith("/Volumes/") or "/Volumes/" in norm) and not _osc_is_synology_drive_fallback_path(norm)


def _osc_is_production_case_root(path: str) -> bool:
    norm = str(path or "").replace("\\", "/")
    return (
...
def _osc_payload_looks_synthetic(payload: dict) -> bool:
    joined = " ".join(
...
def _osc_case_folder_creation_guard(payload: dict, root: str) -> dict:
    root = str(root or "").strip()
    if not root:
        return {"ok": False, "status": "pending", "reason": "mount_required", "error": "nas_case_root_mount_required"}
    if _osc_is_synology_drive_fallback_path(root):
        return {
            "ok": False,
            "status": "pending",
            "reason": "file_provider_root_blocked",
            "error": "nas_case_root_mount_required",
            "message": "建立案件資料夾必須使用真實 SMB/NAS root，不能寫入 CloudStorage、SynologyDrive 或 .magi_mounts fallback。",
            "root": root,
        }
    if _osc_is_production_case_root(root) and _osc_payload_looks_synthetic(payload):
        return {
            "ok": False,
            "blocked": True,
            "reason": "synthetic_case_production_root_blocked",
            "error": "synthetic_case_production_root_blocked",
            "message": "測試/合成案件不得在 production NAS root 建立資料夾。",
            "root": root,
        }
    return {"ok": True}
...
def _osc_select_case_creation_root() -> dict:
    ...
    def _find_existing_non_cloud() -> str:
        for root in roots:
            if not _osc_is_real_nas_case_root(root):
                continue
            try:
...
2. 格式需符合台灣法院書狀慣例：標題、案號/股別、當事人、案由開場、分點理由、此致法院、日期與具狀人。
3. 僅能使用本 Prompt 明確提供的事實與實務見解；資料不足處請以「（待確認）」標示，不得自行杜撰日期、金額、證據編號或裁判字號。
4. 裁判字號只能取自「可引用裁判白名單」，且每個法律命題必須能對應該裁判所附的支持原文；白名單以外的裁判一律不得引用或自行補造。
5. 確保案號、股別、法院名稱正確填入狀頭；不得把內部 OSC 案號誤當法院案號。
6. 論述需有邏輯、條理分明，避免 Markdown 標題、表情符號、程式碼框與解釋性前言。
7. 請直接輸出可貼進 Word 的書狀內文，不要附加「以下是草稿」等說明。
8. 本稿仍須律師人工確認後才能匯出或對外提出。
...
def _osc_auto_create_folder_for_case(row_id: str, payload: dict, case_category: str) -> dict:
    ...
    guard = _osc_case_folder_creation_guard(payload, str(root_selection.get("root") or ""))
    if not guard.get("ok"):
        return guard
...
def _osc_should_auto_create_folder_on_insert(payload: dict, *, is_template: bool, status_value: str) -> bool:
    """Return True only for explicit, guarded folder creation requests."""
...
def _osc_should_ensure_case_folder_for_open(row: dict) -> bool:
    """Read/open/browser routes must never create folders as a side effect."""
    return False
...
def osc_cases_api():
    ...
    try:
        ...
        if requires_closed_storage(
            {"status": status_value, "legal_aid_status": payload.get("legal_aid_status")}
        ):
        ...
        target_final_closed = (
            phase_for_status(target_status) is CaseLifecyclePhase.CLOSED
        ...
        if requires_closed_storage(
            {
                "status": update_payload.get("status") or target_status,
                "legal_aid_status": update_payload.get("legal_aid_status") or target_laf_status,
            }
        ):
...
def osc_case_detail_api(row_id):
    ...
    try:
        ...
        if refreshed and refreshed.get("folder_path"):
            ...
            if folder_sync.get("updated") or folder_sync.get("source") in {"metadata_expected", "sibling_existing"}:
                resp["folder_sync"] = folder_sync
    except Exception:
        _log.debug("silent-catch case folder sync after update", exc_info=True)
    lifecycle_row = {
        "status": payload.get("status") if "status" in payload else (refreshed or {}).get("status"),
        "legal_aid_status": (
            payload.get("legal_aid_status")
            if "legal_aid_status" in payload
            else (refreshed or {}).get("legal_aid_status")
        ),
    }
    if requires_closed_storage(lifecycle_row):
...
def _osc_is_closed_case_status(status: str) -> bool:
    return phase_for_status(status) is not CaseLifecyclePhase.OPEN
...
def _osc_archive_local_base() -> tuple[str, str]:
    ...
    archive_local_candidates = sorted(
        _osc_unique_strings(_osc_local_path_candidates(_osc_norm_path(archive_base))),
        key=lambda value: (
            0 if str(value).startswith("/Volumes/") else
            2 if "/Library/CloudStorage/" in str(value) else
            3 if "/.magi_mounts/" in str(value) else
            1,
            str(value),
        ),
    )
...
def _osc_expected_closed_case_folder_for_row(
    row: dict,
    *,
    require_existing: bool = True,
) -> str:
    """Return the metadata-derived archive folder, preferring real SMB roots.

    Historical rows sometimes stored a flat ``Y:/.../10_結案/<case>`` path
    while the verified archive lives at ``<category>/<type>/<case>``.  A stale
    ``~/.magi_mounts`` shell at the flat path must not win merely because it
    exists.  Resolve the standard layout first, then fall back to the bounded
    case-number search used for migrated legacy folders.
    """
    row = dict(row or {})
    case_number = str(row.get("case_number") or "").strip()
    if not case_number:
        return ""
    folder_name = _osc_expected_case_folder_name(row)
    try:
        from api.osc.folder_utils import build_full_case_path

        active_roots = _get_preferred_case_roots(include_closed=False)
        all_roots = _get_preferred_case_roots(include_closed=True)
        roots = all_roots[len(active_roots):] if len(all_roots) >= len(active_roots) else []
        _archive_base, archive_local = _osc_archive_local_base()
        if archive_local:
            roots.append(archive_local)
        ordered_roots = sorted(
            _osc_unique_strings(roots),
            key=lambda value: (
                0 if str(value).startswith("/Volumes/") else
                2 if "/Library/CloudStorage/" in str(value) else
                3 if "/.magi_mounts/" in str(value) else
        ...
        if any(
            str(root).startswith("/Volumes/") and _osc_dir_listable_quick(str(root))
            for root in ordered_roots
        ):
            ordered_roots = [
                root for root in ordered_roots if "/.magi_mounts/" not in str(root)
            ]
        fallback_expected = ""
        for root in ordered_roots:
            ...
            if _osc_dir_listable_quick(expected):
                return expected
            if (
                not require_existing
                and not fallback_expected
                and _osc_dir_listable_quick(str(root))
            ):
                fallback_expected = expected
        if fallback_expected:
            return fallback_expected
    except Exception:
        _log.debug("silent-catch expected closed case folder", exc_info=True)
    if require_existing:
        return _osc_find_closed_case_folder(case_number, folder_name=folder_name)
    return ""
...
def _osc_active_case_folder_from_closed_path(closed_path: str) -> str:
    ...
    candidates = []
    configured_active_root = (
        os.environ.get("MAGI_ACTIVE_CASE_ROOT")
        or os.environ.get("MAGI_V3_CASE_ROOT")
        or ""
    ).strip()
    if configured_active_root:
        candidates.append(Path(configured_active_root).expanduser() / rel)
    candidates.extend([
...
def _osc_reconcile_case_folder_name(row: dict, folder_path: str, local_folder: str = "", *, update_db: bool = False) -> dict:
    """Keep the physical case folder and all stored paths aligned with metadata.

    The former implementation only *suggested* the metadata-derived path when
    the old folder still existed.  That left OSC, NAS and Drive describing the
    same case with different names and made later sync runs create duplicate
    directory trees.  A metadata update is now a single-owner transaction:
    rename the existing folder on the same volume, update DB references, and
    roll the filesystem rename back if the case-row update fails.
    """
    row = row or {}
    ...
    if local_folder and os.path.isdir(local_folder):
        ...
        suggested_local = candidate_local
    ...
    if (
        update_db
        and not target_local
        and local_folder
        and os.path.isdir(local_folder)
        and suggested_local
        and not os.path.exists(suggested_local)
        and _osc_is_safe_local_path(local_folder)
        and _osc_is_safe_local_path(os.path.dirname(suggested_local))
    ):
        rename_lock = acquire_case_file_operation_lock(
            owner=f"osc_case_folder_reconcile:{row.get('case_number') or row.get('id') or 'unknown'}"
        )
        if rename_lock.get("acquired"):
            try:
                # os.rename is atomic on the same mounted filesystem.  Refuse
                # cross-parent/cross-volume moves here; archive jobs own those.
                if os.path.dirname(os.path.realpath(local_folder)) != os.path.dirname(os.path.realpath(suggested_local)):
                    raise OSError(errno.EXDEV, "case folder target is not a sibling")
                os.rename(local_folder, suggested_local)
                renamed_from = local_folder
                target_local = suggested_local
                source = "filesystem_atomic_rename"
            except Exception as exc:
                rename_error = f"{type(exc).__name__}: {exc}"
        else:
            rename_error = "case_file_operation_lock_busy"
    ...
    db_update_safe = bool(target_local and os.path.isdir(target_local))
    if update_db and db_update_safe and row.get("id") and new_canonical and new_canonical != old_canonical:
        try:
            ...
            rename_error = rename_error or f"db_update_failed:{type(exc).__name__}: {exc}"
            _log.exception("case folder metadata path db update failed")
            # The case row is the authoritative commit point.  If it did not
            # move, restore the old filesystem name so a later retry starts
            # from a coherent state.
            if renamed_from and target_local and os.path.isdir(target_local) and not os.path.exists(renamed_from):
                try:
                    os.rename(target_local, renamed_from)
                    target_local = ""
                    new_canonical = old_canonical
                    db_update_safe = False
                    source = "rename_rolled_back"
                except Exception as rollback_exc:
                    rename_error += f"; rollback_failed:{type(rollback_exc).__name__}: {rollback_exc}"
    ...
    return {
        ...
        "suggested_folder_path": expected_canonical,
        "suggested_local_folder": suggested_local,
        "pending_repair": (not db_update_safe) or bool(update_db and not updated),
...
def _osc_effective_case_folder_for_row(row: dict, *, update_db: bool = False) -> dict:
    ...
    if is_closed:
        closed_local = _osc_expected_closed_case_folder_for_row(row)
        if closed_local and _osc_dir_listable_quick(closed_local):
            translate_local_path_to_canonical = _get_translate_local_path_to_canonical()
            canonical = _osc_norm_path(translate_local_path_to_canonical(closed_local) or closed_local)
            updated = False
            path_updates = {"updated": 0, "attempted": 0, "errors": []}
            if update_db and row.get("id") and canonical and canonical != norm:
                try:
                    _osc_exec(
                        "UPDATE cases SET folder_path=%s, updated_at=NOW() WHERE id=%s",
                        (canonical, row.get("id")),
                        fetch="none",
                    )
                    path_updates = _osc_replace_path_prefix_references(norm, canonical, exec_fn=_osc_exec)
                    updated = True
                except Exception:
                    _log.debug("silent-catch closed case canonical path upgrade", exc_info=True)
            return {
                "folder_path": canonical,
                "local_folder": closed_local,
                "source": "closed_archive",
                "updated": updated,
                "legacy_path_upgraded": bool(canonical and canonical != norm),
                "path_references": path_updates,
            }
        ...
        if is_closed and not _osc_dir_listable_quick(local):
            ...
            if reconciled.get("changed"):
                return {
                    ...
                    "suggested_folder_path": reconciled.get("suggested_folder_path") or "",
                    "suggested_local_folder": reconciled.get("suggested_local_folder") or "",
                    "pending_repair": bool(reconciled.get("pending_repair")),
...
def _osc_merge_existing_archive_source(src: str, dst: str) -> dict:
    ...
    conflicts: list[str] = []
    skipped: list[str] = []
    strict_hash = os.environ.get("MAGI_ARCHIVE_STRICT_HASH", "1").strip().lower() not in {"0", "false", "no", "off"}
            ...
            try:
                ...
                if os.path.exists(dst_file):
                    ...
                    try:
                        ...
                        return {
                            "ok": False,
                            "reason": "merge_unverified_conflict",
                            "error": f"{rel}: hash_unverified:{type(exc).__name__}: {exc}",
                            "source_signature": src_sig,
    ...
    for src_file, dst_file in verify_pairs:
        try:
            ...
            if not same_size or not same_hash:
                return {
                    "ok": False,
                    "reason": "merge_verify_failed",
                    "error": f"copy mismatch: {dst_file}",
                    "source_retained": True,
                    "quarantine_required": True,
                }
        except Exception as e:
...
def _osc_update_archived_case_folder(cid: str, folder_path: str, item: dict) -> None:
    ...
    phase = case_lifecycle_phase(item or {})
    if phase is not CaseLifecyclePhase.OPEN:
        case_status = "已結案" if phase is CaseLifecyclePhase.CLOSED else "結案中"
        _osc_exec(
            """
            UPDATE cases
            SET folder_path=%s,
            ...
            WHERE id=%s
            """,
            (canonical_folder_path, case_status, "auto_archive_closed_case", cid),
            fetch="none",
        )
    else:
...
def osc_case_open_folder_api(row_id):
    ...
    resolved = _osc_effective_case_folder_for_row(row, update_db=False)
...
def osc_case_create_folder_api(row_id):
    ...
    root_selection = _osc_select_case_creation_root()
    if not root_selection.get("ok"):
        return jsonify(root_selection), 503
    ...
    guard = _osc_case_folder_creation_guard(
        {
            "case_number": case_number,
            "client_name": client_name,
            "case_type": case_type,
    ...
    if not guard.get("ok"):
        return jsonify(guard), 409 if guard.get("blocked") else 503
...
def osc_dashboard_api():
    ...
    accounting_summary = load_accounting_summary(
        _osc_exec,
        start_date=str(start_date),
        end_date=str(end_date),
    )
    accounting_totals = accounting_summary.get("totals") or {}
    working_cases_sql = _osc_status_scope_sql("working")
    final_closed_sql = _osc_status_scope_sql("closed")
    legal_aid_sql = _osc_legal_aid_case_sql()
    open_todo_sql = _osc_todo_open_status_sql("status")
    osc_todo_source = osc_todo_source_sql()
    calendar_todo_source = calendar_todo_source_sql()
    active_cases_row, _ = _osc_exec(
        f"""
        SELECT COUNT(*) AS c FROM cases
        WHERE {working_cases_sql}
        """,
        fetch="one",
    )
    ...
    closed_regular_row, _ = _osc_exec(
        f"""
        SELECT COUNT(*) AS c FROM cases
        WHERE {final_closed_sql}
          AND NOT {legal_aid_sql}
        """,
        fetch="one",
    )
    ...
    recent_cases, _ = _osc_exec(
        ...
        SELECT id, case_number, client_name, case_category, case_type, case_stage, case_reason,
               laf_case_no, application_no, court_name, court_case_no, court_division, legal_aid_status,
               lawyer, status, folder_path, updated_at, created_date
        FROM cases
        ORDER BY updated_at DESC, created_date DESC
        LIMIT 12
        """,
    ...
    return jsonify(
        {
            ...
            "stats": {
                ...
                "monthly_revenue": float(accounting_totals.get("income_total") or 0),
                "monthly_expense": float(accounting_totals.get("expense_total") or 0),
                "closed_regular": int((closed_regular_row or {}).get("c") or 0),
                "closed_legal_aid": int((closed_laf_row or {}).get("c") or 0),
            },
            "recent_cases": [_osc_case_api_row(r) for r in (recent_cases or [])],
            "pending_todos": [_osc_todo_api_row(r) for r in (pending_todos or [])],
            "pending_osc_todos": [_osc_todo_api_row(r) for r in (pending_osc_todos or [])],
            "pending_calendar_todos": [_osc_todo_api_row(r) for r in (pending_calendar_todos or [])],
...
def osc_drafts_meta_api():
    ...
    _default_provider = (
        "nvidia"
        if str(os.environ.get("NVIDIA_NIM_ENABLE", "0")).strip().lower()
        in {"1", "true", "yes", "on"}
        else "casper"
    )
    provider = _osc_get_setting_value("ai_draft_provider", _default_provider) or _default_provider
...
def osc_drafts_generate_api():
    TEXT_PRIMARY_MODEL = _get_text_primary_model()
    payload = request.get_json() or {}
    # Every actual OSC pleading workflow supplements local material with the
    # privacy-bounded Taiwan Law MCP research path.  The context builder sends
    # only doc type + abstract case reason, never parties or case facts.
    payload.setdefault("augment_legal_sources", True)
    ...
    provider = str(
        ...
        or _osc_get_setting_value("ai_draft_provider", _default_provider)
        ...
        return jsonify(
            {
                ...
                "legal_research": ctx.get("legal_research") or {},
                "case": ctx.get("case") or {},
                "selected_documents": ctx.get("selected_documents") or [],
                "selected_insights": ctx.get("selected_insights") or [],
                "citeable_insights": ctx.get("citeable_insights") or [],
                "citation_lock": ctx.get("citation_lock") or {},
    ...
    try:
        ...
        actual_model = ""
        if provider in {"nvidia", "nvidia_nim", "nim", "heavy"}:
            draft_text, actual_model = _osc_generate_draft_with_nvidia(prompt)
        ...
        def _assess_draft(candidate: str) -> tuple[dict, dict]:
            candidate_citations = validate_text_against_citation_lock(
                candidate,
                ctx.get("citation_lock") or {},
            )
            candidate_quality = quality_check({
                "mode": "draft",
                "strict_export": True,
                ...
                "plaintiff": ctx.get("plaintiff") or "",
                "defendant": ctx.get("defendant") or "",
                "case_facts": ctx.get("case_facts") or "",
                "grounding_text": prompt,
                "selected_documents": ctx.get("selected_documents") or [],
                "selected_insights": ctx.get("selected_insights") or [],
                "citation_validation": candidate_citations,
                "source_paths": [
            ...
            return candidate_citations, candidate_quality

        citation_validation, draft_quality = _assess_draft(cleaned)
        quality_repair_attempted = False
        quality_repair_passed = False
        if actual_provider == "nvidia" and not (
            citation_validation.get("ok") and draft_quality.get("pass")
        ):
            quality_repair_attempted = True
            issue_lines = [
                str(item.get("message") or item.get("code") or "").strip()
                for item in (draft_quality.get("issues") or [])
            ...
            repair_prompt = (
                ...
                + "上一版未通過正式書狀門檻："
                + "；".join(issue_lines[:8])
                + f"\n第一行必須是「{ctx.get('doc_type') or '書狀'}」。"
                + "僅可使用原輸入資料；不得為了通過檢查而刪除真正資料缺口，"
                + "不得新增法條、裁判、人名、地址、日期、金額或證據。"
                + "\n請重新輸出完整書狀，不要解釋修改過程。"
                + "\n\n【上一版草稿】\n"
                + cleaned[:16000]
            )
            repaired_text, repaired_model = _osc_generate_draft_with_nvidia(repair_prompt)
            repaired_cleaned = _osc_clean_draft_output(repaired_text)
            repaired_citations, repaired_quality = _assess_draft(repaired_cleaned)
            if repaired_citations.get("ok") and repaired_quality.get("pass"):
                cleaned = repaired_cleaned
                citation_validation = repaired_citations
                draft_quality = repaired_quality
                actual_model = repaired_model or actual_model
                quality_repair_passed = True
        if not citation_validation.get("ok"):
            ctx.setdefault("warnings", []).append("draft_contains_out_of_bundle_citation")
        export_allowed = bool(citation_validation.get("ok") and draft_quality.get("pass"))
        if not draft_quality.get("pass"):
            ctx.setdefault("warnings", []).append("draft_quality_gate_failed")
        _osc_log_activity(
            "draft:generate",
            "drafts",
        ...
        return jsonify(
            {
                ...
                "quality_repair_passed": quality_repair_passed,
...
def osc_drafts_export_api():
    ...
    ctx = _osc_build_draft_context(payload)
    citation_lock = ctx.get("citation_lock") or {}
    citation_validation = validate_text_against_citation_lock(text, citation_lock)
    draft_quality = quality_check(
        {
            "mode": "draft",
            "strict_export": True,
            "draft_text": text,
            "case_number": ctx.get("case_number") or payload.get("case_number") or "",
            "reason": ctx.get("reason") or payload.get("reason") or "",
            "doc_type": ctx.get("doc_type") or payload.get("doc_type") or "",
            "court_name": ctx.get("court_name") or payload.get("court_name") or "",
            "plaintiff": ctx.get("plaintiff") or payload.get("plaintiff") or "",
            "defendant": ctx.get("defendant") or payload.get("defendant") or "",
            "case_facts": ctx.get("case_facts") or payload.get("case_facts") or payload.get("facts") or "",
            "grounding_text": ctx.get("prompt") or "",
            "selected_documents": ctx.get("selected_documents") or [],
            "selected_insights": ctx.get("selected_insights") or [],
            "citation_validation": citation_validation,
            "source_paths": [
    ...
    if not citation_validation.get("ok") or not draft_quality.get("pass"):
        return jsonify(
            {
                "ok": False,
                ...
                "citation_validation": citation_validation,
                "quality_check": draft_quality,
                "detail": "書狀仍有未核對引用、錯誤欄位或不完整結構；請依品質檢查修正後再匯出。",
            }
        ), 422
...
def _osc_register_reused_document(target_case: dict, result: dict, payload: dict) -> list[str]:
    ...
    idempotency_key = hashlib.sha256(
        f"{tenant_id}\0{case_id or case_number}\0{output_path}".encode("utf-8")
    ).hexdigest()
    try:
        _osc_exec(
            """
...
def _osc_copy_with_system_cp(
    ...
    *,
    ...
    if expected_size is None and not _osc_is_network_nas_path(local_file):
    ...
    try:
        for command in commands:
            ...
            if deadline is not None:
                ...
                if remaining <= 0.1:
                    return False
                command_timeout = max(0.1, min(command_timeout, remaining))
...
def _osc_stage_file_with_helper(local_file: str, *, timeout: float | None = None) -> str:
    ...
    try:
        with urlopen(req, timeout=timeout or _OSC_HELPER_STAGE_TIMEOUT) as response:
...
def _osc_stat_with_retry(local_file: str, *, max_attempts: int | None = None):
    ...
    for attempt in range(max_attempts):
        try:
            if _osc_is_network_nas_path(local_file):
                return _osc_stat_quick(local_file, timeout=1.5)
...
def _osc_is_dataless_file(path: str) -> bool:
    try:
        return bool(getattr(_osc_stat_quick(path, timeout=1.5), "st_flags", 0) & 0x40000000)
    except Exception:
        return False


def _osc_file_download_candidate_priority(path: str) -> tuple[int, int, int, str]:
    """Prefer hydrated local sync copies, then live mounts, never placeholders."""
    normalized = str(path or "").replace("\\", "/")
    dataless = _osc_is_dataless_file(path)
    hydrated_synology = (
        not dataless
        and (
    ...
    return (
        1 if dataless else 0,
        0 if hydrated_synology else 1,
        0 if normalized.startswith("/Volumes/") else 1,
        normalized.lower(),
    )
...
@_osc_stage_bulkhead
def _osc_stage_file_with_retry(
    local_file: str,
    *,
    ...
    deadline_seconds = max(
        1.0,
        float(os.environ.get("PAPERCLIP_FILE_STAGE_DEADLINE_SEC", "120") or "120"),
    )
    if deadline is None:
    ...
    for attempt in range(max_attempts):
        ...
        try:
            remaining = deadline - time.monotonic()
            if remaining <= 0.1:
                raise TimeoutError(f"file staging deadline exceeded: {deadline_seconds:.1f}s")
            fd, tmp_path = tempfile.mkstemp(prefix="osc-download-", suffix=suffix, dir=tmp_dir)
            if is_network_path:
                os.close(fd)
                try:
                    staged = _osc_stage_file_with_helper(
                        local_file,
                        timeout=max(0.1, min(float(_OSC_HELPER_STAGE_TIMEOUT), remaining)),
                    )
                except OSError as helper_err:
                    last_exc = helper_err
                    # The helper is an optimization and isolation boundary, not
                    # a single point of failure.  A launch/reboot gap used to
                    # make this branch retry the same unavailable localhost
                    # service and never reach the already implemented cp/ditto
                    # fallback below.
                    if _osc_copy_with_system_cp(
                        local_file,
                        tmp_path,
                    ...
                    if attempt < max_attempts - 1:
                        ...
                        if remaining <= 0.1:
                            raise TimeoutError(f"file staging deadline exceeded: {deadline_seconds:.1f}s")
                        time.sleep(min(delay, max(0.0, remaining - 0.1)))
                        continue
                    raise
                try:
                    os.remove(tmp_path)
            ...
            if is_network_path and prefer_system_copy:
                os.close(fd)
                if _osc_copy_with_system_cp(local_file, tmp_path, expected_size=expected_size, deadline=deadline):
            ...
            try:
                ...
                if e.errno in (11, 35) and _osc_copy_with_system_cp(
                    local_file,
                    tmp_path,
                    expected_size=expected_size,
                    deadline=deadline,
            ...
            if e.errno in (11, 35) and attempt < max_attempts - 1:
                ...
                remaining = deadline - time.monotonic()
                if remaining <= 0.1:
                    raise TimeoutError(f"file staging deadline exceeded: {deadline_seconds:.1f}s") from e
                time.sleep(min(delay, max(0.0, remaining - 0.1)))
                continue
            raise
        except Exception as e:
...
def _osc_stream_staged_file(staged_file: str, *, download_name: str, mime: str | None, inline: bool):
    ...
    if range_header.startswith("bytes="):
        ...
        try:
            if size <= 0 or "," in spec:
                raise ValueError("empty or multipart ranges are unsupported")
...
def osc_file_content_api():
    ...
    if not any(
        ...
        return _osc_download_error_response("檔案路徑不在允許範圍內。", 403)
    def _existing_file_candidates(candidate_paths: list[str]) -> list[str]:
        found: list[str] = []
        for cand in candidate_paths:
            try:
                real = os.path.realpath(cand)
                if _osc_is_safe_local_path(real) and _osc_isfile_quick(real):
    ...
    def _existing_dir_candidates(candidate_paths: list[str]) -> list[str]:
        ...
        for cand in candidate_paths:
            try:
                real = os.path.realpath(cand)
                if _osc_is_safe_local_path(real) and _osc_isdir_quick(real):
                    if real not in found:
                        found.append(real)
            except Exception:
                continue
    ...
    existing_candidates.sort(key=_osc_file_download_candidate_priority)
    ...
    if request.method == "HEAD":
        ...
        _osc_audit_file_event(
            "file.content.head",
            chosen,
            metadata={"inline": inline, "size": int(st.st_size), "mime": mime or "application/octet-stream"},
        )
        return resp
    ...
    chosen: str = ""
    staged_file: str = ""
    content_stage_deadline = time.monotonic() + max(
        1.0,
        float(os.environ.get("PAPERCLIP_FILE_STAGE_DEADLINE_SEC", "120") or "120"),
    )
    for local_file in existing_candidates:
        try:
            if content_stage_deadline - time.monotonic() <= 0.1:
                raise TimeoutError("file content staging deadline exceeded")
            staged_file = _osc_stage_file_with_retry(
                local_file,
                deadline=content_stage_deadline,
            )
            chosen = local_file
            break
        except OSError as e:
    ...
    try:
        ...
        _osc_audit_file_event(
            "file.content.get",
            chosen,
            metadata={"inline": inline, "mime": mime or "application/octet-stream"},
        )
        return resp
    except Exception as e:
        if staged_file:
...
def osc_laf_case_status_api(row_id):
    ...
    if requires_closed_storage({"status": next_case_status, "legal_aid_status": target}):
        resp["archive"] = _osc_auto_archive_closed_case(row_id)
...
def _osc_hearing_candidate_payload(payload: dict) -> dict:
    start = str(payload.get("start_date") or payload.get("start") or "").strip()
    end = str(payload.get("end_date") or payload.get("end") or "").strip()
    title = str(payload.get("title") or payload.get("summary") or "開庭").strip()
    if not start:
        raise ValueError("start_date required")
    if not end:
        parsed = _osc_parse_dt(start)
        if parsed:
            end = (parsed + timedelta(hours=1)).isoformat(sep=" ")
        else:
            raise ValueError("end_date required")
    return {
        **payload,
        "title": title,
        "start_date": start,
        "end_date": end,
        "case_number": str(payload.get("case_number") or "").strip(),
        "created_date": str(payload.get("created_date") or payload.get("created_at") or "").strip(),
        "source_kind": str(payload.get("source_kind") or "osc_hearing_conflict_ui").strip(),
    }


def _osc_enqueue_hearing_candidate(candidate: dict) -> tuple[bool, str]:
    """Classify and admit one persisted schedule without false queue success."""

    try:
        from api.osc.hearing_conflict_runtime import enqueue_candidate
        from api.osc.hearing_conflicts import classify_schedule

        if classify_schedule(candidate) != "hearing":
            return False, "not_a_confirmed_hearing"
        admission = enqueue_candidate(candidate, auto_generate=True, notify=True)
        reason = getattr(admission, "reason", "")
        return bool(getattr(admission, "accepted", False)), reason if isinstance(reason, str) else "accepted"
    except Exception:
        _log.warning("calendar hearing-conflict enqueue failed", exc_info=True)
        return False, "enqueue_error"


def _osc_todo_hearing_candidate(row: dict) -> dict:
    day = str(row.get("todo_date") or "").strip()
    clock = str(row.get("todo_time") or "").strip()
    if not day:
        return {}
    start = f"{day} {clock}".strip()
    parsed = _osc_parse_dt(start)
    if not parsed:
        return {}
    ...
    return {
        **row,
        "event_id": f"todo:{row.get('id') or ''}",
        "title": str(row.get("todo_type") or row.get("description") or "行程").strip(),
        "start_date": parsed.isoformat(sep=" "),
        "end_date": (parsed + (timedelta(days=1) if all_day else timedelta(hours=1))).isoformat(sep=" "),
        "created_date": str(row.get("created_date") or datetime.now().isoformat(sep=" ")),
        "source_kind": "case_todos",
        "is_all_day": all_day,
    }


@osc_bp.route("/api/osc/hearing-conflicts", methods=["GET"])
@login_required
def osc_hearing_conflicts_api():
    """List upcoming conflict decisions without producing files or alerts."""

    from api.osc.hearing_conflict_runtime import load_existing_schedules
    from api.osc.hearing_conflicts import scan_all_conflict_decisions

    now = datetime.now()
    start = _osc_parse_dt(request.args.get("start_date") or "") or (now - timedelta(days=30))
    end = _osc_parse_dt(request.args.get("end_date") or "") or (now + timedelta(days=365))
    case_number = str(request.args.get("case_number") or "").strip()
    values = load_existing_schedules(start, end, limit=500)
    schedules, decisions, truncated = scan_all_conflict_decisions(
        values,
        max_decisions=250,
        candidate_case_number=case_number,
    )
    candidates = []
    for item in schedules[:500]:
        if item.kind != "hearing" or (case_number and item.case_number != case_number):
            continue
        candidates.append({
    ...
    filtered_decisions = [
        ...
        if not case_number or decision.candidate.case_number == case_number
    ]
    return jsonify({
        "ok": True,
        "candidate_count": len(candidates),
        "conflict_count": len(filtered_decisions),
        "conflict_count_is_lower_bound": truncated,
        "truncated": truncated,
        "candidates": candidates,
        "items": filtered_decisions,
        "policy": {
            "wait_confirm_excluded": True,
            "older_hearing": "generate_leave_request",
            "older_non_hearing": "notify_only",
            "manual_generation": True,
        },
    })


@osc_bp.route("/api/osc/hearing-conflicts/check", methods=["POST"])
@login_required
def osc_hearing_conflicts_check_api():
    from api.osc.hearing_conflict_runtime import check_candidate
    from api.osc.hearing_conflicts import classify_schedule

    payload = request.get_json(silent=True) or {}
    raw_candidate = payload.get("candidate") if isinstance(payload, dict) and "candidate" in payload else payload
    if not isinstance(raw_candidate, dict):
        return jsonify({"ok": False, "error": "candidate JSON object required"}), 400
    try:
        candidate = _osc_hearing_candidate_payload(dict(raw_candidate))
    ...
    if classify_schedule(candidate) != "hearing":
        return jsonify({
            "ok": True,
            "excluded": True,
            "reason": "not_confirmed_hearing" if "待確認" in str(candidate.get("title") or "") else "not_a_hearing",
            "conflict_count": 0,
            "items": [],
        })
...
@osc_bp.route("/api/osc/hearing-conflicts/generate", methods=["POST"])
@login_required
def osc_hearing_conflicts_generate_api():
    from api.osc.hearing_conflict_runtime import check_candidate, generate_from_decision, generate_manual, load_case
    ...
    if not case:
        return jsonify({"ok": False, "error": "target_case_not_found"}), 404
    try:
        if mode == "manual":
            target_start = _osc_parse_dt(payload.get("target_start") or payload.get("start_date") or "")
            prior_start = _osc_parse_dt(payload.get("prior_start") or "")
            if not target_start or not prior_start:
                raise ValueError("target_start and prior_start required")
            result = generate_manual(
                case,
                target_start=target_start,
                prior_start=prior_start,
                prior_court_name=str(payload.get("prior_court_name") or "").strip(),
                prior_hearing_label=str(payload.get("prior_hearing_label") or "開庭").strip(),
                prior_court_case_no=str(payload.get("prior_court_case_no") or "").strip(),
                lawyer_name=str(payload.get("lawyer_name") or "").strip(),
                party_role=str(payload.get("party_role") or "當事人").strip(),
                target_hearing_label=str(payload.get("target_hearing_label") or "開庭").strip(),
                conflict_statement=str(payload.get("conflict_statement") or "").strip(),
            )
        elif mode == "automatic_conflict":
            raw_candidate = payload.get("candidate")
            existing_hint = payload.get("existing")
            if not isinstance(raw_candidate, dict) or not isinstance(existing_hint, dict):
                raise ValueError("candidate and existing JSON objects required")
            candidate = _osc_hearing_candidate_payload(dict(raw_candidate))
            target_case_number = str(case.get("case_number") or "").strip()
            candidate_case_number = str(candidate.get("case_number") or "").strip()
            if candidate_case_number and candidate_case_number != target_case_number:
                raise ValueError("candidate_case_does_not_match_target_case")
            candidate["case_number"] = target_case_number
            candidate["hearing_type"] = str(
                payload.get("target_hearing_label") or candidate.get("hearing_type") or "開庭"
            ).strip()
            server_decisions = check_candidate(candidate)
            hint_id = str(existing_hint.get("event_id") or existing_hint.get("id") or "").strip()
            hint_source = str(existing_hint.get("source") or existing_hint.get("source_kind") or "").strip()
            hint_start = str(existing_hint.get("start") or existing_hint.get("start_date") or "").replace("T", " ").strip()
            hint_case = str(existing_hint.get("case_number") or "").strip()
            decision = None
            for item in server_decisions:
                if item.action != "generate_leave_request":
                    continue
                if hint_id:
                    if item.existing.event_id != hint_id:
                        continue
                    if hint_source and item.existing.source != hint_source:
                        continue
                else:
                    if hint_case and item.existing.case_number != hint_case:
                        continue
                    if hint_start and item.existing.start.isoformat(sep=" ") != hint_start:
                        continue
                decision = item
                break
            if not decision:
                raise ValueError("selected_schedule_is_not_an_older_hearing_conflict")
            result = generate_from_decision(
                decision,
                lawyer_name=str(payload.get("lawyer_name") or "").strip(),
                party_role=str(payload.get("party_role") or "當事人").strip(),
            )
    ...
    if not document_id:
        return jsonify({"ok": False, "error": "generated_document_not_registered"}), 500
    result["download_url"] = f"/api/osc/hearing-conflicts/download?document_id={quote(document_id)}"
    result.pop("path", None)
    _osc_log_activity(
        "hearing_leave_request_generated",
        "case",
        str(case.get("id") or case.get("case_number") or ""),
        {"mode": mode, "file_name": result.get("file_name"), "created": result.get("created")},
    )
    return jsonify(result)


@osc_bp.route("/api/osc/hearing-conflicts/download", methods=["GET"])
@login_required
def osc_hearing_conflicts_download_api():
    from api.saas_schema import tenant_id_from_env

    document_id = str(request.args.get("document_id") or "").strip()
    if not document_id:
        return jsonify({"ok": False, "error": "document_id required"}), 400
    row, _ = _osc_exec(
        "SELECT id, file_name, file_path FROM case_documents WHERE id=%s AND document_type=%s AND tenant_id=%s LIMIT 1",
        (document_id, "聲請改期／請假狀草稿", tenant_id_from_env()),
        fetch="one",
    )
    raw = str((row or {}).get("file_path") or "").strip()
    local = _osc_resolve_existing_local_path(raw, prefer_dir=False)
    if not local or Path(local).suffix.lower() != ".docx":
        return jsonify({"ok": False, "error": "file_not_found_or_not_allowed"}), 404
    return send_file(
        local,
        as_attachment=True,
        download_name=str((row or {}).get("file_name") or Path(local).name),
    )
...
def osc_calendar_events_api():
    if request.method == "GET":
        ...
        limit = max(1, min(1000, int(request.args.get("limit") or "300")))
        include_todos = str(request.args.get("include_todos") or "1").strip().lower() not in {"0", "false", "no", "off"}
        sql = (
            ...
            "'calendar_events' AS source_table, 'calendar_events' AS source_kind, '行事曆事件' AS source_label "
        ...
        for row in rows or []:
            item = dict(row)
            item.setdefault("source_table", "calendar_events")
            item.setdefault("source_kind", "calendar_events")
            item.setdefault("source_label", "行事曆事件")
            items.append(item)
        source_counts = {"calendar_events": len(items), "gcal_import": 0, "calendar_todo": 0}
        if include_todos:
            todo_sql = (
                "SELECT id, case_number, client_name, todo_type, todo_date, todo_time, description, status, source_file, created_date, completed_date "
                "FROM case_todos WHERE "
                f"{_osc_todo_open_status_sql('status')} AND {calendar_todo_source_sql()} "
            )
            todo_params: list[str | int] = []
            if case_number:
                todo_sql += "AND (case_number=%s OR description LIKE %s) "
                todo_params.extend([case_number, f"%{case_number}%"])
            if start_date:
                todo_sql += "AND todo_date >= %s "
                todo_params.append(start_date[:10])
            if end_date:
                todo_sql += "AND todo_date <= %s "
                todo_params.append(end_date[:10])
            if q:
                like = f"%{q}%"
                todo_sql += "AND (todo_type LIKE %s OR description LIKE %s OR case_number LIKE %s OR client_name LIKE %s) "
                todo_params.extend([like, like, like, like])
            todo_sql += "ORDER BY todo_date DESC, COALESCE(todo_time, '23:59') DESC, id DESC LIMIT %s"
            todo_params.append(limit)
            todo_rows, _ = _osc_exec(todo_sql, tuple(todo_params), fetch="all")
            for row in todo_rows or []:
                event = calendar_todo_to_event(_osc_todo_api_row(row) or row)
                source_counts[event.get("source_kind") or "calendar_todo"] = source_counts.get(event.get("source_kind") or "calendar_todo", 0) + 1
                items.append(event)
        items.sort(key=lambda x: (str(x.get("start_date") or ""), int(x.get("id") or 0)), reverse=True)
        return jsonify({
            "ok": True,
            "items": items[:limit],
    ...
    candidate = {
        **payload,
        ...
        "created_date": datetime.now().isoformat(sep=" "),
        "source_kind": "calendar_events",
        "is_all_day": is_all_day,
    }
    conflict_queued, conflict_queue_reason = _osc_enqueue_hearing_candidate(candidate)
    return jsonify({
        "ok": True,
        "id": result.get("lastrowid"),
        "event_id": event_id,
        "result": result,
        "hearing_conflict_check_queued": conflict_queued,
        "hearing_conflict_queue_reason": conflict_queue_reason,
    })
...
def osc_calendar_event_detail_api(row_id):
    ...
    schedule_changed = bool(
        set(payload)
        & {"title", "summary", "description", "start_date", "end_date", "location", "is_all_day", "case_number"}
    )
    if not schedule_changed:
        return jsonify({
            "ok": True,
            "result": result,
            "hearing_conflict_check_queued": False,
            "hearing_conflict_queue_reason": "no_schedule_change",
        })
    updated, _ = _osc_exec("SELECT * FROM calendar_events WHERE id=%s", (row_id,), fetch="one")
    queued, queue_reason = _osc_enqueue_hearing_candidate({
        **dict(updated or {}),
        "source_kind": "calendar_events",
    }) if updated else (False, "updated_event_not_found")
    return jsonify({
        "ok": True,
        "result": result,
        "hearing_conflict_check_queued": queued,
        "hearing_conflict_queue_reason": queue_reason,
    })
...
def osc_meetings_api():
    ...
    todo_id = (result or {}).get("lastrowid")
    todo_candidate = _osc_todo_hearing_candidate({
        "id": todo_id,
        **dict(zip(cols, vals)),
        "created_date": datetime.now().isoformat(sep=" "),
    })
    queued, queue_reason = _osc_enqueue_hearing_candidate(todo_candidate) if todo_candidate else (False, "todo_has_no_schedule")
    return jsonify({
        "ok": True,
        "result": result,
        "hearing_conflict_check_queued": queued,
...
def osc_todos_api():
    if request.method == "GET":
        ...
        source_clause = ""
        if source == "osc":
            source_clause = f" AND {osc_todo_source_sql()}"
        elif source in {"gcal", "calendar"}:
            source_clause = f" AND {calendar_todo_source_sql()}"
        if q:
            like = f"%{q}%"
            rows, _ = _osc_exec(
                f"""
        ...
        return jsonify({"ok": True, "items": [_osc_todo_api_row(r) for r in (rows or [])]})
    payload = request.get_json() or {}
    case_number = (payload.get("case_number") or "").strip()
...
def osc_todo_detail_api(row_id):
    ...
    if _osc_is_todo_done_status(payload.get("status") or ""):
        completed_row, _ = _osc_exec(
            "SELECT case_number FROM case_todos WHERE id=%s", (row_id,), fetch="one"
        )
        completed_case = str((completed_row or {}).get("case_number") or "").strip()
        if re.fullmatch(r"20\d{2}-\d{4}", completed_case):
            try:
                from magi_v3.business_events import emit_case_evidence_event

                emit_case_evidence_event(
                    domain="osc",
                    case_number=completed_case,
                    source=f"todo:{row_id}:{payload.get('status') or ''}",
                    evidence_kind="todo_completed",
                )
            except Exception:
                logging.getLogger(__name__).warning(
                    "case evidence event could not be queued", exc_info=True
                )
    schedule_changed = bool(
        set(payload)
        & {"case_number", "todo_type", "todo_date", "todo_time", "description", "status", "source_file"}
    )
    if not schedule_changed:
        return jsonify({
            "ok": True,
    ...
    updated, _ = _osc_exec("SELECT * FROM case_todos WHERE id=%s", (row_id,), fetch="one")
    candidate = _osc_todo_hearing_candidate(dict(updated or {})) if updated else {}
    queued, queue_reason = _osc_enqueue_hearing_candidate(candidate) if candidate else (False, "todo_has_no_schedule")
    return jsonify({
        "ok": True,
        "result": result,
        "hearing_conflict_check_queued": queued,
...
def osc_insight_detail_api(insight_id):
    ...
    if sid.startswith("cj-"):
        ...
        if not row:
            return jsonify({"ok": False, "error": "not found"}), 404
        if not _osc_court_summary_displayable(row):
            return jsonify({"ok": False, "error": "not found"}), 404
        return jsonify({"ok": True, "item": row})
...
def osc_forms_export_api():
    ...
    if form_type == "legal_attest":
        ...
        export_dir = str(get_exports_dir())
...
_LABOR_LAW_IMPORT_LOCK = threading.RLock()


def _load_labor_law_action_module():
    """Load the hyphenated labor-law skill with a valid import-module binding."""
    import importlib.util
    ...
    module_name = "labor_law_action"
    with _LABOR_LAW_IMPORT_LOCK:
        spec = importlib.util.spec_from_file_location(module_name, str(skill_path))
        if spec is None or spec.loader is None:
            raise ImportError(f"無法建立 skill import spec：{skill_path}")
...
def osc_labor_law_calc():
    ...
    """
    skill_dir = str(
        Path(__file__).resolve().parents[2] / "skills" / "labor-law-calculator"
    )
    if skill_dir not in sys.path:
        sys.path.insert(0, skill_dir)

    try:
        mod = _load_labor_law_action_module()
    except Exception as e:
...
_BACKUP_TABLE_SET = frozenset(_BACKUP_TABLES)
_BACKUP_IDENTIFIER_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")


def _osc_backup_identifier_is_safe(value: str) -> bool:
    return bool(_BACKUP_IDENTIFIER_RE.fullmatch(str(value or "")))


def _osc_backup_safe_row(table: str, row: dict) -> tuple[list[str], str]:
    if table not in _BACKUP_TABLE_SET or not _osc_backup_identifier_is_safe(table):
        return [], f"disallowed backup table: {table}"
    cols = list(row.keys())
    unsafe_cols = [str(col) for col in cols if not _osc_backup_identifier_is_safe(str(col))]
    if unsafe_cols:
        return [], f"{table}: disallowed backup column(s): {', '.join(unsafe_cols[:5])}"
...
def _osc_backup_digest(payload: dict) -> str:
    canonical = {
        key: payload.get(key)
        for key in ("version", "created_at", "label", "tables", "table_counts")
    }
    raw = json.dumps(
        canonical,
        ensure_ascii=False,
...
def _osc_validate_backup_payload(payload: dict) -> tuple[bool, str]:
    if not isinstance(payload, dict) or not isinstance(payload.get("tables"), dict):
        return False, "invalid_backup_schema"
    try:
        version = int(payload.get("version") or 1)
    except Exception:
        return False, "invalid_backup_version"
    if version < 2:
        return True, "legacy_unverified"
    integrity = payload.get("integrity")
    if not isinstance(integrity, dict) or integrity.get("algorithm") != "sha256":
        return False, "missing_backup_integrity"
    expected = str(integrity.get("digest") or "").strip().lower()
    if not expected or expected != _osc_backup_digest(payload):
        return False, "backup_checksum_mismatch"
    return True, "verified"


def _osc_atomic_write_backup(path: Path, payload: dict) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.debug("backup temporary file cleanup failed: %s", tmp_name, exc_info=True)
        raise
...
def _osc_create_backup(label: str = "manual") -> dict:
    ...
    payload["integrity"] = {"algorithm": "sha256", "digest": _osc_backup_digest(payload)}

    backup_path = _osc_backup_dir() / filename
    _osc_atomic_write_backup(backup_path, payload)
    ...
    return {
        ...
        "table_counts": table_counts,
        "integrity": "verified",
        "sha256": payload["integrity"]["digest"],
    }
...
def _osc_parse_backup_meta(p: Path) -> dict:
    ...
    try:
        ...
        valid, integrity_status = _osc_validate_backup_payload(raw)
        return {
            "filename": p.name,
...
def osc_backup_restore(filename):
    ...
    valid, integrity_status = _osc_validate_backup_payload(payload)
    if not valid:
        return jsonify({"ok": False, "error": integrity_status}), 422
    allow_unverified = bool(data.get("allow_unverified"))
    if integrity_status == "legacy_unverified" and not dry_run and not allow_unverified:
        return jsonify({
            "ok": False,
            "error": "legacy_backup_requires_allow_unverified",
        }), 409
    ...
    for table, rows in tables.items():
        table = str(table or "")
        if table not in _BACKUP_TABLE_SET or not _osc_backup_identifier_is_safe(table):
            errors.append(f"disallowed backup table: {table}")
            continue
        if not rows:
            continue
        ...
        for row in rows:
            if not isinstance(row, dict):
                continue
            cols, identifier_error = _osc_backup_safe_row(table, row)
            if identifier_error:
    ...
    return jsonify(response_payload), (200 if not errors else 500)
