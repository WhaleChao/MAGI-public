...
_MAINTENANCE_MANUAL_ASSETS = {
    "html": ("MAGI_V3_維修百科全書_rc643.html", "text/html; charset=utf-8"),
    "pdf": ("MAGI_V3_維修百科全書_rc643.pdf", "application/pdf"),
    "markdown": ("MAGI_V3_維修百科全書_rc643.md", "text/markdown; charset=utf-8"),
    "source-index.json": ("MAGI_V3_原始碼索引_rc643.json", "application/json"),
}
...
def _runtime_dir() -> Path:
    return Path(
        os.environ.get("MAGI_RUNTIME_DIR", "").strip() or _MAGI_ROOT / ".runtime"
    ).expanduser()


def _worldmonitor_report_dir() -> Path:
    configured = os.environ.get("MAGI_WORLDMONITOR_REPORT_DIR", "").strip()
    if configured:
        return Path(configured).expanduser()
    mutable_root = os.environ.get("MAGI_MUTABLE_STATIC_DIR", "").strip()
    if mutable_root:
        return Path(mutable_root).expanduser() / "worldmonitor_reports"
    return _WORLDMONITOR_REPORT_DIR
...
def _tailscale_cli() -> str:
    """Resolve the CLI explicitly because launchd has a minimal PATH."""
...
def _load_tailscale_serve_url() -> str:
    try:
        result = subprocess.run(
            [_tailscale_cli(), "serve", "status", "--json"],
            capture_output=True,
            text=True,
            timeout=2,
        )
...
def _build_mobile_app_config() -> dict:
    ...
    routes = [
        ...
        {"label": "案件與文件", "path": "/osc", "kind": "core"},
        {"label": "待辦與期限", "path": "/osc?tab=todos", "kind": "core"},
        {"label": "行程", "path": "/osc?tab=calendar", "kind": "core"},
...
def _iter_worldmonitor_reports(limit: int = 20) -> list[dict]:
    reports: list[dict] = []
    report_dir = _worldmonitor_report_dir()
...
def _load_live_health_snapshot() -> dict:
    try:
        ...
        logging.getLogger(__name__).debug(
            "live health snapshot requested outside Flask application context",
            exc_info=True,
        )
    url = os.environ.get("MAGI_INTERNAL_HEALTH_URL") or "http://127.0.0.1:5002/readyz"
    try:
...
def _load_live_scheduler_snapshot() -> dict:
    """Read the current scheduler owner instead of trusting a stale test report."""
    metadata = _read_json_file(
        _runtime_dir() / "locks" / "cron_scheduler_owner.lock.json",
        {},
    )
    try:
    ...
    try:
        ...
        logging.getLogger(__name__).debug(
            "scheduler PID is alive but owned by a protected process: %s", pid
        )
    except OSError:
        return {}
    ...
    try:
        ...
        jobs = list(load_bound_cron_jobs(_MAGI_ROOT).jobs)
    except Exception:
        logging.getLogger(__name__).warning(
            "bound cron inventory could not be loaded for dashboard snapshot",
            exc_info=True,
        )
        jobs = ()
    enabled_count = sum(
        1 for item in (jobs if isinstance(jobs, list) else [])
        if isinstance(item, dict) and item.get("enabled") is True
    )
    return {
        "alive": True,
        "pid": pid,
        "owner": str(metadata.get("owner") or "scheduler"),
...
def _build_beginner_dashboard() -> dict:
    ...
    magi_status_path = _mutable_static_dir() / "magi_status.json"
    ...
    live_status = str(live_health.get("status") or "").lower()
    live_operational = live_health.get("ok") is not False and live_status in {"ready", "live", "operational"}
    ...
    live_scheduler = _load_live_scheduler_snapshot()
    ...
    if live_scheduler.get("alive"):
        enabled_count = int(live_scheduler.get("enabled_count") or 0)
        count_detail = f"，{enabled_count} 個任務啟用" if enabled_count else ""
        schedule = {
            **schedule,
            "pass": True,
            "detail": (
                f"即時排程器運行中{count_detail} "
                f"(pid={int(live_scheduler.get('pid') or 0)})。"
            ),
        }
    elif live_health_ok and schedule.get("pass") is False and "discord_bot" in str(schedule.get("detail") or ""):
        schedule = {
            **schedule,
            "pass": True,
    ...
    evidence = [
        {
            ...
            "summary": f"目前 {live_status}" if live_health_ok else ("即時 health 顯示需檢查" if live_health else "無法讀取即時 health"),
...
def _load_research_dashboard() -> dict:
    rb_root = _runtime_dir() / "research_brief"
    ...
    legacy_crawler_path = _MAGI_ROOT / "_crawl_targets.json"
    runtime_dir = (os.environ.get("MAGI_RUNTIME_DIR") or "").strip()
    mutable_crawler_path = Path(
        (os.environ.get("MAGI_CRAWL_TARGETS_PATH") or "").strip()
        or (Path(runtime_dir) / "_crawl_targets.json" if runtime_dir else legacy_crawler_path)
    )
    crawler_path = mutable_crawler_path if mutable_crawler_path.exists() else legacy_crawler_path
    crawler_state = _read_json_file(crawler_path, {"targets": []})
