    ...
    *,
    ...
    def _collect_drive_sync_status() -> dict[str, Any]:
        ...
        def _public_drive_kind_statuses(candidates: list[dict[str, Any]]) -> dict[str, Any]:
            by_kind: dict[str, tuple[float, dict[str, Any]]] = {}
            try:
            ...
            for candidate in candidates:
                ...
                by_kind[kind] = (
                    rank,
                    {
                        "ok": candidate.get("ok"),
                        "status": status_text,
                        "action_required": bool(candidate.get("action_required")),
                        "pid": pid,
            ...
            return {kind: payload for kind, (_rank, payload) in sorted(by_kind.items())}
        ...
        def _healthy_live_or_newer_kind_status(worker_state: dict[str, Any], last_status: dict[str, Any]) -> Optional[dict[str, Any]]:
            ...
            for candidate in candidates:
                ...
                if candidate.get("ok") is False:
                    continue
                status_text = str(candidate.get("status") or "").strip().lower()
                if status_text in {"", "interrupted", "timeout", "partial_failure", "auth_required"}:
                    continue
                ts = _parse_finished_at(candidate)
                if ts <= 0:
                    continue
            ...
            if healthy_statuses:
                ...
                return {
                    "ok": True,
                    "status": str(newest.get("status") or "ok"),
