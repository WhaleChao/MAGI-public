...
class _LazyFitz:
    """Delay PyMuPDF's native allocation until a PDF route actually uses it."""

    _module = None

    def _load(self):
        if self._module is None:
            self._module = importlib.import_module("fitz")
        return self._module
...
def _upload_dir() -> Path:
    ...
    explicit = str(os.environ.get("MAGI_PDF_UPLOAD_DIR") or "").strip()
    if explicit:
        path = Path(explicit).expanduser()
        path.mkdir(parents=True, exist_ok=True)
        return path.resolve()
    agent_root = get_agent_dir()
    try:
        sealed_in_tree_fallback = (
            (root / "release-manifest.json").is_file()
            and agent_root.is_relative_to(root)
        )
    except (AttributeError, ValueError):
        sealed_in_tree_fallback = False
    if sealed_in_tree_fallback:
        # A self-host plan should bind MAGI_AGENT_DIR.  If it does not, uploads
        # may still be used safely for this session but must never write into
        # an immutable release.  Keep the fallback portable and non-secret.
        uid = str(getattr(os, "getuid", lambda: 0)())
        agent_root = Path(tempfile.gettempdir()) / "magi-pdf-session" / uid
    path = agent_root / "pdf_uploads"
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def _pdf_has_explicit_document_date(path: Path) -> bool:
    extract_doc_date, _extract_base_year = _load_headless_date_helpers()
    return extract_doc_date(path.name, str(path)) is not None
...
def _get_rapid_ocr_engine():
    ...
    try:
        ...
        _RAPID_OCR_ENGINE = get_shared_rapidocr()
        if _RAPID_OCR_ENGINE is None:
            raise RuntimeError("shared RapidOCR unavailable")
        return _RAPID_OCR_ENGINE
    except Exception:
...
def _ocr_pdf_page(page: fitz.Page) -> str:
    ...
    try:
        ...
        if _is_pdf_ocr_timeout(exc):
            log.warning("pdf calendar OCR fallback timed out: %s", str(exc)[:180])
        else:
            log.warning("pdf calendar OCR fallback failed", exc_info=True)
        return ""
...
def _is_pdf_calendar_source_dir_name(value: Any) -> bool:
    """Recognise court-calendar folders without depending on their number.

    General, legal-aid, and migrated case trees use different prefixes such as
    ``07_`` and ``09_`` for the same semantic folder.  Treating those prefixes
    as part of the schema silently skipped otherwise valid court notices.
    """
    raw = str(value or "").strip()
    if not raw:
        return False
    compact = re.sub(r"^[\s\d０-９._\-、()（）]+", "", raw)
    ...
    if any(
        ...
        return True
    return path_has_judgment_folder(raw)
...
_PDF_COURT_CONTAINER_DIR_NAMES = {
    "法院資料",
    "法院文件",
    "法院來文",
    "法院公文",
...
def _pdf_calendar_container_dir_name(value: Any) -> str:
    raw = re.sub(r"^[\s\d０-９._\-、()（）]+", "", str(value or "").strip())
...
def _extract_todos_from_pdf_text(path: Path, text: str) -> list[dict[str, Any]]:
    ...
    filename_doc_date_explicit = _pdf_has_explicit_document_date(path)
    year_prefix = r"(?:中華)?(?:民國)?"
    ...
    def _yearless_dt(month: str, day: str) -> datetime | None:
        ...
        try:
            ...
            if filename_doc_date_explicit and dt.date() < base.date() - timedelta(days=30) and filename_base_year == base.year:
...
def _scan_pdf_for_calendar(
    ...
    *,
    ...
    if any(_todo_semantic_family(t) == "deadline_challenge" for t in todos):
        conflicts = _verified_cross_case_document_conflicts(
            path,
            case_number=case_number,
            client_name=client_name,
        )
        if conflicts:
            blocked_todos = list(todos)
            identity_confirmation = {
                "status": "needs_human",
                "reason": "exact_document_already_owned_by_another_case",
                "current_case_number": case_number,
                "existing_case_numbers": sorted(
                    {
                ...
                "file_name": path.name,
                "blocked_todo_types": sorted(
                    {
                        str(row.get("type") or "").strip()
                        for row in blocked_todos
                        if str(row.get("type") or "").strip()
                    }
                ),
                "blocked_todo_count": len(blocked_todos),
            }
            # The same immutable court document cannot autonomously create a
            # second appeal/objection deadline under another internal case.
            # Keep the file in place for human review, but fail closed before
            # any case_todo or Google Calendar event is produced.
            todos = []
    ...
    if identity_confirmation:
        out["identity_confirmation"] = identity_confirmation
...
_DOCUMENT_IDENTITY_CONFIRM_TYPE = "文件歸檔確認"
_DOCUMENT_IDENTITY_CONFIRM_MARKER = "【MAGI文件身分確認】"
...
def _verified_cross_case_document_conflicts(
    path: Path,
    *,
    case_number: str,
    client_name: str,
) -> list[dict[str, Any]]:
    """Return only byte-identical final documents already owned by another case.

    Filename/client matching is merely the bounded candidate lookup.  A row is
    a conflict only when both source files are readable regular files with the
    same size and SHA-256.  This avoids merging generic same-name judgments and
    avoids guessing from a party name alone.
    """
    current_case = str(case_number or "").strip()
    current_client = str(client_name or "").strip()
    if not current_case or not current_client or not path.is_file():
        return []
    if not path_has_judgment_folder(str(path)):
        return []
    try:
        current_size = int(path.stat().st_size)
        current_digest = _document_sha256(path)
    except OSError:
        return []
    if current_size <= 0:
        return []

    placeholders = ",".join(["%s"] * len(_CHALLENGE_TODO_TYPES))
    rows, _ = _osc_exec(
        f"""
        SELECT t.id, t.case_number, t.client_name, t.todo_type, t.source_file,
               c.case_type, c.case_reason, c.court_name,
               COALESCE(c.court_case_number, c.court_case_no, '') AS court_case_number
        FROM case_todos t
        LEFT JOIN cases c ON c.case_number=t.case_number
        WHERE t.case_number<>%s
          AND t.client_name=%s
          AND t.todo_type IN ({placeholders})
          AND t.source_file IS NOT NULL AND t.source_file!=''
          AND COALESCE(t.status, '')<>'deleted'
        ORDER BY t.id DESC
        LIMIT 200
        """,
        (current_case, current_client, *_CHALLENGE_TODO_TYPES),
        fetch="all",
    )
    basename = path.name
    ...
    for row in rows or []:
        ...
        if not source or source.replace("\\", "/").rsplit("/", 1)[-1] != basename:
            continue
        other = Path(source).expanduser()
        try:
            if not other.is_absolute() or not other.is_file():
                continue
            if int(other.stat().st_size) != current_size:
                continue
            if _document_sha256(other) != current_digest:
                continue
        except OSError:
            continue
        key = (str((row or {}).get("case_number") or ""), str(other))
        if key in seen:
            continue
        seen.add(key)
        matches.append(
            {
                ...
                "case_type": str((row or {}).get("case_type") or ""),
                "case_reason": str((row or {}).get("case_reason") or ""),
                "court_name": str((row or {}).get("court_name") or ""),
                "court_case_number": str((row or {}).get("court_case_number") or ""),
                "file_name": basename,
...
def _insert_document_identity_confirmation(
    *,
    path: Path,
    case_number: str,
    client_name: str,
    ...
    existing_cases = ", ".join(identity_confirmation.get("existing_case_numbers") or []) or "其他案件"
    description = (
        f"{_DOCUMENT_IDENTITY_CONFIRM_MARKER}\n"
        f"同一份終局文件已在 {existing_cases} 建立期限；MAGI 已停止於本案自動建立上訴／抗告待辦。\n"
        f"文件：{path.name}\n"
        "請在 OSC 確認應歸屬的案件；確認前不會同步至 Google 日曆。"
    )
    source_file = str(path)
    existing, _ = _osc_exec(
        """
        SELECT id, status FROM case_todos
        WHERE case_number=%s AND todo_type=%s AND source_file=%s
          AND COALESCE(status, '')<>'deleted'
        ORDER BY id DESC LIMIT 1
        """,
        (case_number, _DOCUMENT_IDENTITY_CONFIRM_TYPE, source_file),
        fetch="one",
    )
    if existing:
        return {
            "inserted": 0,
            "skipped": 1,
            "id": int((existing or {}).get("id") or 0),
    ...
    result, _ = _osc_exec(
        ...
        VALUES (%s,%s,%s,NULL,NULL,%s,%s,'pending','',NULL)
        """,
        (
            str(case_number or "").strip(),
            str(client_name or "").strip(),
            _DOCUMENT_IDENTITY_CONFIRM_TYPE,
            description,
            source_file,
        ),
        fetch="none",
    ...
    return {
        "inserted": 1,
        "skipped": 0,
        "id": int((result or {}).get("lastrowid") or 0),
        "status": "pending",
    }
...
def _iter_indexed_case_pdf_candidates(
    ...
    *,
    ...
    case_inventory_limit = max(
        100,
        min(
            20_000,
            int(os.environ.get("OSC_PDF_CALENDAR_OPEN_CASE_INVENTORY_LIMIT", "10000") or "10000"),
        ),
    )
...
def _bounded_existing_pdf_sources(
    rows: Any, *, limit: int
) -> set[tuple[str, str]]:
    """Keep a single basename key per recent source under a strict row cap."""
    ...
    for index, row in enumerate(rows or []):
        ...
        source = str((row or {}).get("source_file") or "").strip()
        if source:
            bounded.add((str((row or {}).get("case_number") or ""), Path(source).name))
    return bounded
...
def _iter_all_case_pdf_targets(
    ...
    *,
    ...
    from api.case_path_mapper import _is_dir_accessible, local_case_path_candidates, preferred_case_roots

    def _calendar_source_roots() -> tuple[Path, ...]:
        """Return the only roots the unattended calendar scanner may read.

        This deliberately does not trust arbitrary historical ``folder_path``
        rows: an old user's SynologyDrive folder can otherwise create calendar
        candidates for a different office corpus.  Operations may explicitly
        pin roots with ``OSC_PDF_CALENDAR_ALLOWED_ROOTS``.
        """
        configured = [
            value.strip()
            for value in os.environ.get("OSC_PDF_CALENDAR_ALLOWED_ROOTS", "").split(os.pathsep)
            if value.strip()
        ]
        raw_roots = configured or preferred_case_roots(include_closed=True)
        roots: list[Path] = []
    ...
    allowed_source_roots = _calendar_source_roots()

    def _is_allowed_calendar_source(path: Path) -> bool:
        try:
            resolved = path.expanduser().resolve(strict=False)
        except OSError:
            return False
        return any(resolved == root or root in resolved.parents for root in allowed_source_roots)
    ...
    try:
        existing_source_limit = max(
            1000,
            min(
                100_000,
                int(
                    os.environ.get(
                        "OSC_PDF_CALENDAR_EXISTING_SOURCE_LIMIT", "25000"
                    )
                    or "25000"
                ),
            ),
        ...
        source_rows, _ = _osc_exec(
            ...
            LIMIT %s
            """,
            (existing_source_limit,),
            fetch="all",
        )
        existing_sources = _bounded_existing_pdf_sources(
            source_rows, limit=existing_source_limit
        )
    except Exception:
        ...
        def _scan() -> list[Path]:
            ...
            for name in wanted_dir_names:
                ...
                try:
                    ...
                    continue
            # Folder numbering is not stable across regular, legal-aid, and
            # migrated cases.  Inspect direct children semantically so a
            # valid ``07_法院通知或程序裁定`` (or a future numbering variant)
            # cannot disappear from the calendar scanner.
            try:
                for child in folder.iterdir():
                    if not child.is_dir() or child.name.startswith("."):
                        continue
                    if _is_pdf_calendar_source_dir_name(child.name):
                        roots.append(child)
                        continue
                    if _pdf_calendar_container_dir_name(child.name) not in _PDF_COURT_CONTAINER_DIR_NAMES:
                        continue
                    for grandchild in child.iterdir():
                        if (
                            grandchild.is_dir()
                            and not grandchild.name.startswith(".")
                            and _is_pdf_calendar_source_dir_name(grandchild.name)
                        ):
                            roots.append(grandchild)
            except OSError:
                logging.getLogger(__name__).warning(
                    "calendar source directory scan failed: %s", folder, exc_info=True
                )
    ...
    def _strong_final_document_identity(
        pdf: Path,
        client_name: str,
    ) -> tuple[str, str] | None:
        if not path_has_judgment_folder(str(pdf)):
            return None
        name = re.sub(r"\s+", "", pdf.name).lower()
        if not re.search(
            r"(判決|裁定|不起訴處分|緩起訴處分|起訴書|支付命令|調解不成立)",
            name,
        ):
            return None
        # Do not merge generic names such as 判決.pdf across unrelated cases.
        if not re.search(r"\d{2,3}年度.{1,14}?字第?0*\d+號", name) and len(name) < 24:
            return None
        client = re.sub(r"[\s._\-]+", "", str(client_name or "")).lower()
        return name, client

    def _dedupe_known_cross_case_final_documents(
        items: list[tuple[int, int, float, str, Path, str, str]],
    ) -> list[tuple[int, int, float, str, Path, str, str]]:
        groups: dict[
            tuple[str, str],
            list[tuple[int, int, float, str, Path, str, str]],
        ...
        for item in items:
            identity = _strong_final_document_identity(item[4], item[6])
            if identity is not None:
                groups.setdefault(identity, []).append(item)

        drop_ids: set[int] = set()
        for members in groups.values():
            if len({item[5] for item in members}) < 2:
                continue
            known = [
                item
                for item in members
                if (
                    (item[5], str(item[4])) in existing_sources
                    or (item[5], item[4].name) in existing_sources
                )
            ]
            if not known:
                # Two unsourced copies may still represent legitimately
                # different matters; keep both for conservative review.
                continue
            canonical = sorted(
                known,
    ...
    def _append_pdf_candidate(pdf: Path, case_number: str, client_name: str, *, priority_final_doc: bool = False) -> None:
        if pdf.name.startswith(".") or pdf.name.startswith("~$"):
            return
        if not _is_allowed_calendar_source(pdf):
            return
        ...
        def _scan_children() -> list[Path]:
            ...
            for child in folder.iterdir():
                try:
                    if not child.is_dir() or child.name.startswith("."):
                        continue
                    if path_has_judgment_folder(child.name):
                        out.append(child)
                        continue
                    if _pdf_calendar_container_dir_name(child.name) not in _PDF_COURT_CONTAINER_DIR_NAMES:
                        continue
                    for grandchild in child.iterdir():
                        if (
                            grandchild.is_dir()
                            and not grandchild.name.startswith(".")
                            and path_has_judgment_folder(grandchild.name)
                        ):
    ...
    def _append_priority_final_doc_candidates() -> None:
        # Final judgments/rulings must remain visible even when a case was
        # prematurely or incorrectly marked closed.  Only future actionable
        # deadlines survive _active_pdf_todos, so scanning a bounded recent
        # inventory is safer than trusting the case status as a hard gate.
        ...
        for row in priority_rows or []:
            ...
            for candidate in local_case_path_candidates(raw_folder):
                ...
                if not _is_allowed_calendar_source(cand):
                    continue
    ...
    if filename_only and len(candidates) >= candidate_cap:
        candidates = _dedupe_known_cross_case_final_documents(candidates)
        candidates.sort(key=lambda item: (item[0], item[1], item[2], item[3]))
...
def osc_pdf_calendar_scan_api():
    ...
    try:
        ...
        hearing_conflict_checks_queued = 0
        hearing_conflict_checks_skipped_unchanged = 0
        hearing_conflict_checks_rejected = 0
        hearing_conflict_checks_duplicate = 0
        for path, target_case_number, target_client_name in target_specs:
            ...
            if identity_confirmation:
                ...
                if write and write_todos and item.get("case_number"):
                    confirmation_result = _insert_document_identity_confirmation(
                    ...
                    item["manual_confirmation_write"] = confirmation_result
                ...
                # Fail closed: a byte-identical final document already belongs
                # to another case, so this scan must not create a deadline or
                # a Google Calendar event under the current folder.
                continue
            if not write:
                continue
            todo_schedule_changed = False
            ...
            if write_todos and item.get("case_number"):
                ...
                todo_schedule_changed = bool(
                    int(insert_result.get("inserted") or 0)
                    or int(insert_result.get("updated") or 0)
                )
            for event in item.get("events") or []:
                calendar_schedule_changed = False
                if write_osc_calendar_events:
                    status = _insert_calendar_event(event, allow_duplicates=allow_duplicates)
                    if status == "inserted":
                        event_inserted += 1
                        calendar_schedule_changed = True
                    else:
                        event_skipped += 1
                if not (todo_schedule_changed or calendar_schedule_changed):
                    hearing_conflict_checks_skipped_unchanged += 1
                    continue
                try:
                    ...
                    candidate = {
                        **event,
                        "case_number": event.get("case_number") or item.get("case_number") or "",
                        "created_date": datetime.now().isoformat(sep=" "),
                        "source_kind": "court_notice_pdf",
                        "source_file": str(Path(item.get("path") or "")),
                    }
                    if classify_schedule(candidate) == "hearing":
                        ...
                        if bool(getattr(admission, "accepted", False)):
                            hearing_conflict_checks_queued += 1
                        elif getattr(admission, "reason", "") == "already_queued":
                            hearing_conflict_checks_duplicate += 1
                        else:
                            hearing_conflict_checks_rejected += 1
                except Exception:
                    logging.getLogger(__name__).warning(
                        "PDF hearing-conflict enqueue failed for %s",
                        item.get("path"),
                        exc_info=True,
                    )
        ...
        return jsonify(
            {
                ...
                "hearing_conflict_checks_queued": hearing_conflict_checks_queued,
                "hearing_conflict_checks_skipped_unchanged": hearing_conflict_checks_skipped_unchanged,
                "hearing_conflict_checks_rejected": hearing_conflict_checks_rejected,
                "hearing_conflict_checks_duplicate": hearing_conflict_checks_duplicate,
                "items": scanned,
                "message": message,
            }
        )
