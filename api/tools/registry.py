...
class ToolRegistry:
    ...
    def execute(
        ...
        schema_error = self._input_schema_error(entry.spec.input_schema, arguments)
        if schema_error:
            contract["status"] = "blocked"
            contract["validation"] = {"status": "failed", "reason": schema_error}
            if contract["verification"]["status"] == "pending":
                contract["verification"] = {"status": "not_run"}
            return self._result(entry.spec, success=False, error=schema_error, contract=contract)
        contract["validation"] = {"status": "passed"}
        ...
        confirmation_error = self._confirmation_error(entry.spec, context, arguments)
    ...
    def _confirmation_error(
        ...
        validator = spec.metadata.get("confirmation_validator")
        is_irreversible = normalize_tool_side_effect(spec.side_effect) in {
            ToolSideEffect.EXTERNAL_COMMIT,
            ToolSideEffect.DESTRUCTIVE,
        }
        # A caller-controlled nonempty token is never a confirmation binding for
        # an irreversible action.  Such tools must carry an expected token(s)
        # issued by the server, or a server-side validator owned by the spec.
        if is_irreversible and not expected and not callable(validator):
            return "confirmation_binding_unavailable"
        if callable(validator):
            try:
                accepted = self._invoke_handler(
                    validator,
                    context=context,
                    spec=spec,
            ...
            if isinstance(accepted, ToolResult):
                accepted = accepted.success
            elif isinstance(accepted, Mapping):
                accepted = accepted.get("ok", accepted.get("success", False))
    ...
    @staticmethod
    def _input_schema_error(schema: Mapping[str, Any], arguments: Mapping[str, Any]) -> str:
        """Validate the conservative JSON-schema subset accepted by ToolSpec.

        Tool calls are an untrusted boundary.  Unsupported schema shapes block
        execution instead of silently weakening a declared contract.
        """
        if not schema:
            return ""
        ...
        for name in required:
            if name not in arguments:
                return f"input_validation_failed:missing_required:{name}"
        if schema.get("additionalProperties") is False:
            extras = set(arguments).difference(properties)
            if extras:
                return f"input_validation_failed:unexpected_property:{sorted(extras)[0]}"
        for name, value in arguments.items():
            ...
            if expected_type is not None and not ToolRegistry._matches_schema_type(value, expected_type):
                return f"input_validation_failed:invalid_type:{name}"
            if "enum" in rule:
                allowed = rule["enum"]
                if not isinstance(allowed, (list, tuple)):
                    return "input_schema_invalid"
                if value not in allowed:
                    return f"input_validation_failed:invalid_enum:{name}"
        return ""
