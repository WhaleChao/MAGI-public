...
function bindTabs() {
    document.querySelectorAll(".tab-btn[data-tab]").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            // Keep the selected work area shareable and restore it after
            // navigation. Only real sidebar tabs are accepted below, so a
            // crafted query string cannot select an arbitrary element.
            try {
                const url = new URL(window.location.href);
                if (tabId && tabId !== "dashboard") url.searchParams.set("tab", tabId);
            ...
            if (tabId === "hearingConflict") _withLoading("載入衝庭與請假狀...", loadHearingConflictWorkbench);
...
async function dispatchDelegatedAction(act, t) {
    ...
    if (act === "osc-file-preview") return await openFilePreview(t.dataset.path || "", t.dataset.name || "");
    if (act === "osc-file-download") return await startFileDownload(t.dataset.path || "", t.dataset.name || "");
    ...
    if (act === "hearing-conflict-generate") return await generateHearingConflictAutomatic(Number(t.dataset.index));
...
function bindEvents() {
    [
        ...
        ["hearingConflictRefreshBtn", loadHearingConflictWorkbench, "衝庭排程重新掃描"],
        ["hearingConflictCheckBtn", checkHearingConflict, "衝庭檢查"],
        ["hearingConflictManualBtn", generateHearingConflictManual, "人工產生請假狀"],
...
function initThemeToggle() {
    ...
    function applyTheme(name) {
        if (name === "dark") {
            ...
            if (btn) {
                btn.textContent = "☾ 夜";
                btn.setAttribute("aria-label", "目前為夜，切換為日");
                btn.title = "切換為日";
            }
        } else {
            ...
            if (btn) {
                btn.textContent = "☀ 日";
                btn.setAttribute("aria-label", "目前為日，切換為夜");
                btn.title = "切換為夜";
            }
        }
    }
...
async function boot() {
    ...
    _safe("initHearingConflictControls", typeof initHearingConflictControls === "function" ? initHearingConflictControls : () => {});
    ...
    try {
        ...
        const tabExists = Array.from(document.querySelectorAll(".tab-btn[data-tab]")).some(
            button => button.dataset.tab === candidate
        );
        if (candidate && tabExists) {
            requestedTab = candidate;
        }
    } catch (_error) {}
    if (requestedTab) {
        jumpToPaperclipTab(requestedTab);
    } else {
