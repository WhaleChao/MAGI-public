...
function friendlyOscUiError(actionLabel, error) {
    // Do not pass API error strings or browser exception details to users;
    // they may include diagnostic IDs, paths, or implementation information.
    if (error && error.code === "authentication_required") return "登入已逾時，請重新登入後再試。";
    return `${actionLabel || "操作"}暫時無法完成，請稍後再試。`;
}
...
async function sendChat() {
    ...
    try {
        ...
        msg("sys", friendlyOscUiError("送出", e));
    }
}
...
async function loadMeta() {
    ...
    try {
        ...
        if (!res.ok || !data.ok) {
            ...
            dbBadge.textContent = `資料庫：暫時無法連線${foTag}${hint}`;
        ...
        dbBadge.classList.remove("ok");
        dbBadge.textContent = "資料庫：暫時無法連線，請稍後再試";
...
function reportUiError(actionLabel, error) {
    console.error(error);
    const body = friendlyOscUiError(actionLabel, error);
