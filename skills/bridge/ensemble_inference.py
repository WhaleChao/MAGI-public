    ...
    if heavy and os.environ.get("NVIDIA_NIM_ENABLE", "0").strip().lower() in {"1", "true", "yes", "on"}:
        try:
            ...
            if nim_result.get("success") and nim_result.get("response"):
                ...
                if os.environ.get("MAGI_HEAVY_STRICT_NIM_ALLOW_FALLBACK", "0").strip().lower() not in {
                    "1", "true", "yes", "on",
                }:
                    return ConsensusResult(
                        unanimous=False,
                ...
                logger.warning("ensemble heavy NIM failed, falling back to oMLX: %s", err)
            ...
            logger.warning("ensemble heavy NIM exception, falling back to oMLX: %s", exc)
