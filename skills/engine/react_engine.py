...
- 工具選擇必須精準：
  ...
  - 問「通譯判決分類、通譯實證研究、最高法院通譯表格」→ 用 run_skill，skill_name=interpreter-empirical-classifier
...
class ReActEngine:
    ...
    @staticmethod
    def _observation_is_unusable(observation: str) -> bool:
        """Return True when a tool did not provide enough facts to answer.

        Live route tests intentionally use TOOL_CALLED markers to verify that
        the model selected the right tool. Those markers are not factual
        observations, so the engine must not let the model invent a final
        answer from them. The same rule protects production failures.
        """
