...
Feature flags (all env vars):
  ...
  MAGI_CHANDRA_OCR_ENABLE=0       (私用版選用 Chandra OCR；需獨立 CLI/backend，預設 0)
...
業務紅線：
  ...
  - Chandra 僅作私用版 pdf-namer 低品質 OCR 的顯式 opt-in fallback；
    upstream 涉及 Qwen backend，需 MAGI_CHANDRA_PRIVATE_DEPLOYMENT 與
    MAGI_CHANDRA_ACCEPT_QWEN_BACKEND，不得阻塞主流程或進入公開版預設
