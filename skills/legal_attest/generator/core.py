...
def fill_name_address(namelist, addresslist,
                      ...
                      **kwargs):
    # kwargs is threaded through each action call as a mutable state bag
    # (contains painter, x_begin, y_begin, etc.) — propagated via return value.
...
def _fill_info_if_empty(**kwargs):
    # Use kwargs['painter'] to stay consistent with _fill_info_if_nonempty
    kwargs['painter'].draw_string(kwargs['x_begin'], kwargs['y_begin'], u'姓名：')
    kwargs['y_begin'] -= DETAIL_Y_INTERVAL
    kwargs['painter'].draw_string(kwargs['x_begin'], kwargs['y_begin'], u'詳細地址：')
    kwargs['y_begin'] -= DETAIL_Y_INTERVAL
    return kwargs
