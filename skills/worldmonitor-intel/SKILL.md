...
### 自動排程
每日由 cron scheduler 直接執行 `skills/worldmonitor-intel/action.py --task collect --no-reasoning --plain-output`，對應 `cron_jobs.json` 的 `job_worldmonitor_intel`。排程不經自然語言路由，避免被誤導到其他技能。
...
cron_jobs.json (每日)
    ↓ cron scheduler
worldmonitor-intel/action.py
    ↓ 網路抓取 + 結構式來源整理
本機 fallback / 可選 Melchior
    ↓ 報告輸出
