...
_MAX_RELATIVE_DEADLINE_DAYS = 3650


def _bounded_relative_deadline(document_date: datetime, days: int) -> Optional[datetime]:
    """Return a plausible relative deadline without allowing parser overflow."""

    if days <= 0 or days > _MAX_RELATIVE_DEADLINE_DAYS:
        logging.getLogger(__name__).warning(
            "skipping implausible relative deadline duration: days=%s", days
        )
        return None
    try:
...
def extract_document_date_from_filename(filename: str, file_path: str = "") -> Optional[datetime]:
    ...
    # Filed court documents also commonly carry the received date at the end
    # of the normalized filename, for example:
    #   士林地院刑事判決_吳倩茹_115.7.27.pdf
    # Treating only a leading date as explicit made this form silently fall
    # back to the filesystem mtime and shifted deadlines whenever a file was
    # copied or indexed on the following day.
    stem = os.path.splitext(name)[0]
    m = re.search(r"(?<!\d)(20\d{2}|\d{3})[./-](\d{1,2})[./-](\d{1,2})$", stem)
    if m:
        try:
            raw_year = int(m.group(1))
            year = raw_year if raw_year >= 1911 else raw_year + 1911
            if 2000 <= year <= datetime.now().year + 3:
                return datetime(year, int(m.group(2)), int(m.group(3)))
        except Exception:
            logging.getLogger(__name__).debug(
                "invalid trailing document date in %s", name, exc_info=True
            )
...
def _hearing_datetime(
    *,
    ...
    explicit_year: bool,
    allow_yearless_rollover: bool = True,
    ...
    try:
        ...
        if (
            allow_yearless_rollover
            and not explicit_year
            and dt.date() < document_date.date() - timedelta(days=30)
        ):
...
def _extract_hearing_sequence_todos(
    filename: str,
    document_date: datetime,
    base_year: Optional[int] = None,
    *,
    has_explicit_document_date: bool = True,
) -> List[Dict]:
    ...
    for m in explicit_pat.finditer(filename):
        parsed = _hearing_datetime(
            ...
            explicit_year=True,
            allow_yearless_rollover=has_explicit_document_date,
        )
        if parsed:
    ...
    for m in yearless_date_only_pat.finditer(filename):
        ...
        try:
            ...
            if has_explicit_document_date and dt.date() < document_date.date() - timedelta(days=30):
...
def extract_todos_from_filename(
    ...
    *,
    ...
    explicit_document_date = extract_document_date_from_filename(filename, file_path)
    document_date = explicit_document_date
        ...
        for pattern_data in all_patterns[todo_type]:
            ...
            try:
                ...
                if pattern_type in ("relative", "relative_chinese"):
                    ...
                    deadline = _bounded_relative_deadline(document_date, days)
                    if deadline is None:
                        continue
                    ...
                    if (
                        pattern_type == "absolute_time"
                        and explicit_document_date is not None
                        and dt.date() < document_date.date() - timedelta(days=30)
                        and year_to_use >= document_date.year
                    ):
    ...
    for todo in _extract_hearing_sequence_todos(
        filename,
        document_date,
        base_year,
        has_explicit_document_date=explicit_document_date is not None,
    ):
        _append_unique_todo(todos, todo)
