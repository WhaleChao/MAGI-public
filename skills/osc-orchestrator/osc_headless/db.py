...
from api.domains.calendar_sync_policy import osc_only_calendar_review_sql
...
def db_config_from_env(prefix: str = "OSC_DB_") -> DBConfig:
    ...
    if _has_explicit_env(prefix):
        # Capture the explicit port before any fallback lookup.  The legacy
        # failover resolver mutates OSC_DB_PORT as a side effect; evaluating it
        # as an eager default used to overwrite a caller-bound disposable (or
        # otherwise explicit) endpoint before `_get("PORT", ...)` ran.
        explicit_host = os.environ.get(f"{prefix}HOST", "").strip()
        explicit_port = os.environ.get(f"{prefix}PORT", "").strip()
        return DBConfig(
            host=explicit_host or _failover_host(),
            port=int(explicit_port or "3306"),
...
def ensure_cases_schema(conn: mysql.connector.MySQLConnection) -> None:
    ...
    try:
        ...
        if "legal_aid_status" not in cols:
            _add("ALTER TABLE `cases` ADD COLUMN `legal_aid_status` VARCHAR(50) DEFAULT ''")
...
def upsert_case(
    ...
    *,
    ...
    try:
        ...
        # Insert new
        stable_id = "idx-" + hashlib.sha256(cn.encode("utf-8")).hexdigest()[:20]
        cur.execute(
            """
            INSERT INTO `cases`
              (`id`,`case_number`,`client_name`,`case_type`,`case_category`,`case_reason`,`folder_path`,
