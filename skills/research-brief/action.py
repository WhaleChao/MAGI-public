...
_RUNTIME_DIR = _RUNTIME_ROOT / "research_brief"
...
def _ensure_seeds_bootstrapped() -> None:
    ...
    if os.environ.get("RESEARCH_BRIEF_DISABLE_SEED_BOOTSTRAP", "").strip().lower() in {
        "1", "true", "yes", "on"
    }:
        return
...
def _fetch_namespace(
    ...
    for src in ns.get("sources", []):
        if "<MOCK_PORT>" in str(src.get("url") or ""):
            fixture_url = os.environ.get("RESEARCH_BRIEF_FIXTURE_SOURCE_URL", "").strip()
            fixture_root = Path(
            ...
            if (
                ...
                or not fixture_url.startswith("http://127.0.0.1:")
            ):
                raise RuntimeError("research brief fixture source failed closed")
            src = {**src, "url": fixture_url}
        try:
...
def _ingest_entries_to_memory(namespace: str, entries: List[Dict[str, Any]]) -> int:
    ...
    fixture_sink = os.environ.get("RESEARCH_BRIEF_MEMORY_SINK_PATH", "").strip()
    if fixture_sink:
        sink = Path(fixture_sink).expanduser()
        fixture_root = Path(
            os.environ.get("MAGI_V3_SCHEDULE_FIXTURE_ROOT", "")
        ).expanduser()
        if (
            ...
            or not (fixture_root / ".magi-v3-schedule-fixture").is_file()
            or fixture_root.resolve() not in sink.resolve().parents
        ):
            logger.error("research brief fixture memory sink failed closed")
            return 0
        ...
        return already_ingested + len(payloads)
    try:
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            if result.get("ok") is True and failed == 0 and (inserted + skipped + blocked) >= len(payloads):
                return already_ingested + len(payloads)
            return already_ingested + inserted + skipped
        if isinstance(result, int):
            return already_ingested + result
        return already_ingested + len(payloads)
    except Exception as e:
...
def task_digest(namespace: Optional[str] = None, *, notify: bool = True) -> int:
    ...
    for n in names:
        ...
        try:
            ...
            ingested = 0
        ingest_degraded = bool(entries and ingested < len(entries))
        if ingest_degraded:
