...
_MUTABLE_STATIC_DIR = (os.environ.get("MAGI_MUTABLE_STATIC_DIR") or "").strip()
AGENDA_FILE = os.path.join(_AGENT_DIR, "nightly_council_agenda.md") if _AGENT_DIR else f"{_MAGI_ROOT}/nightly_council_agenda.md"
MINUTES_FILE = os.path.join(_AGENT_DIR, "nightly_council_minutes.md") if _AGENT_DIR else f"{_MAGI_ROOT}/nightly_council_minutes.md"
DAEMON_LOG_FILE = (
    os.environ.get("MAGI_DAEMON_LOG_PATH", "").strip()
    or (os.path.join(_AGENT_DIR, "daemon.log") if _AGENT_DIR else f"{_MAGI_ROOT}/daemon.log")
)
...
def _enabled_cron_jobs() -> set[str]:
    try:
        ...
        jobs = load_bound_cron_jobs(_MAGI_ROOT).jobs
        return {
            str(job.get("id") or "")
            for job in jobs
...
def _optional_watcher_anomalies(bridge, watcher_online: bool) -> dict:
    """Avoid querying an optional Watcher after its health probe failed."""

    if bridge is None or not watcher_online:
        return {}
    try:
        result = bridge.get_anomalies(unresolved_only=True)
    except Exception:
        logging.getLogger(__name__).debug(
            "optional Watcher anomaly query failed", exc_info=True
        )
        return {}
...
def start_night_talk():
    ...
    issues = []
    try:
        anomalies = _optional_watcher_anomalies(watcher_bridge, watcher_online)
    ...
    try:
        ...
        try:
            _report_dir = os.path.join(_EXPORTS_DIR, "night-patrol") if _EXPORTS_DIR else f"{_MAGI_ROOT}/reports"
            _reports = sorted(
        ...
        try:
            _lint_report = os.path.join(
                _MUTABLE_STATIC_DIR or os.path.join(_MAGI_ROOT, "static"),
                "knowledge_lint_latest.json",
            )
    ...
    # --- PHASE 4: ARCHIVE ---
    try:
        os.makedirs(os.path.dirname(MINUTES_FILE) or ".", exist_ok=True)
