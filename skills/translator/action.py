...
if _current_fork_depth >= _MAX_FORK_DEPTH:
    ...
    sys.exit(87)
# Keep imports side-effect free.  Each recursive translator child receives the
# increment explicitly at the only subprocess boundary below.
...
def translate(payload: dict) -> dict:
    ...
    env[_FORK_DEPTH_ENV] = str(_current_fork_depth + 1)
    try:
        cp = subprocess.run(
            cmd,
