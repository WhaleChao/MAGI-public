...
def laf_report_command_help() -> str:
    ...
    return (
        ...
        "可直接說：\n"
        "1. `幫我做蕭仁俊開辦回報`\n"
        "2. `幫江家瑋做疑義回報 原因資力不合標準`\n"
        "3. `劉信義訴訟中費用支付回報 說明裁判費2000`\n"
        "4. `張慧敏二階段回報`\n"
        "5. `蔡旭欽結案回報 原因本案無閱卷必要`\n\n"
        "6. `江家瑋受扶助人撤回回報 原因申請人撤回`\n\n"
        "也可用結構化寫法：\n"
        ...
        "直接說：\n"
        "- `劉信義 已開辦` → 更新狀態為「進行中」\n"
        "- `宣愛華 改定子女 已報結` → 同名多案時用案由消歧義\n"
        "- `宣愛華 消債 已報結` → 支援簡寫（消債＝消費者債務清理）\n"
...
_STATUS_MAP = {
    "開辦": "進行中",
    # 報結/結案/撤回/撤案 → 新主狀態「已結案」（deprecated alias「已報結」保留至 2026-07-26）
    "報結": "已結案",
    "結案": "已結案",
    "撤回": "已結案",
    "撤案": "已結案",
