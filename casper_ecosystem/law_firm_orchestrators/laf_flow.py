...
# ---------------------------------------------------------------------------
# Progress submit pending registration & resolution (Plan C 兩階段確認碼)
# ---------------------------------------------------------------------------

def register_laf_progress_submit_pending(
    orch, *, platform: str, requester_user_id: str, payload: dict, result_data: dict
) -> str:
    """進度回報：登記 pending submit，產生 6-hex token，寫入獨立 pending file。

    Returns the token string (不是 dict，與 go_live 版稍有不同以符合 api.domains.laf_flow 相同介面)。
    使用獨立 pending file (laf_progress_submit_pending.json) 與 go_live 嚴格隔離。
    """
    from api.domains.laf_flow import (
        register_laf_progress_submit_pending as _api_register,
    )
...
def resolve_laf_progress_pending_token(orch, token: str):
    """從 pending file 驗證 progress token，回傳 entry dict 或 None。

    kind 嚴格匹配 "laf_progress_submit"，不誤吃 go_live token。
    """
    from api.domains.laf_flow import (
        resolve_laf_progress_pending_token as _api_resolve,
    )
    return _api_resolve(orch, token)
